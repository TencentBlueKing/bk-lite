...
## Task 1: P0 基础设施 — model_reflection + A/B 端骨架 + 02/03/04 schema
...
- Produces:
  ...
  - `test_stargazer_prometheus_alignment.ALIGNMENT_COVERED_MODEL_IDS: list[str]`
  - `test_cmdb_vm_format_alignment.ALIGNMENT_COVERED_MODEL_IDS: list[str]`
...
def test_a_alignment_metric_name_suffix(model_id, load_fixture, runner_plugin_factory):
    ...
    if model_id.startswith("k8s_"):
        return  # 跳过,K8s 走 minimal path 特殊处理
...
### Task 3.1: hwcloud(11 子对象)

**Files:**
- Create: `server/apps/cmdb/tests/e2e/fixtures/hwcloud/{01,02,03,04}.json`
- Create: `server/apps/cmdb/tests/e2e/schemas/hwcloud/{01,04}.schema.json`
- Create: `server/apps/cmdb/tests/e2e/test_hwcloud_pipeline.py`
- Modify: `server/apps/cmdb/tests/e2e/conftest.py`(`_MODEL_RUNNER_MAP` 加 hwcloud)

- [ ] **Step 1: 读华为云 ECS API 文档**

读 https://support.huaweicloud.com/api-ecs/ecs_02_0101.html,了解 ECS 实例 API 响应结构

- [ ] **Step 2: 写 01 fixture(hwcloud_ecs 真实样本)**

参考华为云 ECS ListServersDetails 响应结构,写 100+ 行 fixture(JSON 格式对齐 aliyun 模板)

- [ ] **Step 3: 写 02 / 03 fixture**

- [ ] **Step 4: 写 04 expected(11 子对象预期,hwcloud_ecs / hwcloud_evs / hwcloud_vpc / hwcloud_obs / hwcloud_subnet / hwcloud_eip / hwcloud_sg / hwcloud_elb / hwcloud_rds / hwcloud_dcs)**

- [ ] **Step 5: 写 01 / 04 schema**

- [ ] **Step 6: 写 test_hwcloud_pipeline.py(5 个测试:step1 / pipeline / drift / A 端 / B 端)**

- [ ] **Step 7: conftest._MODEL_RUNNER_MAP 加 hwcloud**

- [ ] **Step 8: A/B 端 ALIGNMENT_COVERED_MODEL_IDS 加 hwcloud**
...
git add server/apps/cmdb/tests/e2e/fixtures/hwcloud/ \
        ...
        server/apps/cmdb/tests/e2e/test_stargazer_prometheus_alignment.py \
        server/apps/cmdb/tests/e2e/test_cmdb_vm_format_alignment.py
git commit -m "test(cmdb/e2e): Task 3.1 - hwcloud 云采集(11 子对象)+ A/B 端覆盖"
```
...
Expected: 7 × 3 + 7 × 2 = 35 tests PASS
...
"""字段漂移报告测试 —— 跑全量 e2e 后,生成 model 定义 vs fixture / 04 expected 的字段差异报告。"""
import json
import subprocess
...
def test_drift_report_generates_summary():
    """跑完全量 e2e 后,字段漂移报告必须生成 summary。"""
    # 触发 e2e
    result = subprocess.run(
        ["python", "-m", "pytest", "apps/cmdb/tests/e2e/", "--tb=no", "-q"],
    ...
    assert "missing_fields" in summary or "extra_fields" in summary
```

- [ ] **Step 2: 实现 drift_report.py**

(略,500+ 行 Python 脚本,跑全量 e2e 后扫描 fixtures + model 反射,生成 drift 报告)
