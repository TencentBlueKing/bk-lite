...
| --- | --- | --- | --- | --- | --- |
| 1. 隔离环境、基线与对照 | `package.json`；Vitest 的 `src/**/*.test.ts` 与 `*.real-llm.test.ts` | mock 与真实 LLM 基线分层 | 直接复制 Node/Vitest 命令 | Django 模型、迁移、API 依赖测试数据库 | OpsPilot 隔离命令已 `12 passed in 11.17s` 并退出 0；coverage 收尾问题已单独定界 |
