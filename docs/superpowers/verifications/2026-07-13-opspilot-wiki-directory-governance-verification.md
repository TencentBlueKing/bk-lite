...
| --- | --- | --- | --- | --- | --- |
| 1. 隔离环境、基线与对照 | `package.json`；Vitest 的 `src/**/*.test.ts` 与 `*.real-llm.test.ts`；集中测试辅助位于 `src/test-helpers/` | mock 与真实 LLM 基线分层；测试对象通过集中 helper 构造 | 直接复制 Node/Vitest 命令；用文件路径或测试专用对象替代 Django 领域入口 | Django 模型、迁移、API 依赖测试数据库；新增 Wiki-local `WikiFactory`/fixture，有效页面统一调用 `create_manual_page`，显式 legacy escape hatch 只服务脏数据契约；AST guard 约束后续目录、generation、structure 测试 | 基线隔离命令已 `12 passed in 11.17s` 并退出 0；Task 1.4 factory 契约、模型与重建联合复验 `26 passed in 9.29s` 并退出 0；规格与代码质量审查通过；coverage 收尾问题已单独定界 |
