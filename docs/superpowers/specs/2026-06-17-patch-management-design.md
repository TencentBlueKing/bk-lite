...
- Linux：Ubuntu 20.04/22.04、Debian 11/12、CentOS 7、Rocky Linux 8/9、RHEL 8/9。
- 不在支持矩阵内的目标可以录入和展示，但扫描结果必须返回 `unsupported`，不得混入 `failed`。
...
- `unsupported`：目标系统不在 MVP 支持矩阵内，或目标缺少当前扫描方式所需的基础能力。
...
- 目标系统不在 MVP 支持矩阵内时，结果记录为 `unsupported`。
...
- `blocked_unsupported`：目标系统不在支持矩阵内，阻断该目标。
...
- 已安装补丁必须幂等跳过，不得重复安装。
- 不适用、缺包、目标不可达、目标不支持必须在执行前拦截，并写入目标级结果。
...
- 独立 Ansible 目标扫描。
- Node NATS 目标扫描。
- `missing / installed / not_applicable / unsupported / failed` 状态判断。
- 周期扫描调度。
- 扫描结果主机视角/补丁视角查询。
- 扫描结果 Excel 导出。
- 扫描通知配置、通知触发、通知日志。
...
4. 执行一次扫描任务，得到缺失、已安装、不适用、不支持、失败中的至少两类结果。
5. 导出扫描结果 Excel。
6. 扫描任务完成后发送通知，并记录通知日志。
7. 从扫描结果发起安装任务，查看并确认安装预览矩阵，指定未来执行时间。
