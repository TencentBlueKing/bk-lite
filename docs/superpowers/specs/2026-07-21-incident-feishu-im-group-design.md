...
| `GET /api/v1/alerts/incidents/{id}/im-group/` | 当前绑定、状态和成员明细 | 可查看 Incident |
...
| `DELETE .../im-group/` | 解除绑定，不删除外部群 | Incident 负责人 |
...
入口位于 Incident 详情现有“协作”区域，不新增一级页面。
...
┌──────────────────────────────────────────────────────────┐
│ 飞书协作群                                                │
│ 建群后可将负责人和协作人加入群聊，并持续同步新增人员。     │
│                                           [ 一键拉群 ]     │
└──────────────────────────────────────────────────────────┘
```

非负责人可看到说明，但不显示“一键拉群”按钮。
...
│ 建群负责人[ 张三（zhangsan）             ▾ ] │
│                                                │
│ 首批成员  已映射 4  未映射 2  冲突 0 [查看]  │
│   ✓ 张三 负责人       open_id 已映射           │
│   ! 李四 协作人       未映射 [去系统管理]      │
│                                                │
│ [✓] 持续同步新增 Incident 人员                 │
│     只增不减；移出 Incident 不会自动退群。      │
│                         [取消] [创建飞书群]     │
└────────────────────────────────────────────────┘
```

- 渠道仅列出当前用户可访问、启用、运行态就绪并支持 `im_group` 的飞书渠道。
- 切换渠道后重新加载映射预览和负责人候选。
- 群名默认由 Incident 编号和标题生成，可编辑，并按飞书限制前后端双重校验。
- 未映射成员不阻止提交，但必须明确显示；负责人全部未映射时禁用提交并给出修复入口。
- 创建按钮提交后立即关闭弹窗并进入“创建中”，防止重复点击。
...
│ [INC-001] 数据库连接异常                                 │
│ 渠道：生产环境飞书应用  持续同步：开启                   │
│ 已入群 4 · 待映射 2 · 失败 1   最近同步 14:32            │
│ 李四未映射；王五加入失败，请检查飞书用户状态。            │
│ [查看详情] [重试拉人] [暂停同步] [打开群聊] [解除绑定]   │
└──────────────────────────────────────────────────────────┘
```
...
|---|---|---|
| 未建群 | 无有效绑定 | 一键拉群 |
| 创建中 | `pending_create/creating` | 查看进度；禁用重复创建 |
| 正常运行 | `active` 且无待处理成员 | 查看详情、暂停 |
| 部分成功 | `active_partial` 或存在待映射/失败成员 | 重试拉人、修复映射 |
| 已暂停 | `paused` | 查看原因、恢复 |
| 配置异常 | `degraded/create_failed` | 查看错误、重试或解绑重建 |

“查看详情”使用抽屉展示成员、角色、映射状态、入群状态、最近错误和更新时间。默认按“失败/待处理优先”排序，可筛选全部、待映射、失败、已入群。

### 8.4 管理操作

- 持续同步开关：开启后立即对账；关闭只停止自动新增，不影响已在群成员。
- 暂停/恢复：暂停需显示影响说明；恢复立即触发一次对账。
- 重试拉人：仅重试未映射重新解析和失败/待处理成员，不重建群、不重复处理已入群成员。
- 打开群聊：只有后端返回可靠的飞书跳转链接时显示；否则提供复制群 ID，不拼装未经验证的 URL。
- 解除绑定：二次确认文案必须说明“只解除 BK-Lite 关联，不会删除飞书群；再次建群会创建一个新群”。
- 非负责人仅能查看状态和详情，不渲染管理按钮；服务端仍做权限校验。

### 8.5 刷新策略与可用性

- 创建中页面短轮询状态；进入稳定状态后停止轮询，窗口重新聚焦时刷新。
- 操作提交使用按钮 loading 和幂等请求，避免连点。
- 状态不能只靠颜色表达，必须有文字、图标和下一步说明。
- 异步错误显示安全、可行动的业务文案；技术错误码可在详情中复制用于排障。
