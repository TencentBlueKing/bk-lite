...
- `pyproject.toml` 与 `uv.lock` 不一致：镜像中的 `uv sync --locked` 及提交前的锁文件检查均失败；不允许只改声明不更新锁文件。
...
1. RED：增加镜像依赖合同检查，要求 Dockerfile 的默认索引与 Nexus 分支均执行 `uv sync --locked --all-groups --all-extras`；当前配置缺少 `--locked`，合同失败。
2. 行为证明：在临时目录构造已过期的 `pyproject.toml`/`uv.lock`，实际执行 `uv sync --locked --offline`，断言命令非零且明确报告锁文件需要更新。
3. GREEN：修改 Dockerfile，使合同检查通过。
4. 锁文件一致性：执行 `uv lock --check`。
5. 环境解析：在同步环境中断言：
   - `importlib.metadata.version("sanic") == "24.6.0"`；
   - `importlib.metadata.version("tracerite") == "1.1.3"`；
   - `hasattr(tracerite.html, "style")`；
   - `import sanic` 成功。
6. 启动烟测：确认 `which sanic` 和 `which python` 均位于 `/app/.venv/bin`，并启动 service/worker 至完成模块导入，不连接真实生产 Redis 或 NATS。
7. 执行 Stargazer 最小质量门禁 `make lint`。
