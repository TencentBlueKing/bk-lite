...
- 最近更新：2026-09-20
- 当前规格：本文件；实现边界见 `specs/capabilities/legacy-ard-modules-system-mgmt.md` §4.2
...
- 下拉再加未停用。
- 引用列按真实引用展示。计数问询与拦截规则见「跨模块契约」：确认无引用显示 0；无数据源显示「—」；删除、改组织只在确认存在引用时拦截。
...
- 列表芯片：成功返回且 `count > 0` 的模块按模块加总（如「CMDB 2」「监控 1」）。至少一家成功且合计为 0 → 显示「0」。两家都失败或都未接线 → 无数据源，显示「—」。
- 一家成功一家失败：只展示成功那家的正数芯片；成功那家为 0 则显示「0」。失败那家不参与拦截。
- 删除、改组织：只在任一家**确认** `count > 0` 时拦截。问询失败、超时、无订阅者、`result: false` 或合计 0 → 放行。
- 列表询问建议短超时（数秒），避免拖垮仓库页；删除/改组织同样短超时，超时不当成仍有引用。
...
- 监控实现 `monitor_count_credential_refs`；作业等计数口后置。CMDB handler 已接线。
...
- 曾写引用列禁止写死 0，询问失败、超时在删除/改组织时按仍有引用处理；2026-09-20 改为：确认无引用显示 0；无数据源仍为「—」；只在确认存在引用时拦截，问询失败放行。
...
- 2026-09-20：引用列确认 0 显示 0；删除/改组织只在确认存在引用时拦截，问询失败放行。
