...
- 列表只展示名称，不常驻 `credential_id`、不提供行内复制。查看/编辑抽屉标题也不展示 ID。业务表单用 `CredentialPicker` 按名称选用，页面不手填 ID。分类栏不展示分类个数，分类项和「全部类型」不展示数量。「全部类型」右侧类型胶囊展示该类型全库 `credential_count`（含停用、不按组织，与类型页「凭据数」同一字段）；组织/搜索只裁表格。类型只 Tag；分类列（类型 Tab）双行中文+英文。名称旁无装饰图标。停用凭据用二次确认，不预检引用。
