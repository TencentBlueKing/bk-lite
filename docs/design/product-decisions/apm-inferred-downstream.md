...
- MVP 唯一产品入口是 **服务拓扑**（`/apm/services` 下的拓扑页）。应用列表、服务目录、接入实例、应用详情子拓扑第一版都不展示推断节点。
...
- **第一版只做服务拓扑。** 推断查询挂在现有拓扑构图（有界 Trace 样本）上，打开服务拓扑时整窗一次取数，不按服务轮询 VT。应用详情子拓扑可以复用同一构图，但本切片不改那条页面，避免和应用级 KPI / 圈选范围缠在一起。原因：拓扑已经在回答「谁连谁」；目录和应用卡一旦出现 mysql，就会被理解成 APM 服务。
...
6. 应用列表、服务列表、接入实例、应用详情子拓扑仍只有插桩服务；mysql 不会多出一行。

## 明确后置

- 应用详情子拓扑展示推断下游（可复用服务拓扑同一构图，另开切片）。
- 应用列表 / 服务目录展示推断下游。
