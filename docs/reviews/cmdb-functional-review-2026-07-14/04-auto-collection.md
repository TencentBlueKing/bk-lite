...
资源与审计同样不满足生产边界。IP 范围在 CMDB Worker 内无上限展开为列表，目标×凭据逐个执行并把全部 collect/format/raw 数据聚合后写入单行 JSON；过期清理则按任务全量查询图实例、全量收集 ID、一次批删，并把完整 ID 集合放进返回值和日志。即时和过期删除都没有实例删除审计。节点下发 debug 日志还记录完整 `node_params`，而该对象由解密凭据生成并包含 `env_config` 密钥值。

本域确认 6 个主 Finding：P0 1 个、P1 5 个、P2/P3 0 个，编号为 `CMDB-F20`–`CMDB-F25`。采集 CRUD 外部同步无持久化恢复、实例 merge 绕过统一批写编排、关联按名称裸解析分别与既有 `CMDB-F04/F11`、`CMDB-F10/F11`、`CMDB-F14` 同根因，仅记录跨域证据，不重复计数。Recommendation 为 **Request changes**。
...
- Root cause category: 异步生命周期不完整
- Evidence: 抢占首先执行 `filter(exec_status=RUNNING, task_id=execution_id).update(...)`；同 token 重投时该更新每次都返回 1，所有 Worker 都取得任务。timeout/新 execution 后，`_save_collect_result_if_current` 能拒绝旧结果，但 `Management` 的图实体、边、自动关系和审计调用只携带持久任务 ID，不读取当前 execution ID，也没有 lease owner/fencing token。已有旧 Worker 测试在采集函数内部把 ORM task_id 改为 B，随后仅断言 A 的摘要未覆盖 B，没有让 A 执行真实图写。
...
- Long-term design note: `task_id` 不能同时承担持久采集任务 ID 和 execution owner 语义；使用独立 execution 记录保存 owner、generation、deadline、阶段和批次进度。
...
- Evidence: IP start-end 用 Python `range` 全量扩为 list，没有地址数上限；派发把 targets、pending keys、attempts、collect_data、format_data、raw_data 全量留在内存，最多再乘 3 组凭据，最终写进 `CollectModels` 单行 JSON。过期清理 `query_entity` 全量读一个任务的实例，构建全量 expired_ids，一次 `batch_delete_entity`，日报返回并日志输出全部 delete_ids。没有 rows/bytes/attempts/deadline/JSON 大小预算或游标。
...
### Finding CMDB-F25：节点参数 debug 日志包含解密凭据与远端响应

- Severity: P1
- Location: `server/apps/cmdb/services/collect_service.py:390-412`；`server/apps/cmdb/node_configs/base.py:51-56,167-188,235-247`；SSH/数据库/云/SNMP NodeParams 的 `env_config`
- Root cause category: 敏感信息处理缺失
- Evidence: NodeParams 从 `instance.decrypt_credentials` 构建；`push_params` 包含 `env_config`，各驱动将 password、access secret、SNMP community/authkey/privkey 放入其中。`push_butch_node_params` 直接以 `%s` 记录完整 node_params，并继续记录完整 RPC result。debug 日志一旦在生产开启或被集中采集，密钥值会落盘。
- Trigger: create/update 非 K8s 采集任务并启用 debug 日志；RPC 错误响应回显配置。
- Impact: SSH/数据库/云/SNMP 凭据进入应用和日志平台，扩大横向移动与长期泄露面。
- Why existing tests missed it: CRUD 测试 Mock 掉 push 方法；没有日志捕获与 canary secret 断言。
- Minimal safe fix: 日志只记录 task_id、节点数、配置 ID、RPC 状态和稳定错误码；在 logger/RPC 边界递归脱敏 secret/env/password/token/community/key 字段，禁止记录请求/响应正文。
- Required tests: 各驱动 canary secret 不出现在日志、异常、collect_digest 和 RPC 诊断；debug/info/error 级别全覆盖；嵌套与列表字段递归脱敏。
- Long-term design note: 统一 SecretValue/RedactedPayload 类型，避免调用者靠约定不打印普通 dict。
...
6. 日志是否足够且不泄密：execution_id、batch、target outcome 指标不足；节点参数日志确定包含 secret 风险，cleanup 还打印全量 ID。
...
先关闭 P0 `CMDB-F20`：建立真正单 owner 的 execution、同 token 重投幂等和覆盖所有图/审计副作用的 fencing。随后修复 `CMDB-F21/F22`，让每个目标的最终 outcome 可见并以结构化错误码驱动凭据轮换；为执行与清理建立批次、内存、JSON 和 deadline 预算；补齐删除审计和全链路 secret 脱敏。跨域 `CMDB-F04/F10/F11/F14` 也必须在统一采集 Operation 中收敛。仅保留当前结果 token 条件更新、提高 timeout 或增加成功路径 Mock，不能批准生产。
