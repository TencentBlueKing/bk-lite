...
- 业务承诺：分类/模型/字段/唯一规则/自动关联规则/展示字段/公共枚举形成可授权、可校验、可恢复的模型主数据；字段分组与枚举库是关系库投影，图数据库承载模型、关系和实例展示投影；Enterprise 能力经注册表扩展且社区缺失时安全降级。
...
- 未验证项：Enterprise overlay 实现不在当前仓库；真实 FalkorDB 故障/恢复、Celery broker 故障与重投、公共枚举大规模实例 `_display` 重建、PostgreSQL/MySQL JSON 行为和并发写未执行。主 Findings：`CMDB-F01`–`CMDB-F07`，详见 [01-model-governance.md](01-model-governance.md)。
