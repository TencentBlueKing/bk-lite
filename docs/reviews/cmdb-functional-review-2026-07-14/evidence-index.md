...
- 核心调用链：task/config PUT → `update_task` → `NodeMgmtSyncConfig` + django-celery-beat → 全部系统 CollectModels NodeParams delete/push；run_sync/Beat → `NodeMgmt.cloud_region_list/node_list` 分页 → 按 region 选 container access point → 全量 host map → 缺失 host `InstanceManage.instance_create(system)` → 区域 `CollectModels` 先查后建/更新 → NodeMgmt 节点参数 delete/push → `NodeMgmtSyncRun`/last_sync_at；run_collect/Beat → 区域任务 → `CollectModelService.exec_task` 提交 Celery → 父 Run 立即终态/last_collect_at；latest/display → 全局 Run 或全部系统 CollectModels 聚合。Alerts k8s_meta → `AlertSource` 本地元数据；snmp_trap_nodes → 当前用户/组织 permission_data → `NodeMgmt.node_list` → 无本地运行持久化。
...
- 结果：首次命令被沙箱拒绝读取 uv cache，退出 2、未收集；受控权限重跑退出 0，36 passed in 2.90s。有效证明顶层异常 FAILED、单节点/区域任务失败继续并 PARTIAL、正常分页和序列化；没有证明 View/组织、真实更新、重复运行、租约/恢复、异常分页、实际采集终态或脱敏。
- 覆盖率：Node sync Service 64%、Model 100%，合计 66%；View 未被 brief 两文件导入，coverage 无该模块行。本域未达相关模块 80%/核心路径 90% 目标。
- 未验证项：真实 NodeMgmt/NATS、Celery broker/多 Worker/硬退出、FalkorDB、MySQL/PostgreSQL、并发配置/区域任务、跨组织普通用户、NodeMgmt 重复页与大规模节点、NodeParams delete/push 部分失败、实际 CollectModels Worker 终态。主 Findings `CMDB-F44`–`CMDB-F49`（P0 3/P1 2/P2 1）；提交即成功与无恢复/错误泄露引用 `CMDB-F04/F11/F25`。Recommendation Block，详见 [09-node-sync.md](09-node-sync.md)。
