...
- 结果：首次命令被沙箱拒绝读取 uv cache，退出 2、未收集；受控权限重跑退出 0，82 passed in 3.28s；提交前新鲜重跑仍为 82 passed in 3.16s。有效证明 checkpoint/Delivery 事务、SHA 去重、broker 重派、退避/永久错误、15 分钟恢复、attempt_count 旧代次保护和 Mirror 100 RPC 分片；没有证明 View 授权、真实多 Worker/渠道、外部成功后崩溃、Mirror 部分成功，或规则/Delivery 稳定游标、per-rule 总预算/deadline/checkpoint。
...
- 未验证项：真实 FalkorDB/Neo4j、Celery broker/多 Worker、SystemMgmt NATS/邮件/机器人/webhook、MySQL/PostgreSQL；规则组织/模型/实例/条件/收件人/渠道负向权限、ChangeRecord 两组织查询/详情/导出、外部成功而 SENT 未提交、Mirror 单条事务回滚/失败重试/批内第 N 条失败/5 次终态；规则/Delivery 稳定 cursor 与公平性、中断续跑、实例异常 count/重复页、关系 fallback RPC、关系/ChangeRecord/Delivery 总预算、deadline、长事务和脱敏。主 Findings `CMDB-F58`–`CMDB-F61`（P0 2/P1 2）；`CMDB-F61` 负责订阅资源边界，`CMDB-F23` 仅作相似模式/Mirror 批量参考，敏感日志引用 `CMDB-F25`。Recommendation Block，详见 [11-change-subscription.md](11-change-subscription.md)。
