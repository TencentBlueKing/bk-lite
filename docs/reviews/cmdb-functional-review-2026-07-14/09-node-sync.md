...
| --- | --- | --- | --- |
| `CMDB-F44` | 已解决（代码合同） | `test_view_permission_can_read_but_cannot_update_both_config_urls`、`test_execute_permission_can_update_both_config_urls`、`test_regular_detail_actions_cannot_access_node_mgmt_system_task`、`test_disabled_auto_collect_cannot_be_bypassed_through_regular_exec_endpoint`、`test_collect_tool_prefill_rejects_node_mgmt_system_task`、`test_collect_tool_masked_execute_cannot_restore_node_mgmt_system_task_credentials`、`test_config_file_receive_result_cannot_mutate_node_mgmt_system_task`、`test_permission_task_enum_excludes_node_mgmt_system_tasks` | 未在真实 IAM/网关环境验收权限配置 |
...
| `CMDB-F48` | 已解决（代码合同） | `test_fetch_node_mgmt_pages_单页节点数超预算时拒绝结果`、`test_fetch_node_mgmt_pages_单节点字节超预算时拒绝结果`、`test_existing_host_scan_fails_closed_when_host_budget_is_exceeded`、`test_existing_host_scan_fails_closed_when_byte_budget_is_exceeded` | 未在真实图数据库与生产规模数据上执行内存/查询基准 |
| `CMDB-F49` | 已解决（代码合同） | `test_config_is_database_singleton`、`test_only_one_run_can_hold_global_active_scope`、`test_system_code_migration_deduplicates_and_adds_unique_constraint`、`test_region_task_create_unique_race_reloads_and_reuses_winner` | 未在 MySQL/PostgreSQL 与真实多进程下执行并发压力验收 |
...
最终验证证据：节点同步及关联回归集合在 `ENABLE_CELERY=true`、SQLite 内存库和 `--nomigrations` 下为 **448 passed**；两项真实迁移测试为 **2 passed**，`makemigrations --check --dry-run` 为 `No changes detected`。前端状态行为合同 `cmdb-node-mgmt-sync-health-test.ts` 通过，节点同步触及文件定向 ESLint 为 0 错误。核心覆盖率门禁已通过：覆盖集合 **309 passed**，`node_mgmt_sync_reconciler.py` 为 **91%**、`node_mgmt_sync_service.py` 为 **91%**、`node_mgmt_sync.py` View 为 **99%**，合计 **91.44%**，高于 90%。普通采集视图补充执行 **27 passed**，证明节点同步系统任务不能再由普通 retrieve/update/destroy/exec API 按 ID 访问，关闭自动采集后也不能从普通执行入口绕过；采集工具的 prefill/脱敏凭据恢复、配置文件 receive_result 回调、NATS 权限任务枚举与采集统计旁路也已封闭。全仓 `make test` 原命令被缺失 MinIO 环境阻断，补齐 MinIO/SQLite/Celery 后又被当前测试虚拟环境缺少 `mlflow` 阻断，均未进入测试；等价全仓 Web ESLint 仍有 44 个既有错误，CMDB type-check 仍被既有缺失依赖/Enterprise registry/React 类型冲突阻断，均未命中本轮节点同步触及文件。

真实开发环境验收未执行：当前没有可用的 Node Management 外部服务、真实节点数据、Celery broker/多 Worker、FalkorDB 与生产数据库组合。首次打开由 `test_first_open_reconciles_default_enabled_switches_to_beat` 覆盖；打开后关闭、刷新不反弹及再次开启由 `test_disable_then_refresh_keeps_both_schedules_absent`、`test_disable_then_enable_recreates_both_schedules` 覆盖；关闭时节点配置删除、失败重试、旧消息 fencing 与区域退休另有状态机测试。以上只证明数据库/服务行为合同，不能宣称真实环境已经通过。核心覆盖率门禁已经通过；但基于 `CMDB-F45` 仍部分解决、全仓门禁受既有环境/依赖问题阻断以及真实环境验收缺失，本审查 Recommendation 继续保持 **Block**。
...
四个原 P0 已由权限边界、系统身份、真实主机更新和子 execution 聚合代码合同关闭；单活租约、deadline、旧 owner fencing、僵尸恢复、资源预算、系统任务唯一身份及 delete→push 补偿也已有行为/迁移测试，核心覆盖率达到 91.44%。生产批准前仍需：在真实 IAM/Node Management/Celery 多 Worker/FalkorDB/目标数据库组合中完成首次打开、打开后关闭、关闭重启、重开和故障恢复验收；补齐 F45 的组织裁剪与系统作业审计；完成图查询/内存基准及目标数据库并发压力证据；同时恢复全仓 Server/Web 门禁环境并清理既有阻断。在这些证据完成前保持 Block，不能把单元/服务测试等同于真实环境验收。
