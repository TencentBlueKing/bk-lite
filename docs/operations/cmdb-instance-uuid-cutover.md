...
> - 清洗 **不得** 进入 `batch_init`；**不得** 改 `migrate \|\| true` 语义来掩盖失败  
...
- 停止 Web 写流量、Celery worker/beat、NATS CMDB 消费者、Stargazer 配置任务
  及其他 CMDB 写入者；确认旧消息与配置回调已排空或作废。
- 确认待部署的 Server、Worker、OpsPilot、operation_analysis、Stargazer 同版本。  
  **Web 本阶段仍用数字 ID**：若后端已切 UUID 而前端未发，资产详情/关系等会失败——
  要么同发前端（见 FRONTEND_HANDOFF），要么在前端就绪前不要对生产开写流量。
- 图库若存在业务自定义冲突的 `inst_uuid` 语义，停止并人工定责。
- 保存各命令 `--dry-run` / `--apply` / `--verify` 原始输出。非法/重复 UUID、
  未知结构均为阻断项。
...
4. 普通 `migrate` 应用 `0045_instance_uuid_transition`（启动脚本既有顺序；**不要**在 `batch_init` 内跑清洗）。  
5. 在维护编排中执行 §2 清洗与 verify；保存原始输出。  
6. 启动 NATS/Celery/Stargazer 等消费者；按发布节奏启动 Web。  
7. 冒烟：数字 URL/path 安全失败；UUID API/OpenAPI/配置回调成功；Telegraf 标签仍为
   `cmdb_{task_id}`。

禁止：把清洗塞进 `batch_init`；用 `sleep`/吞异常掩盖启动依赖；旧 Worker 与新版本并行写。
