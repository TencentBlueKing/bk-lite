#!/bin/bash
# 目标检测模型训练脚本
# 功能：从 MinIO 下载训练数据集和配置文件，解压并训练YOLO目标检测模型，结果记录到 MLflow
# 
# 用法: ./train-model.sh [BUCKET] [DATASET] [CONFIG]
# 示例: ./train-model.sh datasets object_detection_data.zip configs/train.json
# 
# 参数说明：
#   BUCKET   - MinIO bucket 名称 (默认: datasets)
#   DATASET  - 数据集文件路径 (默认: object_detection_train_data.zip)
#   CONFIG   - 配置文件路径 (可选，默认使用本地 train.json)
# 
# 数据集格式要求：
#   ZIP 包解压后应为 YOLO 格式，且根目录必须包含 data.yaml 文件：
#   dataset/
#     ├── data.yaml           # 必需：YOLO数据集配置文件
#     ├── images/
#     │   ├── train/
#     │   ├── val/
#     │   └── test/
...
if unzip -q -o "${DATASET_FILE}" -d "${EXTRACT_DIR}"; then
    log_info "数据集解压成功"
else
    log_error "数据集解压失败"
    exit 1
...
# 验证 YOLO 数据集格式（简单检查 dataset.yaml 是否在根目录）
log_info "验证数据集格式（YOLO格式）..."
DATASET_ROOT="${EXTRACT_DIR}"
DATASET_YAML="${DATASET_ROOT}/data.yaml"

if [ ! -f "${DATASET_YAML}" ]; then
    log_error "数据集格式错误：根目录未找到 data.yaml 文件"
    log_error "期望路径: ${DATASET_YAML}"
    log_error ""
    log_error "YOLO数据集要求 data.yaml 必须在解压后的根目录"
    log_error "期望结构:"
    log_error "  dataset/"
    log_error "    ├── data.yaml    # 必需"
    log_error "    ├── images/"
    log_error "    └── labels/"
    log_error ""
    log_error "请检查您的数据集压缩包结构是否正确"
    exit 1
fi

log_info "✓ 数据集格式验证通过：找到 data.yaml"
...
# ==================== 训练模型 ====================
log_info "开始训练YOLO目标检测模型..."
log_info "数据集目录: ${DATASET_ROOT}"
log_info "训练配置: ${CONFIG_FILE}"
...
# 构建训练命令（使用注册的 CLI 命令）
TRAIN_CMD="uv run classify_object_detection_server train \
