#!/bin/bash
# 目标检测模型训练脚本
# 功能：从 MinIO 下载训练数据集和配置文件，解压并训练目标检测模型，结果记录到 MLflow
# 
...
#   DATASET_NAME  - 数据集文件名 (默认: object_detection_train_data.zip)
...
TRAIN_CMD="python -m classify_object_detection_server.cli.bootstrap train \
