...
class MLFlowUtils:
    ...
    def setup_experiment(tracking_uri: Optional[str], experiment_name: str) -> str:
        ...
        if experiment is None:
            ...
            logger.info(f"使用现有 MLflow 实验: {experiment_name} (ID: {experiment_id})")
    ...
    @staticmethod
    def log_classification_report(report_text: str, filename: str = "classification_report.txt"):
        """记录分类报告到 MLflow
    ...
    def log_class_distribution(
        *datasets,
        ...
        for i, dataset_name in enumerate(dataset_names):
            ...
            counts = [data[data['Class'] == cls]['Count'].values[0] if cls in data['Class'].values else 0 
                     for cls in classes]
            offset = width * (i - n_datasets / 2 + 0.5)
            ax.bar(x + offset, counts, width, label=dataset_name, color=colors[i % len(colors)])
        
        ax.set_xlabel('类别', fontsize=12)
        ax.set_ylabel('数量', fontsize=12)
        ax.set_title(title, fontsize=14, pad=20)
        ax.set_xticks(x)
        ax.set_xticklabels(classes, rotation=45 if n_classes > 10 else 0, ha='right' if n_classes > 10 else 'center')
        ax.legend()
    ...
    def log_error_analysis(
        ...
        for idx in sorted_indices:
            ...
            error_data.append({
                'text': X_texts[idx][:200] if len(X_texts[idx]) > 200 else X_texts[idx],  # 截断长文本
    ...
    def log_feature_importance(
        ...
        if feature_names:
            top_features = [feature_names[i] if i < len(feature_names) else f"特征_{i}" 
