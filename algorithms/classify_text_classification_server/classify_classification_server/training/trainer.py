...
class UniversalTrainer:
    ...
    def train(self):
        ...
        # 步骤3: 数据预处理
        train_texts_processed, val_texts_processed, test_texts_processed = self._preprocess_data(
        ...
        X_train, X_val, X_test = self._feature_engineering(
            ...
            self.train_df["text"].tolist(), self.val_df["text"].tolist(), self.test_df["text"].tolist()
        )
        ...
        with mlflow.start_run(run_name=self.run_name) as run:
            ...
            train_val_metrics = self._evaluate_model(X_train_val, y_train_val, prefix="train_val")
            # 使用 MLFlowUtils 批量记录（自动过滤内部数据）
            MLFlowUtils.log_metrics_batch(train_val_metrics, prefix="")
            # 只输出数值统计指标到日志
            train_val_summary = {
                k: v for k, v in train_val_metrics.items() 
    ...
    def _preprocess_data(self, train_df, val_df, test_df):
        ...
        # 批量预处理
        train_texts_processed = self.preprocessor.preprocess_batch(train_df["text"].tolist())
        val_texts_processed = self.preprocessor.preprocess_batch(val_df["text"].tolist())
        test_texts_processed = self.preprocessor.preprocess_batch(test_df["text"].tolist())
    ...
    def _feature_engineering(
        ...
        X_train = self.feature_engineer.fit_transform(
            train_texts_processed,
            original_texts=train_texts_original
        )
        
        X_val = self.feature_engineer.transform(
            val_texts_processed,
            original_texts=val_texts_original
        )
        
        X_test = self.feature_engineer.transform(
            test_texts_processed,
            original_texts=test_texts_original
        )
    ...
    def _create_model(self):
        ...
        # 移除非模型参数
        model_params_filtered = {k: v for k, v in model_params.items() if k not in ["type", "name"]}
    ...
    def _log_config(self):
        ...
        # 记录配置参数
        MLFlowUtils.log_params_batch(self.config.get_all())
    ...
    def _log_dataset_statistics(self, y_train, y_val, y_test):
        ...
        for idx, label in enumerate(class_labels):
            ...
            MLFlowUtils.log_metrics_batch({
                ...
                f"class_distribution/{label}/train_ratio": train_count / len(y_train) if len(y_train) > 0 else 0,
                f"class_distribution/{label}/imbalance_ratio": total_count / (len(y_train) + len(y_val) + len(y_test))
            })
        
        logger.info(f"✓ 记录数据集统计: 训练集={len(y_train)}, 验证集={len(y_val)}, 测试集={len(y_test)}")
    ...
    def _optimize_hyperparams(self, X_train, y_train, X_val, y_val):
        ...
        if not hasattr(self.model, 'optimize_hyperparams'):
            logger.warning(f"模型不支持超参数优化（缺少 optimize_hyperparams 方法），跳过")
            return
        ...
        try:
            ...
            MLFlowUtils.log_params_batch({"hyperopt_failed": True, "hyperopt_error": str(e)})
