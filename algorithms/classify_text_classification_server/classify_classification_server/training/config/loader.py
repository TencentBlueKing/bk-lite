...
class TrainingConfig:
    ...
    def _validate_structure(self):
        ...
        missing_keys = [key for key in REQUIRED_TOP_LEVEL_KEYS if key not in self.config]
    ...
    def _validate_required_fields(self):
        ...
        missing_hyperparams_keys = [key for key in REQUIRED_HYPERPARAMS_KEYS if key not in hyperparams_config]
        if missing_hyperparams_keys:
            raise ConfigError(f"hyperparams 配置缺少必需字段: {missing_hyperparams_keys}")
        
        # 验证 preprocessing 部分
        preprocessing_config = self.config.get("preprocessing", {})
        missing_preprocessing_keys = [key for key in REQUIRED_PREPROCESSING_KEYS if key not in preprocessing_config]
        if missing_preprocessing_keys:
            raise ConfigError(f"preprocessing 配置缺少必需字段: {missing_preprocessing_keys}")
        
        # 验证 feature_engineering 部分
        feature_engineering_config = self.config.get("feature_engineering", {})
        missing_fe_keys = [key for key in REQUIRED_FEATURE_ENGINEERING_KEYS if key not in feature_engineering_config]
        if missing_fe_keys:
            raise ConfigError(f"feature_engineering 配置缺少必需字段: {missing_fe_keys}")
        
        # 验证 mlflow 部分
        mlflow_config = self.config.get("mlflow", {})
        missing_mlflow_keys = [key for key in REQUIRED_MLFLOW_KEYS if key not in mlflow_config]
    ...
    def _validate_business_rules(self):
        ...
        if metric not in SUPPORTED_METRICS:
            raise ConfigError(f"不支持的优化指标: '{metric}'. "
        ...
        if jieba_mode not in JIEBA_MODES:
            raise ConfigError(f"不支持的 jieba 分词模式: '{jieba_mode}'. "
                            f"支持的模式: {JIEBA_MODES}")
        ...
        if not use_tfidf and not use_statistical:
            raise ConfigError("必须至少启用一种特征类型（use_tfidf 或 use_statistical）")
        ...
        if use_tfidf:
            ...
            if max_features < 100:
                raise ConfigError(f"tfidf.max_features 建议 >= 100，当前: {max_features}")
    ...
    def _validate_dependencies(self):
        ...
        if not use_fe:
            # 如果禁用特征工程，警告用户可能影响性能
            logger.warning("use_feature_engineering=false 将跳过TF-IDF和统计特征提取，可能影响模型性能")
        ...
        use_statistical = self.config["feature_engineering"]["use_statistical"]
        if use_statistical:
            stat_features = self.config["feature_engineering"].get("statistical_features", [])
        ...
        for key, value in search_space.items():
            if not isinstance(value, list):
                raise ConfigError(f"搜索空间 '{key}' 的值必须是列表，当前类型: {type(value)}")
            if len(value) == 0:
