...
class MLService:
    ...
    async def predict(
        ...
        logger.debug(f"Processed texts type: {type(processed_texts)}, length: {len(processed_texts) if processed_texts else 0}")
        ...
        try:
            ...
            logger.info(f"✅ Classification completed: {summary.total_samples} texts, "
    ...
    def _preprocess_texts(self, texts: list[str]) -> tuple[list[str], list[list[TextWarning]]]:
        """
        预处理文本（截断超长文本）.
        ...
        for text in texts:
            ...
            if len(text) > MAX_TEXT_LENGTH:
                ...
                warnings.append(TextWarning(
                    type="TEXT_TRUNCATED",
                    message=f"文本超过最大长度限制（{MAX_TEXT_LENGTH}字符），已自动截断",
                    original_length=len(text),
                    truncated_length=MAX_TEXT_LENGTH
                ))
                logger.warning(f"Text truncated: {len(text)} -> {MAX_TEXT_LENGTH} chars")
    ...
    def _build_results(
        ...
        for i, (original_text, processed_text) in enumerate(zip(original_texts, processed_texts)):
            # 提取当前样本的预测结果
            ...
            # 提取所有类别概率
            prob_columns = [col for col in model_output.columns if col.startswith("prob_")]
            all_probs = {
                col.replace("prob_", ""): float(row[col])
    ...
    def _get_top_k_predictions(
        ...
        for rank, (label, prob) in enumerate(sorted_probs[:top_k], start=1):
            top_k_results.append(ClassificationLabel(
                label=label,
