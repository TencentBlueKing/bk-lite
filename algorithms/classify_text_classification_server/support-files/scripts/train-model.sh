...
# 示例: ./train-model.sh datasets classification_data.zip configs/train.json
# 
# 参数说明：
#   BUCKET   - MinIO bucket 名称 (默认: datasets)
#   DATASET  - 数据集文件路径 (默认: classification_train_data.zip)
#   CONFIG   - 配置文件路径 (可选，默认使用本地 train.json)
# 
# 数据集格式要求：
#   ZIP 包解压后应包含训练所需的 CSV 或 JSON 格式文件
#   支持 train.csv, test.csv, val.csv 或 data.csv
# 
...
DATASET_NAME="${2:-${DATASET_NAME:-classification_train_data.zip}}"
CONFIG_NAME="$3"
...
# 构建训练命令（使用注册的 CLI 命令）
TRAIN_CMD="uv run classify_classification_server train \
