...
class MLFlowUtils:
    ...
    def flatten_dict(d: dict, parent_key: str = "", sep: str = ".") -> dict:
        ...
        Examples:
            ...
            {
                "hyperparams.search_space.n_estimators": [50, 100, 200],
                "hyperparams.max_evals": 5
            }
        """
        items = []
        for k, v in d.items():
    ...
    def plot_prediction_results(
        ...
        if isinstance(test_data, pd.Series):
            ...
            # 如果训练数据有索引,测试数据接续
            if isinstance(train_data, pd.Series) and isinstance(
    ...
    def plot_residuals_analysis(
        ...
        # 2. 残差分布直方图
        axes[0, 1].hist(
            residuals, bins=30, color="#06A77D", alpha=0.7, edgecolor="black"
        )
        ...
        plot_acf(
            residuals, lags=min(40, len(residuals) // 2), ax=axes[1, 1], alpha=0.05
        )
