#!/bin/bash
# 异常检测模型训练脚本
# 功能：训练异常检测模型，结果记录到 MLflow
# 
# 用法: ./train-model.sh [DATASET_PATH] [CONFIG_PATH]
# 示例: ./train-model.sh ./data/train_data.csv ./train.json
# 
# 参数说明：
#   DATASET_PATH  - 数据集路径 (默认: ./data/train_data.csv)
#   CONFIG_PATH   - 配置文件路径 (默认: ./train.json)
# 
...
# ==================== 参数解析 ====================
DATASET_PATH="${1:-${SCRIPT_DIR}/data/train_data.csv}"
CONFIG_FILE="${2:-${SCRIPT_DIR}/train.json}"
...
# DATASET_NAME="${2:-${DATASET_NAME:-anomaly_train_data.zip}}"
# CONFIG_NAME="$3"
#
# # 检查 mc 二进制文件
# if [ ! -f "${MC_BIN}" ]; then
