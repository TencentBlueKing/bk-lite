#!/bin/bash
# 异常检测模型训练脚本
# 功能：从 MinIO 下载数据集，训练异常检测模型，结果记录到 MLflow
# 
# 用法: ./train-model.sh [BUCKET] [DATASET] [CONFIG]
# 示例: ./train-model.sh munchkin-public datasets/train.zip configs/train.json
# 
# 参数说明：
#   BUCKET   - MinIO bucket 名称 (默认: datasets)
#   DATASET  - 数据集文件路径 (默认: anomaly_train_data.zip)
