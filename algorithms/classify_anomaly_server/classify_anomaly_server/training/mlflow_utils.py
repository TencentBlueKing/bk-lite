...
class MLFlowUtils:
    ...
    def plot_ecod_decomposition(
        ...
        try:
            # 检查模型是否有必要的属性
            if not hasattr(model, 'U_l') or not hasattr(model, 'U_r') or not hasattr(model, 'O'):
            ...
            # 堆叠柱状图
            bar_width = 0.6
            ax1.bar(x_pos, left_tail_scores, bar_width, 
                   label='左尾异常 (值过小)', color='#2E86AB', alpha=0.8)
            ax1.bar(x_pos, right_tail_scores, bar_width, 
                   bottom=left_tail_scores,
                   label='右尾异常 (值过大)', color='#F24236', alpha=0.8)
            
            # 标注总分
            for i, (idx, score) in enumerate(zip(sample_indices, total_scores)):
                ax1.text(i, score + 0.05, f'{score:.2f}', 
            ...
            # 绘制热图
            im = ax2.imshow(feature_contributions, cmap='YlOrRd', aspect='auto', 
