...
class UniversalTrainer:
            ...
            try:
                ...
                if val_data is not None:
                    ...
                    train_metrics = self.model.evaluate(
                        train_data[0], train_data[1], prefix="train"
                    )
    ...
    def _preprocess_data(
        ...
        def clean_and_extract_labels(df):
            """清洗数据并提取标签"""
            cleaned_df, frequency = self.preprocessor.clean(
        ...
        if test_df is not None:
            ...
            test_labels = (
                train_labels[split_point:].copy() if train_labels is not None else None
            )
        ...
        if self.config.use_feature_engineering and fe_config:
            ...
            train_labels = (
                train_labels.loc[train_data.index] if train_labels is not None else None
            )
            logger.info(
            ...
            if val_data is not None:
                ...
                # 对齐验证集标签索引
                val_labels = (
                    val_labels.loc[val_data.index] if val_labels is not None else None
                )
                logger.info(
            ...
            # 对齐测试集标签索引
            test_labels = (
                test_labels.loc[test_data.index] if test_labels is not None else None
            )
            logger.info(
    ...
    def _log_data_info(
        self, train_data, val_data, test_data, train_labels, val_labels, test_labels
    ):
        """记录数据信息到 MLflow"""
        # 收集所有参数到字典
        data_params = {}
    ...
    def _train_model(
        ...
        if val_data is not None and val_data[0] is not None:
            ...
            y_train = (
                pd.concat([train_data[1], val_data[1]], axis=0)
    ...
    def _evaluate_model_on_test(
        self, test_data: Tuple[pd.DataFrame, pd.Series]
    ) -> Dict[str, float]:
        """评估测试集
        ...
        if mlflow.active_run():
            ...
            if self.config.model_type.upper() == "ECOD":
                try:
                    # 获取PyOD ECOD模型实例
                    pyod_model = (
                        self.model.model if hasattr(self.model, "model") else None
                    )
