...
class YOLOClassificationModel(BaseImageClassificationModel):
    ...
    def fit(self, train_data: Tuple[str, List[str]], 
            ...
            **kwargs) -> 'YOLOClassificationModel':
        """
        训练YOLO模型.
        ...
        train_kwargs = {
            ...
            'imgsz': self.hyperparams.get('imgsz', 224),
        ...
        if self.hyperparams.get('augment') is False:
            ...
            for aug_param in ['degrees', 'translate', 'scale', 'fliplr', 'flipud',
                            'hsv_h', 'hsv_s', 'hsv_v']:
        ...
        # 执行训练
        logger.info(f"🚀 开始训练 - epochs={train_kwargs['epochs']}, batch={train_kwargs['batch']}, imgsz={train_kwargs['imgsz']}")
    ...
    def evaluate(self, test_data: Tuple[str, List[str]], 
                prefix: str = "test",
                log_artifacts: bool = False) -> Dict[str, float]:
        """
        评估模型性能.
        ...
        logger.info(f"评估完成: top1_acc={eval_results[f'{prefix}_acc_top1']:.4f}, top5_acc={eval_results[f'{prefix}_acc_top5']:.4f}")
    ...
    def _log_yolo_eval_artifacts_to_mlflow(self, eval_kwargs: Dict[str, Any], prefix: str = "test"):
        """
    ...
    def optimize_hyperparams(self, train_data: Tuple[str, List[str]],
        ...
        def objective(params):
            ...
            try:
                ...
                logger.info(f"  Trial {trial_count[0]} 使用 {trial_epochs} epochs（完整训练为 {self.hyperparams.get('epochs', 100)} epochs）")
                ...
                if hasattr(temp_model, '_results') and temp_model._results is not None:
                    try:
                        ...
                        results_dict = temp_model._results.results_dict if hasattr(temp_model._results, 'results_dict') else {}
                        if 'train/loss' in results_dict:
                            final_loss = results_dict['train/loss']
                        elif hasattr(temp_model._results, 'box') and hasattr(temp_model._results.box, 'loss'):
                ...
                if val_data:
                    ...
                    # 记录结果，包含loss信息
                    loss_info = f", loss={final_loss:.4f}" if final_loss is not None else ""
                    logger.info(f"  Trial {trial_count[0]} 结果: top1={val_acc_top1:.4f}, top5={val_acc_top5:.4f}{loss_info}")
                    
                    if mlflow.active_run():
                        mlflow.log_metric(f"hyperopt/val_acc_top1", val_acc_top1, step=trial_count[0])
                        mlflow.log_metric(f"hyperopt/val_acc_top5", val_acc_top5, step=trial_count[0])
                        if final_loss is not None:
                            mlflow.log_metric(f"hyperopt/final_loss", final_loss, step=trial_count[0])
                        ...
                        if score < best_score[0]:
                            best_score[0] = score
                            mlflow.log_metric("hyperopt/best_acc_top1", -best_score[0], step=trial_count[0])
                    
                    return score
                else:
                    train_metrics = temp_model.evaluate(train_data, prefix="train")
                    score = -train_metrics['train_acc_top1']
                    logger.info(f"  Trial {trial_count[0]} 结果: acc={train_metrics['train_acc_top1']:.4f}")
                    return score
        ...
        if mlflow.active_run():
            success_trials = [t for t in trials.trials if t['result']['status'] == 'ok' and t['result']['loss'] != 1.0]
            failed_trials = [t for t in trials.trials if t['result']['status'] != 'ok' or t['result']['loss'] == 1.0]
            ...
            mlflow.log_metrics({
                ...
                "hyperopt_summary/optimization_duration_min": optimization_duration / 60,
            })
            
            if early_stop_enabled:
                mlflow.log_metrics({
                    "hyperopt_summary/early_stop_enabled": 1.0,
                    "hyperopt_summary/early_stopped": 1.0 if is_early_stopped else 0.0,
        ...
        logger.info(f"转换后的参数类型: {[(k, type(v).__name__) for k, v in converted_params.items()]}")
    ...
    def _decode_hyperopt_params(self, params: Dict[str, Any], 
                               search_space_config: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        for param_name, value in params.items():
            ...
            param_config = search_space_config.get(param_name) or default_space.get(param_name)
            
            # 如果配置是列表格式，且value是索引，转换为实际值
            if isinstance(param_config, list):
                # 支持Python int和numpy整数类型（np.int64, np.int32等）
                if isinstance(value, (int, np.integer)) and 0 <= value < len(param_config):
                    decoded[param_name] = param_config[value]
                else:
                    decoded[param_name] = value
            # 兼容旧的dict格式
            elif isinstance(param_config, dict) and param_config.get('type') == 'choice':
