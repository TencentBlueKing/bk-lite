...
test('packed UI package keeps a registry dependency on core', () => {
  const fixture = fs.mkdtempSync(path.join(os.tmpdir(), 'webchat-ui-pack-'));
  try {
    const pack = spawnSync(
      'npm',
      ['pack', '--workspace', '@webchat/ui', '--pack-destination', fixture, '--json'],
      { cwd: rootDir, encoding: 'utf8', env: { ...process.env, npm_config_cache: path.join(fixture, 'npm-cache') } }
    );
    assert.equal(pack.status, 0, pack.stderr);
    const [{ filename }] = JSON.parse(pack.stdout);
    const packageJsonResult = spawnSync(
      'tar',
      ['-xOf', path.join(fixture, filename), 'package/package.json'],
      { encoding: 'utf8' }
    );
