"""系统管理代理监控模块数据时的组织授权契约。"""

from apps.system_mgmt.viewset.group_data_rule_viewset import GroupDataRuleViewSet


def _get_monitor_app_data(monkeypatch, api_client, authenticated_user, *, group_ids, group_id, is_superuser=False):
    captured = {}
...
def test_monitor_get_app_data_rejects_unauthorized_group(monkeypatch, api_client, authenticated_user):
    response, payload, captured = _get_monitor_app_data(
...
def test_monitor_get_app_data_allows_authorized_group(monkeypatch, api_client, authenticated_user):
    response, payload, captured = _get_monitor_app_data(
    ...
    assert "team" not in captured
    assert "user_info" not in captured
    assert "actor_context" not in captured


def test_monitor_get_app_data_allows_superuser(monkeypatch, api_client, authenticated_user):
    response, payload, captured = _get_monitor_app_data(
        monkeypatch,
        api_client,
        authenticated_user,
...
def test_monitor_get_app_data_rejects_invalid_group_id(monkeypatch, api_client, authenticated_user):
    response, payload, captured = _get_monitor_app_data(
