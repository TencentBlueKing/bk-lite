...
class Controller(TimeInfo, MaintainerInfo):
    ...
    version_command = models.CharField(
        max_length=500, blank=True, default="", verbose_name="获取版本命令"
    )
...
class Node(TimeInfo, MaintainerInfo):
    ...
    operating_system = models.CharField(
        max_length=50, choices=OS_TYPE, verbose_name="操作系统类型"
    )
    collector_configuration_directory = models.CharField(
        max_length=200, verbose_name="采集器配置目录"
    )
    ...
    cloud_region = models.ForeignKey(
        CloudRegion, default=1, on_delete=models.CASCADE, verbose_name="云区域"
    )
    install_method = models.CharField(
        max_length=50, default=ControllerConstants.AUTO, verbose_name="安装方式"
    )
...
class Collector(TimeInfo, MaintainerInfo):
    ...
    service_type = models.CharField(
        max_length=100, choices=ServiceType, verbose_name="服务类型"
    )
    node_operating_system = models.CharField(
        max_length=50, choices=OS_TYPE, verbose_name="节点操作系统类型"
    )
    ...
    validation_parameters = models.CharField(
        blank=True, null=True, max_length=200, verbose_name="验证参数"
    )
    ...
    controller_default_run = models.BooleanField(
        default=False, verbose_name="控制器默认运行"
    )
    default_config = JSONField(default=dict, verbose_name="默认初始化的配置")
    tags = JSONField(default=list, verbose_name="标签")
    package_name = models.CharField(
        max_length=100, blank=True, default="", verbose_name="包名称"
    )
...
class CollectorConfiguration(TimeInfo, MaintainerInfo):
    ...
    nodes = models.ManyToManyField(
        Node, through="NodeCollectorConfiguration", blank=True, verbose_name="节点"
    )
    cloud_region = models.ForeignKey(
        CloudRegion, default=1, on_delete=models.CASCADE, verbose_name="云区域"
    )
...
class NodeCollectorConfiguration(models.Model):
    ...
    collector_config = models.ForeignKey(
        "CollectorConfiguration", on_delete=models.CASCADE
    )
...
class ChildConfig(TimeInfo, MaintainerInfo):
    ...
    collector_config = models.ForeignKey(
        CollectorConfiguration, on_delete=models.CASCADE, verbose_name="采集器配置"
    )
    env_config = JSONField(default=dict, verbose_name="环境变量配置")
    sort_order = models.IntegerField(default=0, verbose_name="排序序号")
    config_section = models.CharField(
        max_length=50, blank=True, default="", verbose_name="配置段"
    )
