...
class ComposeGenerator:
    ...
    # 模板文件路径
    TEMPLATE_DIR = os.path.join(settings.BASE_DIR, 'apps', 'lab', 'support-files', 'compose')
    ...
    def _prepare_ide_service(cls, lab_env, volumes_set: set) -> Dict[str, Any]:
        ...
        if lab_env.ide_image.expose_ports:
            ide_service['ports'] = [f"{port}:{port}" for port in lab_env.ide_image.expose_ports]
        
        # 卷挂载
        volumes = cls._prepare_volume_mounts(
            lab_env.ide_image.volume_mounts,
            ide_service_name,
            volumes_set
        )
        if volumes:
    ...
    def _prepare_infra_services(cls, lab_env, volumes_set: set) -> List[Dict[str, Any]]:
        ...
        for instance in lab_env.infra_instances.all():
            ...
            labels = cls._get_traefik_labels(service_name, instance, lab_env.id, container_name)
            if labels:
    ...
    @classmethod
    def _prepare_volume_mounts(cls, volume_mounts, service_name: str, volumes_set: set) -> List[str]:
        """准备卷挂载列表"""
    ...
    @classmethod
    def _get_traefik_labels_for_ide(cls, lab_env, container_name: str) -> Optional[Dict[str, str]]:
    ...
    @classmethod
    def _get_traefik_labels(cls, service_name: str, instance, env_id: int, container_name: str) -> Optional[Dict[str, str]]:
        ...
        labels = {
            ...
            f'traefik.http.middlewares.{container_name}-strip.stripprefix.prefixes': path_prefix,
            f'traefik.http.routers.{container_name}.middlewares': f'{container_name}-strip',
            f'traefik.http.services.{container_name}.loadbalancer.server.port': str(container_port)
        }
