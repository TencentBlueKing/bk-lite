...
class LabEnvViewSet(ModelViewSet):
    ...
    queryset = (
        LabEnv.objects.select_related("ide_image")
        .prefetch_related("infra_instances")
        .order_by("-created_at")
    )
        ...
        try:
            ...
            if webhook_url:
                ...
                if status_response.status_code == 200:
                    ...
                    if status_result.get("status") == "success" and status_result.get(
                        "data"
                    ):
                        # 创建状态映射
                        status_map = {
                            item["id"]: item for item in status_result["data"]
                        }
    ...
    def start(self, request, pk=None):
        ...
        if lab_env.state == "running":
            return Response(
                {"detail": "Lab 环境已经在运行中"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        try:
            ...
            setup_payload = {"id": compose_id, "compose": compose_config}
            ...
            try:
                ...
                if lab_env.state not in ["stopped", "error"]:
                    logger.warning(
                        f"环境 {lab_env.name} 状态已变为 {lab_env.state},放弃启动"
                    )
                    return Response(
            ...
            if response.status_code == 200:
                ...
                if result.get("status") == "success":
                    ...
                    try:
                        ...
                        if lab_env.state != "starting":
                            ...
                            logger.warning(
                                f"环境 {lab_env.name} 状态已变为 {lab_env.state},虽然启动成功但不标记为running"
                            )
                            return Response(
                                {
                                    "status": "cancelled",
                ...
                logger.error(
                    f"启动请求响应失败: HTTP {response.status_code}, 详情: {error_detail}"
                )
            ...
            logger.error(
                f"Lab 环境 {lab_env.name} 启动超时: webhook 请求超过 {self.WEBHOOK_TIMEOUT_OPERATION} 秒"
            )
            # 超时回滚状态
            try:
                lab_env.refresh_from_db()
            ...
            try:
                ...
                logger.error(f"状态回滚失败: {rollback_err}")
            return Response(
                {"status": "error", "message": "启动请求失败", "error": str(e)},
                status=status.HTTP_502_BAD_GATEWAY,
            )
            ...
            try:
                ...
                logger.error(f"状态回滚失败: {rollback_err}")
            return Response(
                {"status": "error", "message": "启动失败", "error": str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
    ...
    def stop(self, request, pk=None):
        ...
        try:
            ...
            if lab_env.state == "stopped":
                return Response(
                    {"detail": "Lab 环境已经停止"}, status=status.HTTP_400_BAD_REQUEST
                )
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"Webhook 请求失败: HTTP {response.status_code}, 详情: {error_detail}"
                )
            ...
            logger.error(
                f"Lab 环境 {lab_env.name} 停止超时: webhook 请求超过 {self.WEBHOOK_TIMEOUT_OPERATION} 秒"
            )
            lab_env.mark_error()
            return Response(
                {
            ...
            try:
                ...
                logger.error(f"状态更新失败: {rollback_err}")
            return Response(
                {"status": "error", "message": "停止请求失败", "error": str(e)},
                status=status.HTTP_502_BAD_GATEWAY,
            )
            ...
            try:
                ...
                logger.error(f"状态更新失败: {rollback_err}")
            return Response(
                {"status": "error", "message": "停止失败", "error": str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
        ...
        try:
            ...
            if stop_response.status_code == 200:
                ...
                if stop_result.get("status") == "success":
                    ...
                    logger.warning(
                        f"停止环境时出现警告: {stop_result.get('message', '')}"
                    )
            else:
            ...
            if start_response.status_code != 200:
                ...
                try:
                    ...
                    logger.error(f"状态更新失败: {rollback_err}")
                return Response(
                    {"status": "error", "message": "启动失败", "error": error_detail},
                    status=status.HTTP_502_BAD_GATEWAY,
                )
            ...
            if start_result.get("status") == "success":
                ...
                try:
                    ...
                    if lab_env.state != "running":
                        ...
                        logger.info(
                            f"环境 {lab_env.name} 已经是running状态,跳过状态更新"
                        )
                except Exception as e:
            ...
            logger.error(
                f"环境 {lab_env.name} 重启超时: 请求超过 {self.WEBHOOK_TIMEOUT_OPERATION} 秒"
            )
            try:
                lab_env.refresh_from_db()
                if lab_env.state not in ["error", "stopped"]:
            ...
            try:
                ...
                logger.error(f"状态更新失败: {rollback_err}")
            return Response(
                {"status": "error", "message": "重启请求失败", "error": str(e)},
                status=status.HTTP_502_BAD_GATEWAY,
            )
            ...
            try:
                ...
                logger.error(f"状态更新失败: {rollback_err}")
            return Response(
                {"status": "error", "message": "重启失败", "error": str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
    ...
    def status(self, request, pk=None):
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                logger.error(
                    f"获取状态失败: HTTP {response.status_code}, {error_detail}"
                )
                return Response(
                    {
                        "status": "error",
            ...
            return Response(
                {"status": "error", "message": "获取环境状态失败", "error": str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
    ...
    def status_batch(self, request):
        ...
        try:
            ...
            if not env_ids:
                return Response(
                    {"status": "success", "data": [], "message": "没有可用的环境"}
                )
                    ...
                    if env_id_str.startswith("lab-env-"):
                        ...
                        if env and item.get("status") == "success":
                            ...
                            if not containers:
                                ...
                                c.get("State") in ["exited", "dead"] for c in containers
                            ):
                                env.state = "stopped"
                            else:
                                env.state = "error"
            ...
            return Response(
                {"status": "error", "message": "批量获取环境状态失败", "error": str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
    ...
    def setup(self, request, pk=None):
        ...
        try:
            ...
            # 准备请求数据
            payload = {"id": f"lab-env-{lab_env.id}", "compose": compose_config}
            ...
            return Response(
                {"status": "error", "message": "Webhook 请求超时"},
                status=status.HTTP_504_GATEWAY_TIMEOUT,
            )
            ...
            return Response(
                {"status": "error", "message": f"Webhook 请求失败: {str(e)}"},
                status=status.HTTP_502_BAD_GATEWAY,
            )
            ...
            return Response(
                {"status": "error", "message": f"生成配置失败: {str(e)}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
