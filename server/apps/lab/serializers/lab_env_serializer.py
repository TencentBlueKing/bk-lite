...
class LabEnvSerializer(AuthSerializer):
    ...
    ide_image_version = serializers.CharField(
        source="ide_image.version", read_only=True
    )
    ...
    def _create_infra_instances_from_images(self, lab_env, image_ids):
        ...
        for image_id in image_ids:
            try:
                ...
                logger.info(
                    f"自动创建基础设施实例: {instance_name} (镜像: {image.name}:{image.version})"
                )
    ...
    def _update_infra_instances_from_images(self, lab_env, image_ids):
        ...
        if current_image_ids == new_image_ids:
            logger.info(
                f"环境 {lab_env.name} 的镜像列表未变化，检查并同步镜像配置到实例"
            )
            ...
            for instance in current_instances:
                ...
                if (
                    image.volume_mounts
                    and instance.volume_mounts != image.volume_mounts
                ):
                ...
                if updated_fields:
                    instance.save()
                    logger.info(
                        f"实例 {instance.name} 已同步镜像配置，更新字段: {', '.join(updated_fields)}"
                    )
                else:
        ...
        logger.info(
            f"环境 {lab_env.name} 的镜像配置已变化，从 {current_image_ids} 更新为 {new_image_ids}"
        )
    ...
    def _setup_docker_compose(self, lab_env):
        ...
        try:
            ...
            logger.info(
                f"正在为环境 {lab_env.name} (ID: {lab_env.id}) 生成 docker-compose 配置"
            )
