...
def test_incident_im_migrations_extend_the_merged_alerts_leaf():
    first = import_module("apps.alerts.migrations.0022_incident_im_group").Migration
    assert {item for item in first.dependencies if item[0] == "alerts"} == {
        ("alerts", "0022_merge_20260717_0921"),
    }
    assert import_module(
        "apps.alerts.migrations.0023_incidentimgroup_last_reconcile_attempt_at"
    ).Migration.dependencies == [("alerts", "0022_incident_im_group")]
    assert import_module(
        "apps.alerts.migrations.0024_incidentimgroup_actor_fields_length"
    ).Migration.dependencies == [
        ("alerts", "0023_incidentimgroup_last_reconcile_attempt_at"),
    ]
    assert import_module(
        "apps.alerts.migrations.0025_incidentimgroup_delivery_lease"
    ).Migration.dependencies == [
