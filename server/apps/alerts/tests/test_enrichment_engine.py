...
def test_enrich_writes_namespaced_field():
    events = [{"resource_type": "host", "resource_id": "1", "enrichment": {}}]
...
def test_dedup_same_resource_single_provider_call():
    events = [
        {"resource_type": "host", "resource_id": "1", "enrichment": {}},
        {"resource_type": "host", "resource_id": "1", "enrichment": {}},
    ]
    EnrichmentEngine(rules=[_rule()]).enrich_batch(events)
...
def test_missing_binding_field_skipped():
    events = [{"resource_type": "host", "enrichment": {}}]  # 缺 resource_id
    EnrichmentEngine(rules=[_rule()]).enrich_batch(events)
    assert events[0]["enrichment"] == {}
...
def test_different_provider_config_no_cache_collision():
    ...
    events_a = [{"resource_type": "host", "resource_id": "99", "enrichment": {}}]
    events_b = [{"resource_type": "host", "resource_id": "99", "enrichment": {}}]
...
def test_negative_cache_sentinel_returns_empty(settings):
    ...
    events1 = [{"resource_type": "host", "resource_id": "42", "enrichment": {}}]
    events2 = [{"resource_type": "host", "resource_id": "42", "enrichment": {}}]
