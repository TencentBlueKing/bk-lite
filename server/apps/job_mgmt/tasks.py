...
def execute_script_task(self, execution_id: int):
    ...
    """
    logger.info(f"[execute_script_task] 开始执行脚本任务: execution_id={execution_id}")

    try:
        execution = JobExecution.objects.get(id=execution_id)
    except JobExecution.DoesNotExist:
        logger.error(f"[execute_script_task] 执行记录不存在: execution_id={execution_id}")
        return

    # 检查是否已取消
    if execution.status == ExecutionStatus.CANCELLED:
        logger.info(f"[execute_script_task] 任务已取消: execution_id={execution_id}")
        return
    ...
    if not target_details:
        logger.warning(f"[execute_script_task] 无待执行目标: execution_id={execution_id}")
    ...
    with ThreadPoolExecutor(max_workers=min(MAX_WORKERS, len(target_details))) as executor:
        futures = {executor.submit(_execute_script_on_target, td, script_content, script_type, execution.timeout): td for td in target_details}

        for future in as_completed(futures):
            target_detail = futures[future]
            try:
                result = future.result()
                logger.info(f"[execute_script_task] 目标 {target_detail.target.name} 执行完成: success={result['success']}")
            except Exception as e:
                logger.exception(f"[execute_script_task] 目标 {target_detail.target.name} 执行异常: {e}")
    ...
    # 检查是否被取消
    if execution.status == ExecutionStatus.CANCELLED:
        logger.info(f"[execute_script_task] 任务被取消: execution_id={execution_id}")
        return
...
def distribute_files_task(self, execution_id: int):
    ...
    Args:
        execution_id: 作业执行记录ID
    """
    logger.info(f"[distribute_files_task] 开始执行文件分发任务: execution_id={execution_id}")

    try:
        execution = JobExecution.objects.get(id=execution_id)
    except JobExecution.DoesNotExist:
        logger.error(f"[distribute_files_task] 执行记录不存在: execution_id={execution_id}")
        return

    # 检查是否已取消
    if execution.status == ExecutionStatus.CANCELLED:
        logger.info(f"[distribute_files_task] 任务已取消: execution_id={execution_id}")
        return
    ...
    if not target_details:
        logger.warning(f"[distribute_files_task] 无待执行目标: execution_id={execution_id}")
        _update_execution_status(execution, ExecutionStatus.SUCCESS, finished_at=timezone.now())
        return
    ...
    with ThreadPoolExecutor(max_workers=min(MAX_WORKERS, len(target_details))) as executor:
        ...
        for future in as_completed(futures):
            ...
            try:
                result = future.result()
                logger.info(f"[distribute_files_task] 目标 {target_detail.target.name} 分发完成: success={result['success']}")
            except Exception as e:
                logger.exception(f"[distribute_files_task] 目标 {target_detail.target.name} 分发异常: {e}")
    ...
    # 检查是否被取消
    if execution.status == ExecutionStatus.CANCELLED:
        logger.info(f"[distribute_files_task] 任务被取消: execution_id={execution_id}")
        return
...
def execute_playbook_task(self, execution_id: int):
    ...
    6. 清理临时文件
    """
    logger.info(f"[execute_playbook_task] 开始执行Playbook任务: execution_id={execution_id}")

    try:
        execution = JobExecution.objects.get(id=execution_id)
    except JobExecution.DoesNotExist:
        logger.error(f"[execute_playbook_task] 执行记录不存在: execution_id={execution_id}")
        return

    # 检查是否已取消
    if execution.status == ExecutionStatus.CANCELLED:
        logger.info(f"[execute_playbook_task] 任务已取消: execution_id={execution_id}")
        return

    # 获取 Playbook
    playbook = execution.playbook
    if not playbook:
        logger.error(f"[execute_playbook_task] Playbook 不存在: execution_id={execution_id}")
    ...
    if not target_details:
        logger.warning(f"[execute_playbook_task] 无待执行目标: execution_id={execution_id}")
        _update_execution_status(execution, ExecutionStatus.SUCCESS, finished_at=timezone.now())
        return
    ...
    with ThreadPoolExecutor(max_workers=min(MAX_WORKERS, len(target_details))) as executor:
        ...
        for future in as_completed(futures):
            ...
            try:
                result = future.result()
                logger.info(f"[execute_playbook_task] 目标 {target_detail.target.name} 执行完成: success={result['success']}")
            except Exception as e:
                logger.exception(f"[execute_playbook_task] 目标 {target_detail.target.name} 执行异常: {e}")
    ...
    # 检查是否被取消
    if execution.status == ExecutionStatus.CANCELLED:
        logger.info(f"[execute_playbook_task] 任务被取消: execution_id={execution_id}")
        return
    ...
    logger.info(f"[execute_playbook_task] Playbook任务完成: execution_id={execution_id}, status={final_status}")
...
def execute_scheduled_task(self, scheduled_task_id: int):
    ...
    execution_name = f"{scheduled_task.name} - 定时执行 #{scheduled_task.run_count}"
    ...
    if job_type == JobType.SCRIPT:
        ...
        logger.error(f"[execute_scheduled_task] 未知的作业类型: {job_type}")
