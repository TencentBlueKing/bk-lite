...
This module is intentionally free of Django imports and config package imports
so that it can be loaded without triggering Django setup or Celery initialization.
"""
...
def detect_enterprise_footprint(base_dir=None) -> EnterpriseFootprintStatus:
    ...
    base_dir defaults to the parent of this file's utils/ package,
    i.e. the server/ directory (apps/core/utils/enterprise_footprint.py -> server/).
    """
    if base_dir is None:
        # server/apps/core/utils/enterprise_footprint.py -> server/
