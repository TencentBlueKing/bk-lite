...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not apps:
            apps_list = [
                'system_mgmt', 'cmdb', 'monitor', 'node_mgmt',
                'alerts', 'operation_analysis', 'opspilot', 'log'
            ]
        else:
            apps_list = [app.strip() for app in apps.split(',')]

        self.stdout.write(self.style.SUCCESS(
            f'开始批量初始化，目标模块: {", ".join(apps_list)}'))
    ...
    def _init_system_mgmt(self):
        ...
        call_command('create_user', 'admin', 'password',
                     email='admin@bklite.net', is_superuser=True)
