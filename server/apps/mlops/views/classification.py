...
class ClassificationServingViewSet(ModelViewSet):
        ...
        try:
            ...
            for serving_data in servings:
                ...
                if container_info:
                    ...
                    # 同步到数据库
                    serving_obj = ClassificationServing.objects.get(id=serving_data['id'])
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        try:
            ...
            if container_info:
                ...
                # 更新数据库
                ClassificationServing.objects.filter(id=response.data['id']).update(
    ...
    @HasPermission("classification_servings-View")
    def predirect(self, request, *args, **kwargs):
        """
        调用 serving 服务进行分类预测
        """
        try:
