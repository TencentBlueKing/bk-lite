...
class LogClusteringTrainJobViewSet(ModelViewSet):
    ...
    def train(self, request, pk=None):
        ...
        try:
            ...
            # 构建训练任务标识
            job_id = f"LogClustering_{train_job.algorithm}_{train_job.id}"
    ...
    def get_run_data_list(self, request, pk=None):
        ...
        try:
            ...
            # 构造实验名称（与训练时保持一致）
            experiment_name = f"LogClustering_{train_job.algorithm}_{train_job.id}"
    ...
    def get_model_versions(self, request, pk=None):
        ...
        try:
            train_job = self.get_object()
            model_name = f"LogClustering_{train_job.algorithm}_{train_job.id}"
    ...
    def download_model(self, request, run_id: str):
        ...
        try:
            ...
            # 尝试从 Experiment name 提取 algorithm (格式: LogClustering_{algorithm}_{id})
            experiment_id = run.info.experiment_id
            experiment = mlflow_client.get_experiment(experiment_id)
            experiment_name = experiment.name if experiment else ""
            
            # 从 experiment_name 解析: LogClustering_{algorithm}_{id}
            algorithm = 'unknown'
            if experiment_name.startswith('LogClustering_'):
            ...
            if model_name and model_version:
                filename = f"{model_name}_v{model_version}_{run_id[:8]}.zip"
            else:
                filename = f"log_clustering_{algorithm}_{run_id[:8]}.zip"
            
            # 5. 下载模型到临时目录
            temp_dir = tempfile.mkdtemp()
            try:
                model_path = mlflow_client.download_artifacts(run_id, "model", temp_dir)
                
                # 6. 打包为 ZIP
                zip_buffer = BytesIO()
                shutil.make_archive(
                    base_name=os.path.join(temp_dir, 'model'),
                    format='zip',
                    root_dir=model_path
                )
                ...
                logger.info(f"模型下载成功: {filename} (run_id: {run_id})")
                return response
...
class LogClusteringDatasetReleaseViewSet(ModelViewSet):
    queryset = LogClusteringDatasetRelease.objects.all()
        ...
        try:
            ...
            for serving_data in servings:
                ...
                if container_info:
                    ...
                    # 同步到数据库
                    serving_obj = LogClusteringServing.objects.get(id=serving_data['id'])
