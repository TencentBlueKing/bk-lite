...
def publish_dataset_release_async(release_id, train_file_id, val_file_id, test_file_id):
    ...
    try:
        ...
        with transaction.atomic():
            release = LogClusteringDatasetRelease.objects.select_for_update().get(
                id=release_id
            )
        ...
        logger.info(
            f"开始发布日志聚类数据集 - Dataset: {dataset.id}, Version: {version}, Release ID: {release_id}"
        )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            files_info = [
                (train_obj.train_data, "train_data.txt"),
                (val_obj.train_data, "val_data.txt"),
                (test_obj.train_data, "test_data.txt"),
            ]
            ...
            for file_field, filename in files_info:
                if file_field and file_field.name:
                    ...
                    # 统计样本数（txt文件行数）
                    line_count = file_content.decode("utf-8").strip().count("\n") + 1
                    sample_count = max(0, line_count) if file_content.strip() else 0
            ...
            # 创建ZIP压缩包
            zip_filename = f"log_clustering_dataset_{dataset.name}_{version}.zip"
            ...
            with open(zip_path, "rb") as f:
                ...
                zip_object_path = (
                    f"log_clustering_datasets/{dataset.id}/{date_prefixed_path}"
                )
            ...
            logger.info(
                f"日志聚类数据集发布成功 - Release ID: {release.id}, 样本数: {train_samples}/{val_samples}/{test_samples}"
            )
