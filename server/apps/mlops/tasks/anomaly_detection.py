...
def publish_dataset_release_async(release_id, train_file_id, val_file_id, test_file_id):
    ...
    try:
        ...
        logger.info(f"开始发布数据集 - Dataset: {dataset.id}, Version: {version}, Release ID: {release_id}")
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for data_obj, filename, data_type in [
                ...
                if data_obj.train_data and data_obj.train_data.name:
                    ...
                    if data_obj.metadata:
                        ...
                        anomaly_count = len(anomaly_point) if isinstance(anomaly_point, list) else 0
                    ...
                    logger.info(f"下载文件成功: {filename}, 大小: {len(file_content)} bytes, 样本数: {sample_count}, 异常数: {anomaly_count}")
            ...
            with open(zip_path, 'rb') as f:
                ...
                zip_object_path = f'anomaly_detection_datasets/{dataset.id}/{date_prefixed_path}'
            ...
            logger.info(f"数据集发布成功 - Release ID: {release_id}, Version: {version}")
