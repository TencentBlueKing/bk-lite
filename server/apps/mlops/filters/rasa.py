...
class RasaPipelineFilter(django_filters.FilterSet):
    ...
    name = django_filters.CharFilter(
        lookup_expr="icontains", help_text="管道名称（支持模糊搜索）"
    )
    name_exact = django_filters.CharFilter(
        field_name="name", lookup_expr="exact", help_text="管道名称（精确匹配）"
    )
    description = django_filters.CharFilter(
        lookup_expr="icontains", help_text="描述（支持模糊搜索）"
    )

    # 关联数据集过滤
    dataset = django_filters.NumberFilter(
        field_name="datasets__id", help_text="关联的数据集ID"
    )
    dataset_name = django_filters.CharFilter(
        field_name="datasets__name",
        lookup_expr="icontains",
        help_text="关联的数据集名称（模糊搜索）",
    )
    datasets = django_filters.CharFilter(
        method="filter_datasets", help_text="多个数据集ID，用逗号分隔"
    )
    ...
    created_by = django_filters.CharFilter(
        lookup_expr="icontains", help_text="创建者（模糊搜索）"
    )
    updated_by = django_filters.CharFilter(
        lookup_expr="icontains", help_text="更新者（模糊搜索）"
    )
    domain = django_filters.CharFilter(lookup_expr="exact", help_text="域名")

    # 自定义过滤方法
    has_datasets = django_filters.BooleanFilter(
        method="filter_has_datasets", help_text="是否关联了数据集"
    )
    dataset_count = django_filters.NumberFilter(
        method="filter_dataset_count", help_text="关联数据集数量"
    )
    dataset_count_gte = django_filters.NumberFilter(
        method="filter_dataset_count_gte", help_text="关联数据集数量大于等于"
    )
    dataset_count_lte = django_filters.NumberFilter(
        method="filter_dataset_count_lte", help_text="关联数据集数量小于等于"
    )

    # 配置相关过滤
    has_config = django_filters.BooleanFilter(
        method="filter_has_config", help_text="是否有配置"
    )
    config_contains = django_filters.CharFilter(
        method="filter_config_contains", help_text="配置中包含的文本"
    )
    ...
    def filter_datasets(self, queryset, name, value):
        ...
        try:
            dataset_ids = [
                int(dataset_id.strip())
                for dataset_id in value.split(",")
                if dataset_id.strip()
            ]
            if dataset_ids:
    ...
    def filter_config_contains(self, queryset, name, value):
        ...
        # 使用 Django JSONField 的 icontains 查询，安全且高效
        return queryset.filter(config__icontains=value)
