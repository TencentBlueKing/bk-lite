...
class ImageClassificationTrainJobFilter(FilterSet):
    ...
    dataset_version = NumberFilter(field_name="dataset_version__id", label="数据集版本ID")
    dataset__name = CharFilter(field_name="dataset_version__dataset__name", lookup_expr="icontains", label="数据集名称")
    created_by = CharFilter(field_name="created_by", lookup_expr="icontains", label="创建者")
    created_at = DateFromToRangeFilter(field_name="created_at", label="创建时间范围")
...
class ImageClassificationServingFilter(FilterSet):
    ...
    train_job__name = CharFilter(field_name="train_job__name", lookup_expr="icontains", label="训练任务名称")
