...
class LogClusteringTrainData(MaintainerInfo, TimeInfo):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.pk:
            try:
                old_instance = LogClusteringTrainData.objects.get(pk=self.pk)
                ...
                if old_path and old_path != new_path:
                    try:
                        ...
                        logger.info(
                            f"Deleted old train_data file for LogClusteringTrainData {self.pk}: "
                            f"old={old_path}, new={new_path or 'None'}"
                        )
                    except Exception as delete_err:
                        logger.warning(f"Failed to delete old file '{old_path}': {delete_err}")
                        
            except LogClusteringTrainData.DoesNotExist:
                pass
...
class LogClusteringDatasetRelease(MaintainerInfo, TimeInfo):
    """日志聚类数据集发布版本"""
...
class LogClusteringTrainJob(MaintainerInfo, TimeInfo):
    ...
    def _build_complete_config(self):
        ...
        # 生成模型标识：LogClustering_algorithm_id（此时 pk 已存在）
        model_identifier = f"LogClustering_{self.algorithm}_{self.pk}"
