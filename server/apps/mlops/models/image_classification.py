...
class ImageClassificationTrainData(MaintainerInfo, TimeInfo):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.pk:
            try:
                old_instance = ImageClassificationTrainData.objects.get(pk=self.pk)
...
class ImageClassificationTrainJob(MaintainerInfo, TimeInfo):
    """图片分类训练任务"""
    ...
    def _build_complete_config(self):
        ...
        # 生成模型标识：ImageClassification_YOLOv11n_123（此时 pk 已存在）
        model_identifier = f"ImageClassification_{self.algorithm}_{self.pk}"
        ...
        config['model'] = {
            'type': self.algorithm,
            'name': model_identifier,                # 类别名称（训练时动态确定）
        }
