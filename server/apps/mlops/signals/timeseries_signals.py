...
from apps.mlops.models.timeseries_predict import (
    ...
    TimeSeriesPredictTrainHistory,
)


@receiver(post_delete, sender=TimeSeriesPredictDatasetRelease)
...
@receiver(post_delete, sender=TimeSeriesPredictTrainData)
...
@receiver(post_delete, sender=TimeSeriesPredictTrainJob)
...
@receiver(post_delete, sender=TimeSeriesPredictTrainHistory)
def cleanup_train_history_files(sender, instance, **kwargs):
    """
    清理训练历史的 MinIO 配置文件

    在 TimeSeriesPredictTrainHistory 删除后触发，删除 config_url 文件
    """
    logger.info(
        f"[Signal] post_delete 触发: TimeSeriesPredictTrainHistory, "
        f"train_history_id={instance.id}, "
        f"has_config_url={bool(instance.config_url)}"
    )
    ...
    try:
        if instance.config_url:
            instance.config_url.delete(save=False)
            logger.info(
                f"成功删除训练历史配置文件: {instance.config_url.name}, "
                f"train_history_id={instance.id}"
            )
        else:
            logger.debug(
