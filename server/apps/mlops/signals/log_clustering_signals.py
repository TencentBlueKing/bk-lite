...
@receiver(
    post_delete,
    sender=LogClusteringDatasetRelease,
    dispatch_uid="cleanup_lc_dataset_release",
)
def cleanup_dataset_release_files(sender, instance, **kwargs):
    """
    清理数据集发布版本的 MinIO 文件

    在 LogClusteringDatasetRelease 删除后触发,删除 MinIO 中的压缩包文件
    """
    logger.info(
        f"[Signal] post_delete 触发: LogClusteringDatasetRelease, "
...
@receiver(
    post_delete, sender=LogClusteringTrainData, dispatch_uid="cleanup_lc_train_data"
)
def cleanup_train_data_files(sender, instance, **kwargs):
    """
    清理训练数据的 MinIO 文件

    在 LogClusteringTrainData 删除后触发,删除训练数据文件和元数据文件
    """
    logger.info(
        f"[Signal] post_delete 触发: LogClusteringTrainData, "
...
@receiver(
    post_delete, sender=LogClusteringTrainJob, dispatch_uid="cleanup_lc_train_job"
)
def cleanup_train_job_files(sender, instance, **kwargs):
    """
    清理训练任务的 MinIO 配置文件

    在 LogClusteringTrainJob 删除后触发,删除 config_url 文件
    """
    logger.info(
        f"[Signal] post_delete 触发: LogClusteringTrainJob, "
...
@receiver(
    post_delete,
    sender=LogClusteringTrainJob,
    dispatch_uid="cleanup_lc_mlflow_experiment",
)
def cleanup_mlflow_experiment(sender, instance, **kwargs):
    """
    清理 MLflow 实验和模型

    在 LogClusteringTrainJob 删除后触发，删除关联的 MLflow 实验、运行和注册模型
    """
    logger.info(
        f"[Signal] post_delete 触发: LogClusteringTrainJob (MLflow清理), "
...
@receiver(
    post_delete,
    sender=LogClusteringServing,
    dispatch_uid="cleanup_lc_docker_container",
)
def cleanup_docker_container(sender, instance, **kwargs):
    """
    清理 Serving 服务的 Docker 容器

    在 LogClusteringServing 删除后触发，停止并删除关联的 Docker 容器
    """
    logger.info(
        f"[Signal] post_delete 触发: LogClusteringServing (容器清理), "
    ...
    def delete_container():
        try:
            ...
            if "not found" in str(e).lower() or "does not exist" in str(e).lower():
                logger.warning(
                    f"容器已不存在，跳过删除: container_id=LogClustering_Serving_{instance.id}, "
