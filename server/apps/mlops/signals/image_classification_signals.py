...
@receiver(
    post_delete,
    sender=ImageClassificationTrainJob,
    dispatch_uid="cleanup_ic_mlflow_experiment",
)
def cleanup_mlflow_experiment(sender, instance, **kwargs):
    """
    清理 MLflow 实验和模型

    在 ImageClassificationTrainJob 删除后触发，删除关联的 MLflow 实验、运行和注册模型
    """
    logger.info(
        f"[Signal] post_delete 触发: ImageClassificationTrainJob (MLflow清理), "
...
@receiver(
    post_delete,
    sender=ImageClassificationServing,
    dispatch_uid="cleanup_ic_docker_container",
)
def cleanup_docker_container(sender, instance, **kwargs):
    """
    清理 Serving 服务的 Docker 容器

    在 ImageClassificationServing 删除后触发，停止并删除关联的 Docker 容器
    """
    logger.info(
        f"[Signal] post_delete 触发: ImageClassificationServing (容器清理), "
    ...
    def delete_container():
        try:
            ...
            if "not found" in str(e).lower() or "does not exist" in str(e).lower():
                logger.warning(
                    f"容器已不存在，跳过删除: container_id=ImageClassification_Serving_{instance.id}, "
