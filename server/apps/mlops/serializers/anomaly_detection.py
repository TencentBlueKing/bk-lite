...
class AnomalyDetectionDatasetReleaseSerializer(AuthSerializer):
    ...
    def validate(self, attrs):
        ...
        if dataset and version:
            existing_releases = AnomalyDetectionDatasetRelease.objects.filter(dataset=dataset, version=version)
            if self.instance:
    ...
    def _create_from_files(self, validated_data, train_file_id, val_file_id, test_file_id):
        ...
        try:
            ...
            existing = AnomalyDetectionDatasetRelease.objects.filter(dataset=dataset, version=version).first()
