...
## 前置要求
### 1. 账号权限
*   创建专用账号（如cmdb_collector），授予最小权限：SELECT（系统库）、SHOW系列、PROCESS、SHOW STATUS，及mysql.user/mysql.db的查询权限。
*   限制访问 IP（如@'192.168.1.%'）。
### 2. 凭据准备
...
*   数据库：max_connections预留 5 个连接，skip_networking=OFF。
## 采集内容
### 实例基础信息
| Key 名称                     | 含义                          |
| :------------------------- | :-------------------------- |
...
| mysql.instance.start_time | 启动时间（YYYY-MM-DD HH:MM:SS） |
''',
    "zookeeper": '''## 说明
基于 **TCP 协议**（TCP 传输，支持 SSL 加密），自动化采集 MySQL 实例数据并同步至 CMDB，支撑资源盘点、权限审计与故障排查。
## 前置要求
