...
class DisplayFieldHandler:
    ...
    def build_display_fields(
        cls, model_id: str, instance_data: dict, attrs: List[dict]
    ) -> dict:
        """
        ...
        for attr in attrs:
            ...
            if attr_id not in instance_data:
                logger.debug(
                    f"[DisplayFieldHandler] 字段 {attr_id} 在实例中不存在，跳过"
                )
                continue
    ...
    def remove_display_fields(cls, instance_data: dict) -> dict:
        ...
        """
        display_fields = [
            key for key in instance_data.keys() if key.endswith(DISPLAY_SUFFIX)
        ]
        ...
        logger.debug(
            f"[DisplayFieldHandler] 移除了 {len(display_fields)} 个 _display 字段"
        )
    ...
    def get_all_exclude_fields(cls) -> List[str]:
        ...
        try:
            ...
            logger.error(
                f"[DisplayFieldHandler] 获取所有排除字段失败: {e}", exc_info=True
            )
            return []
