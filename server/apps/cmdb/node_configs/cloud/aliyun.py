...
class AliyunNodeParams(BaseNodeParams):
    ...
    def set_credential(self, *args, **kwargs):
        _access_key = f"PASSWORD_access_id_{self._instance_id}"
        _access_secret = f"PASSWORD_access_key_{self._instance_id}"
        regions_id = self.credential["regions"]["resource_id"]
        credential_data = {
            "access_id": "${" + _access_key + "}",
            "access_key": "${" + _access_secret + "}",
    ...
    def env_config(self, *args, **kwargs):
        env_config = {
            f"PASSWORD_access_id_{self._instance_id}": self.credential.get("accessKey", ""),
            f"PASSWORD_access_key_{self._instance_id}": self.credential.get("accessSecret", ""),
        }
        return env_config
