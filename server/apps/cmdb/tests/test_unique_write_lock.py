...
def test_same_unique_signature_has_single_owner_and_stale_takeover():
    ...
    CmdbUniqueWriteLock.objects.filter(lock_key="host:r1:abc").update(lease_expires_at=now() - timedelta(seconds=1))
...
def test_lock_keys_are_stable_and_ignore_empty_unique_values():
    ...
    keys = UniqueWriteLockService.build_lock_keys("host", {"serial": "S-1", "region": "cn", "name": "api"}, check_attr_map)
    assert len(keys) == 2
    assert keys == UniqueWriteLockService.build_lock_keys("host", {"name": "api", "region": "cn", "serial": "S-1"}, check_attr_map)
    assert UniqueWriteLockService.build_lock_keys("host", {"serial": "", "region": "cn", "name": ""}, check_attr_map) == []


def test_lock_keys_treat_cloud_int_and_numeric_string_as_same():
    check_attr_map = {
        "is_only": {"cloud": "云区域"},
        "unique_rules": [type("Rule", (), {"rule_id": "r1", "field_ids": ["ip_addr", "cloud"]})()],
    }
    assert UniqueWriteLockService.build_lock_keys(
        "host", {"ip_addr": "10.0.0.1", "cloud": 1}, check_attr_map
    ) == UniqueWriteLockService.build_lock_keys("host", {"ip_addr": "10.0.0.1", "cloud": "1"}, check_attr_map)
