### 说明
基于脚本方式采集 Spark 实例的版本与关键配置信息（监听端口、安装路径、Web UI 端口、Java 环境、日志路径等），标准化同步至 CMDB。采集为只读，不修改目标任何配置。
...
2. **采集账号与权限（仅 SSH 远程时需要）**
   - 需要能够执行 `spark-shell`，并具备读取日志目录的权限。
3. **目标依赖**
   - 目标主机已安装 Java 运行环境。
   - 目标主机存在运行中的 Spark 进程。
...
| bk_inst_name | 实例展示名 |
| ip_addr | 主机内网 IP |
| port | Master 监听端口（`--port`） |
| install_path | 安装路径 |
| version | Spark 版本（`spark-shell --version`） |
| webui_port | Web UI 端口（`--webui-port`） |
| java_path | Java 路径 |
| java_version | Java 版本 |
| log_path | 日志路径（`-Dspark.log.dir`，或 `install_path/logs`） |

> 补充说明：`log_path` 优先取 `-Dspark.log.dir` 指定的目录，未指定时回退到 `install_path/logs`；`port`、`webui_port` 从进程启动参数解析，未显式配置时可能为空。
