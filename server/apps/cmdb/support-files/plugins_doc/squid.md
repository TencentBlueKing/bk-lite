### 说明
基于脚本方式采集 Squid 实例的版本与关键配置信息（监听端口、安装路径、配置文件、缓存目录、日志路径等），标准化同步至 CMDB。采集为只读，不修改目标任何配置。
...
2. **采集账号与权限（仅 SSH 远程时需要）**
   - 需要具备读取 Squid 配置文件（`squid.conf`）的权限。
3. **目标依赖**
   - 目标主机存在运行中的 Squid 进程。
   - 可执行 `squid -v` 获取版本信息。
...
| bk_inst_name | 实例展示名 |
| ip_addr | 主机内网 IP |
| port | 监听端口（`http_port`，默认 `3128`） |
| version | Squid 版本（`squid -v`） |
| install_path | 安装路径 |
| config_file_path | 配置文件路径（`-f` 指定，或 `/etc/squid/squid.conf`） |
| cache_dir | 缓存目录 |
| access_log | 访问日志路径 |
| error_log | 错误日志路径（`cache_log`） |
| visible_hostname | 对外可见主机名 |

> 补充说明：`config_file_path` 优先取启动命令 `-f` 指定的配置文件，未指定时回退到 `/etc/squid/squid.conf`；`cache_dir`、`access_log`、`error_log`、`visible_hostname` 在配置文件未配置对应项时可能为空。
