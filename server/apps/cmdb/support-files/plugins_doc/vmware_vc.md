...
1. 点击“新增任务”。
2. 选择 vCenter：从下拉列表选中目标 vCenter。
3. 基本配置：任务名称、周期、超时时间、组织、接入点。
4. 凭据：用户名、密码、端口（默认 443）。
5. SSL 验证：
	 - 建议默认开启；
	 - 若 vCenter 使用自签证书且接入点未安装信任链，可先关闭以完成连通验证，后续再补齐证书信任并开启。
6. 保存后，回到任务列表点击“同步”（立即执行）验证。

---

## 验收与常见问题
1. 执行状态为“成功”，且资产数据中出现/更新 ESXi、VM、Datastore 等对象。
2. 若部分对象为空：优先检查账号权限是否为只读且对对象范围授予并继承。
3. 若 SSL 相关报错：按“SSL 验证”建议处理。
