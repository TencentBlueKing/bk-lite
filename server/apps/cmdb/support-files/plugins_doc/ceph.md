### 说明
基于脚本方式采集 Ceph 节点的版本、角色与配置信息（版本、角色、安装路径、配置文件、启动命令行等），标准化同步至 CMDB。采集为只读，不修改目标任何配置。
...
2. **采集账号与权限（仅 SSH 远程时需要）**
   - 需要能够执行 `ceph` 命令，并具备读取 `/etc/ceph/ceph.conf` 的权限。
3. **目标依赖**
   - 目标主机存在运行中的 Ceph 进程（ceph-mon / ceph-osd / ceph-mds / ceph-mgr 之一）。
   - 可执行 `ceph version` 获取版本信息。
...
| ip_addr | 主机内网 IP |
| port | 监听端口 |
| version | Ceph 版本（`ceph version`） |
| role | 节点角色（ceph-mon / ceph-osd / ceph-mds / ceph-mgr） |
| install_path | 安装路径 |
| config_file | 配置文件路径（`--conf` 指定，或 `/etc/ceph/ceph.conf`） |
| cmdline | 进程启动命令行 |
| ceph_exe | ceph 可执行文件路径 |

> 补充说明：`config_file` 优先取启动命令 `--conf` 指定的配置文件，未指定时回退到 `/etc/ceph/ceph.conf`；`role` 依据实际运行的 Ceph 进程类型识别。
