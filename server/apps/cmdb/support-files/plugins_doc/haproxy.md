### 说明
基于脚本方式采集 HAProxy 实例的版本与关键配置信息（监听端口、安装路径、配置文件，以及 global/defaults 段的核心参数），标准化同步至 CMDB。采集为只读，不修改目标任何配置。
...
2. **采集账号与权限（仅 SSH 远程时需要）**
   - 需要具备读取 HAProxy 配置文件（`haproxy.cfg`）的权限。
3. **目标依赖**
   - 目标主机存在以 `-f` 指定配置文件方式启动的 HAProxy 进程。
   - 可执行 `haproxy -v` 获取版本信息。

### 操作步骤
#### 步骤 0：操作入口
1. 进入“CMDB → 管理 → 自动发现 → 采集 → 专业采集”。
2. 选择插件 **HAProxy**，点击“新增任务”。
...
| ip_addr | 主机内网 IP |
| port | 监听端口 |
| version | HAProxy 版本（`haproxy -v`） |
| install_path | 安装路径 |
| conf_file | 配置文件路径（`-f` 指定） |
| global_maxconn | global 段最大连接数 |
| global_pidfile | global 段 pidfile 路径 |
| global_user_name | global 段运行用户 |
| global_group_name | global 段运行用户组 |
| defaults_maxconn | defaults 段最大连接数 |
| defaults_mode | defaults 段工作模式 |
| defaults_retries | defaults 段重试次数 |

> 补充说明：global/defaults 段的各参数均从 `haproxy.cfg` 中解析，配置文件未显式配置对应项时相应字段可能为空。
