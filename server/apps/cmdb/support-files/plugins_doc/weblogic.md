### 说明
基于脚本（JOB）采集 WebLogic 实例的版本、监听端口、安装/域目录、Server 名称与管理 Server 等信息，并标准化同步至 CMDB。采集为只读，不修改目标任何配置。
...
### 版本兼容性
- 适用于以脚本方式可识别的 WebLogic 实例；不同大版本的目录结构与配置写法可能有差异，请以实际试采结果为准。
...
2. **采集账号与权限（仅 SSH 远程时需要）**
   - 需能读取 domain 下的 `config/config.xml` 配置文件。
3. **目标依赖**
   - WebLogic 已部署并启动。
   - 目标安装了 Java 运行环境。
...
| inst_name | 实例展示名 |
| ip_addr | 主机内网 IP |
| port | 监听端口（`config.xml` listen-port，默认 `7001`） |
| wlst_path | WLST 工具路径 |
| weblogic_home | WebLogic 安装目录 |
| domain_path | 域目录路径 |
| name | Server 名称 |
| domain_version | 域版本 |
| admin_server_name | 管理 Server 名称 |
| java_version | JDK 版本 |

> 补充说明：当 domain 的 `config/config.xml` 路径非常规或无权读取时，相关字段可能为空。本插件为 **BETA / 未测试**，请核对结果后再投入使用。
