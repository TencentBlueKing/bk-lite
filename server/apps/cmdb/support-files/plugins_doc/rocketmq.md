### 说明
基于脚本（JOB）采集 RocketMQ Broker 的版本、监听端口、安装路径、集群与 NameServer 信息等，并标准化同步至 CMDB。采集为只读，不修改目标任何配置。
...
### 版本兼容性
- 适用于以脚本方式可识别的 RocketMQ 实例；不同大版本的目录结构与配置写法可能有差异，请以实际试采结果为准。
...
2. **采集账号与权限（仅 SSH 远程时需要）**
   - 需能读取 `broker.conf` 配置文件。
   - 需能执行 `mqadmin`。
3. **目标依赖**
   - RocketMQ 已部署并启动。
   - 目标安装了 Java 运行环境，且具备 `mqadmin`。

### 操作步骤
#### 步骤 0：操作入口
1. 进入“CMDB → 管理 → 自动发现 → 采集 → 专业采集”。
2. 选择插件 **RocketMQ**，点击“新增任务”。
...
| bk_inst_name | 实例展示名 |
| ip_addr | 主机内网 IP |
| port | 监听端口（`broker.conf` listenPort，默认 `10911`） |
| install_path | 安装路径 |
| configfile | 配置文件路径（`broker.conf`） |
| version | RocketMQ 版本 |
| broker_id | Broker ID |
...
> 补充说明：当 `broker.conf` 路径非常规、或 `mqadmin` 不可用时，相关字段可能为空。本插件为 **BETA / 未测试**，请核对结果后再投入使用。
