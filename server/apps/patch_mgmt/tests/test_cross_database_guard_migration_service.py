...
OLD_TARGET = [("patch_mgmt", "0007_patchsource_sync_in_progress")]
NEW_TARGET = [("patch_mgmt", "0008_cross_database_kb_guard")]
...
def test_mysql_duplicate_kb_preflight_can_be_fixed_and_retried():
    ...
    with migrated_from(connection, OLD_TARGET, NEW_TARGET) as old_apps:
        ...
        try:
            ...
            assert not MigrationRecorder(connection).migration_qs.filter(app="patch_mgmt", name="0008_cross_database_kb_guard").exists()
        finally:
            duplicate.delete()
