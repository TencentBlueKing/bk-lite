"""Contract tests for the SonicWall SonicOS firewall SNMP plugin.

Validated against the generic **firewall** baseline (Telegraf/snmp/firewall),
NOT the Cisco switch baseline. SonicWall must be a strict superset of the
firewall baseline (every baseline interface/uptime metric present with
byte-identical metric_group/unit/query) plus SonicWall-specific health and
firewall state from the SONICWALL-FIREWALL-IP-STATISTICS-MIB:

  - device_cpu_usage     (percent, reported directly by the device)
  - device_memory_usage  (percent, sonicCurrentRAMUtil reported directly --
                          a single RAM-utilization scalar, NOT total/free)
  - firewall_current_connections / firewall_max_connections   (short)
  - firewall_nat_translations                                 (short)
  - interface_ifHCInOctets / ifHCOutOctets                    (64-bit HC)

Unlike the bytes-pool firewalls (checkpoint/stormshield), SonicWall reports
memory utilization directly as a percentage, so there are NO
device_memory_total / device_memory_free / device_memory_used metrics. VPN
tunnels (per-SA walk), temperature / fan / PSU are N/A and must not be
modelled. The collectType must wire into the firewall object, not the switch.
...
SW_DIR = PLUGINS / "snmp_sonicwall" / "firewall"
...
PLUGIN_NAME = "Firewall SonicWall SNMP"
...
SW_HEALTH = (
    ...
    "firewall_current_connections", "firewall_max_connections",
...
# RAM% is reported directly, so the bytes-pool memory metrics must NOT exist
ABSENT_MEMORY = ("device_memory_total", "device_memory_free", "device_memory_used")
ABSENT_METRICS = ABSENT_MEMORY + (
    "device_temperature_celsius", "device_fan_state", "device_psu_state",
    "firewall_vpn_tunnels", "firewall_ha_mode",
)
...
def metrics():
    return _read_json(SW_DIR / "metrics.json")
...
@pytest.fixture(scope="module")
def policy():
    return _read_json(SW_DIR / "policy.json")


@pytest.fixture(scope="module")
def ui():
    return _read_json(SW_DIR / "UI.json")


@pytest.fixture(scope="module")
def toml_text():
    return (SW_DIR / "sonicwall.child.toml.j2").read_text(encoding="utf-8")
...
def test_plugin_lives_under_firewall_dir():
    assert SW_DIR.parent.name == COLLECT_TYPE
    assert SW_DIR.name == INSTANCE_TYPE


@pytest.mark.unit
def test_toml_filename_follows_convention():
    assert (SW_DIR / f"{CONFIG_TYPE}.child.toml.j2").exists()
...
def test_is_superset_of_firewall_baseline(metrics, base_metrics):
    sw = {m["name"] for m in metrics["metrics"]}
    base = {m["name"] for m in base_metrics["metrics"]}
    missing = base - sw
    assert missing == set(), f"sonicwall dropped baseline firewall metrics: {missing}"
...
# --------------------------------------------------------------------------- #
# SonicWall-specific health: CPU% + RAM% direct, connection/NAT counters
# --------------------------------------------------------------------------- #
@pytest.mark.unit
def test_sonicwall_health_metrics_present(metrics):
    names = {m["name"] for m in metrics["metrics"]}
    missing = [m for m in SW_HEALTH if m not in names]
    assert missing == [], f"sonicwall health metrics missing: {missing}"


@pytest.mark.unit
def test_cpu_and_memory_usage_are_direct_percent(metrics):
    """Both CPU and RAM are reported directly as percentages -- the memory
    query must be the raw series, not a computed total/free expression."""
    ...
    for name in ("device_cpu_usage", "device_memory_usage"):
        ...
        assert q.startswith(f"{name}{{"), f"{name} must be a direct series, not computed"


@pytest.mark.unit
def test_no_bytes_pool_memory_metrics(metrics):
    """RAM% is direct, so there are no total/free/used bytes-pool metrics."""
    names = {m["name"] for m in metrics["metrics"]}
    present = [m for m in ABSENT_MEMORY if m in names]
    assert present == [], f"RAM%-direct vendor must not have bytes-pool memory: {present}"


@pytest.mark.unit
def test_connection_counters_are_short(metrics):
    by = {m["name"]: m for m in metrics["metrics"]}
    for name in SHORT_COUNTERS:
...
def test_frontend_collecttype_not_wired_to_switch_object():
    ...
    assert COLLECT_TYPE not in text, "sonicwall firewall must not be wired into the switch object"
