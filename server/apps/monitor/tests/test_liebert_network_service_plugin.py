"""Contract tests for the Liebert PDU/UPS/Environmental NetworkService SNMP plugin.

Liebert Global Products MIB facts identify rack PDU, MPX, MPH, RPC2,
power-measurement, power-line, DC-power, threshold, load-circuit, temperature,
humidity and remote-sensor tables. The currently available evidence does not
prove stable shared health scalars for CPU, memory, fan, or power. The plugin
therefore stays conservative: it uses the shared SNMP floor only, keeps brand
metrics as a zero-delta child, and does not promote UPS, PDU or environmental
business tables into device-health metrics.
"""
import json
...
BRAND_DIR = PLUGINS / "snmp" / "network_service_liebert"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "liebert"
COLLECT_TYPE = "snmp_liebert"
CONFIG_TYPE = "liebert"
INSTANCE_TYPE = "network_service"
PLUGIN_NAME = "NetworkService Liebert PDU UPS Environmental SNMP"
OBJECT_NAME = "NetworkService"
PEN_ROOT = "1.3.6.1.4.1.476.1.42"
...
UNSUPPORTED_LIEBERT_METRICS = (
    "liebert_input_voltage",
    "liebert_input_current",
    "liebert_output_voltage",
    "liebert_output_current",
    "liebert_power_watts",
    "liebert_power_factor",
    "liebert_battery_state",
    "liebert_bypass_state",
    "liebert_load_percent",
    "liebert_temperature_celsius",
    "liebert_humidity_percent",
    "liebert_remote_sensor_state",
    "liebert_threshold_state",
)
...
@pytest.mark.unit
def test_liebert_private_health_oids_are_not_guessed(metrics, policy, toml_text):
    names = {m["name"] for m in metrics["metrics"]}
    for absent in UNSUPPORTED_HEALTH_METRICS + UNSUPPORTED_LIEBERT_METRICS:
...
def test_brand_label_present_in_common_without_generic_terms():
    ...
    assert "Liebert" in text
    assert "mm-liebert_liebert" in text
    assert "liebert|vertiv" in text
    assert "liebert|pdu|ups|network.?service" not in text


@pytest.mark.unit
def test_brand_svg_exists():
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-liebert_liebert.svg").exists()
...
def test_brand_files_have_no_external_source_residue():
    ...
    paths = list(BRAND_DIR.glob("*")) + [
        WEB_ROOT / "public" / "assets" / "icons" / "mm-liebert_liebert.svg"
    ]
    offenders = {
