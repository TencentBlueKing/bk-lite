"""Contract tests for the Synology storage SNMP plugin.

Synology NAS (enterprise PEN 6574) exposes chassis temperature, system fan and
power-supply scalar state, and a per-array RAID state table. The private tree
has no pollable system-level CPU/memory gauge, so CPU/memory are N/A.

  - device_temperature_celsius : synologyTemperature (.6574.1.2.0), direct °C, no scaling
  - device_fan_state           : synologySystemFanStatus (.6574.1.4.1.0), enum 1=normal/2=fault
  - device_psu_state           : synologyPowerStatus (.6574.1.3.0), enum 1=normal/2=fault
  - device_raid_state          : synologyRaidStatus (.6574.3.1.1.3), worst array, enum 1=normal

The storage object is shared (hardwareDevice/storage.tsx, instance_type=storage);
a common.tsx match and the storage.tsx collect-type wire are required.
"""
import json
...
BRAND_DIR = PLUGINS / "snmp" / "storage_synology"
...
PLUGIN_NAME = "Storage Synology SNMP"
OBJECT_NAME = "Storage"
PEN_ROOT = "1.3.6.1.4.1.6574"

SUPPORTED_SCALAR_UNITS = {
    "byteps", "bytes", "bitps", "counts", "cps", "percent", "celsius", "s", "short", "none",
}
HC_METRICS = ("interface_ifHCInOctets", "interface_ifHCOutOctets")
STATE_METRICS = ("device_fan_state", "device_psu_state", "device_raid_state")
...
@pytest.mark.unit
def test_uses_private_pen_6574(toml_text):
    assert PEN_ROOT in toml_text, "Synology private health lives under PEN 6574"
...
def test_temperature_is_celsius_no_scaling(metrics, toml_text):
    ...
    # direct °C, no /10 scaling in the synthetic-table OID
    assert "1.3.6.1.4.1.6574.1.2.0" in toml_text
...
def test_enum_processors_normalize_states_to_1_normal(toml_text):
    for measurement in ("device_fan", "device_psu", "device_raid"):
...
@pytest.mark.unit
def test_no_cpu_memory_modelled(metrics):
    names = {m["name"] for m in metrics["metrics"]}
    for absent in ("device_cpu_usage", "device_memory_usage", "device_memory_used"):
        assert absent not in names, \
            f"{absent}: Synology private tree has no pollable gauge -> N/A"
...
def test_storage_dashboard_renders_health_panels():
    ...
    text = storage_tsx.read_text(encoding="utf-8")
    for panel in ("device_temperature_celsius", "device_fan_state",
                  "device_psu_state", "device_raid_state"):
