"""Contract tests for the Avocent ACS console-server SNMP plugin."""
import json
...
BRAND = "avocent"
COLLECT_TYPE = "snmp_avocent"
CONFIG_TYPE = "avocent"
INSTANCE_TYPE = "console_server"
PLUGIN_NAME = "ConsoleServer Avocent ACS SNMP"
OBJECT_NAME = "ConsoleServer"
AVOCENT_ROOT = "1.3.6.1.4.1.10418"
BASE_METRICS = {
    "snmp_uptime",
...
def test_no_unverified_private_health_metrics_are_promoted(metrics, policy, toml_text):
    ...
    assert AVOCENT_ROOT not in toml_text
...
def test_brand_label_present_in_common_without_generic_terms():
    ...
    assert "Avocent ACS" in text
    assert "mm-avocent_avocent" in text
    assert "cyclades" in text.lower()
    avocent_line = next(line for line in text.splitlines() if "mm-avocent_avocent" in line)
    assert "console" not in avocent_line.lower()
    assert "serial" not in avocent_line.lower()
    assert "server" not in avocent_line.lower()


@pytest.mark.unit
def test_brand_svg_exists():
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-avocent_avocent.svg").exists()
...
def test_brand_files_have_no_external_source_residue():
    ...
    paths = list(BRAND_DIR.glob("*")) + [
        WEB_ROOT / "public" / "assets" / "icons" / "mm-avocent_avocent.svg"
    ]
    offenders = {
