"""Contract tests for the CeresData storage SNMP plugin.

CeresData all-flash arrays expose shared storage health under enterprise PEN
25359:

  - device_disk_state: system-disk and physical-disk status, worst disk per instance
  - device_raid_state: storage-pool health, worst pool per instance
...
temperature, fan, and power-supply gauges are intentionally not modeled.
"""
import json
...
BRAND_DIR = PLUGINS / "snmp" / "storage_ceresdata"
...
PLUGIN_NAME = "Storage CeresData SNMP"
OBJECT_NAME = "Storage"
PEN_ROOT = "1.3.6.1.4.1.25359"
DISK_OIDS = (
    "1.3.6.1.4.1.25359.4.1.3",
    "1.3.6.1.4.1.25359.1.2.3",
)
RAID_OID = "1.3.6.1.4.1.25359.1.1.3"
...
def test_no_unverified_health_gauges_modelled(metrics):
    names = {item["name"] for item in metrics["metrics"]}
    for absent in (
        "device_cpu_usage",
        "device_memory_usage",
...
def test_policy_templates_reference_existing_metrics(metrics, policy):
    known = {item["name"] for item in metrics["metrics"]}
...
def test_en_desc_has_no_halfwidth_colon_space(languages):
    entry = (languages["en"].get("monitor_object_plugin") or {}).get(PLUGIN_NAME) or {}
    assert ": " not in entry.get("desc", "")
...
def test_new_files_do_not_leak_external_source_names():
    checked_paths = [
        ...
        Path(__file__),
        WEB_ROOT / "public" / "assets" / "icons" / "mm-ceresdata_ceresdata.svg",
    ]
