"""Contract tests for the Check Point Quantum firewall SNMP plugin.

Validated against the generic **firewall** baseline (Telegraf/snmp/firewall),
NOT the Cisco switch baseline. Check Point must be a strict superset of the
firewall baseline (every baseline interface/uptime metric present with
byte-identical metric_group/unit/query) plus Check Point-specific health and
firewall state from the CHECKPOINT-MIB (enterprise 2620):

  - device_cpu_usage                              (percent)
  - device_memory_total / device_memory_free      (bytes)
  - device_memory_used  = total - free            (bytes, computed inline)
  - device_memory_usage = 100*(total-free)/total  (percent, computed inline)
  - firewall_vpn_encrypted_pps / decrypted_pps    (packets/sec, cps)
  - interface_ifHCInOctets / ifHCOutOctets        (64-bit HC traffic)

The memory utilization is computed inside metrics.json (no shared-dashboard /
config.ts dependency), so there is no per-vendor frontend dashboard change.
Temperature / fan / PSU and active-connection / HA are intentionally N/A
(Firewall object has no env-sensor display slot; HA/connections are walk/LLD,
not single scalars) — so none are modelled and there is no enum processor.
The collectType must wire into the firewall object, not the switch.
...
CP_DIR = PLUGINS / "snmp_checkpoint" / "firewall"
...
PLUGIN_NAME = "Firewall Check Point SNMP"
...
def metrics():
    return _read_json(CP_DIR / "metrics.json")
...
@pytest.fixture(scope="module")
def policy():
    return _read_json(CP_DIR / "policy.json")


@pytest.fixture(scope="module")
def ui():
    return _read_json(CP_DIR / "UI.json")


@pytest.fixture(scope="module")
def toml_text():
    return (CP_DIR / "checkpoint.child.toml.j2").read_text(encoding="utf-8")
...
def test_plugin_lives_under_firewall_dir():
    assert CP_DIR.parent.name == COLLECT_TYPE
    assert CP_DIR.name == INSTANCE_TYPE


@pytest.mark.unit
def test_toml_filename_follows_convention():
    assert (CP_DIR / f"{CONFIG_TYPE}.child.toml.j2").exists()
...
def test_is_superset_of_firewall_baseline(metrics, base_metrics):
    cp = {m["name"] for m in metrics["metrics"]}
    base = {m["name"] for m in base_metrics["metrics"]}
    missing = base - cp
    assert missing == set(), f"checkpoint dropped baseline firewall metrics: {missing}"
...
# --------------------------------------------------------------------------- #
# Check Point-specific health / memory model / VPN pps
# --------------------------------------------------------------------------- #
@pytest.mark.unit
def test_checkpoint_health_metrics_present(metrics):
    names = {m["name"] for m in metrics["metrics"]}
    missing = [m for m in CP_HEALTH if m not in names]
    assert missing == [], f"checkpoint health metrics missing: {missing}"


@pytest.mark.unit
def test_cpu_usage_is_percent(metrics):
...
def test_memory_total_and_free_are_bytes(metrics):
    ...
    for name in ("device_memory_total", "device_memory_free"):
        ...
        assert by[name]["metric_group"] == "Firewall"
...
@pytest.mark.unit
def test_vpn_pps_metrics_are_packet_rate(metrics):
    by = {m["name"]: m for m in metrics["metrics"]}
    for name in ("firewall_vpn_encrypted_pps", "firewall_vpn_decrypted_pps"):
        assert by[name]["unit"] == "cps", f"{name} must use the cps packet-rate unit"
...
# --------------------------------------------------------------------------- #
# N/A sensors and not-modelled-as-capability checks
# --------------------------------------------------------------------------- #
@pytest.mark.unit
...
def test_frontend_collecttype_not_wired_to_switch_object():
    ...
    assert COLLECT_TYPE not in text, "checkpoint firewall must not be wired into the switch object"
