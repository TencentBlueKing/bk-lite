"""Contract tests for the NetModule router SNMP plugin.

NetModule NB/NG-series (IANA PEN 31496, NB-MIB) are fanless industrial
4G/5G LTE routers. The private MIB exposes a single system temperature scalar
(systemTemperature, FloatSyntax d-1 -> raw /10 = Celsius). No CPU/memory/fan/
power objects are reliably available, so those are N/A; the plugin collects
system temperature plus standard IF-MIB (64-bit HC) interface metrics.

OID values are pending on-site SNMP walk; only the PEN root, the /10 scaling
and the N/A contract are asserted here.
"""
import json
...
BRAND_DIR = PLUGINS / "snmp" / "router_netmodule"
...
PLUGIN_NAME = "Router NetModule SNMP"
OBJECT_NAME = "Router"
PEN_ROOT = "1.3.6.1.4.1.31496"
...
@pytest.mark.unit
def test_uses_private_pen_31496(toml_text):
...
def test_temperature_metric_is_celsius_with_tenths_scaling(metrics, toml_text):
    ...
    assert "/10)" in q, "FloatSyntax tenths must be divided by 10"
