"""Contract tests for the Perle IOLAN console-server SNMP plugin."""
import json
...
BRAND_DIR = PLUGINS / "snmp" / "console_server_perle"
...
PLUGIN_NAME = "ConsoleServer Perle IOLAN SNMP"
OBJECT_NAME = "ConsoleServer"
PERLE_ROOT = "1.3.6.1.4.1.1966"
BASE_METRICS = {
    "snmp_uptime",
...
def test_no_unverified_private_health_metrics_are_promoted(metrics, policy, toml_text):
    ...
    assert PERLE_ROOT not in toml_text
...
def test_brand_label_present_in_common_without_generic_terms():
    ...
    assert "Perle IOLAN" in text
    assert "mm-perle_perle" in text
    assert "iolan" in text.lower()
    perle_line = next(line for line in text.splitlines() if "mm-perle_perle" in line)
    assert "console" not in perle_line.lower()
    assert "serial" not in perle_line.lower()
    assert "server" not in perle_line.lower()


@pytest.mark.unit
def test_brand_svg_exists():
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-perle_perle.svg").exists()
...
def test_brand_files_have_no_external_source_residue():
    ...
    paths = list(BRAND_DIR.glob("*")) + [
        WEB_ROOT / "public" / "assets" / "icons" / "mm-perle_perle.svg"
    ]
    offenders = {
