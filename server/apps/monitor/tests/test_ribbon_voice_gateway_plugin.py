"""Contract tests for the Ribbon/Sonus voice gateway SNMP plugin.

Ribbon/Sonus SBC and voice gateway devices are represented conservatively here:
...
BRAND_DIR = PLUGINS / "snmp" / "voice_gateway_ribbon"
...
PLUGIN_NAME = "VoiceGateway Ribbon SNMP"
...
def test_brand_label_present_in_common():
    ...
    assert "mm-ribbon_ribbon" in text


@pytest.mark.unit
def test_brand_svg_exists():
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-ribbon_ribbon.svg").exists()
...
def test_brand_files_have_no_external_source_residue():
    ...
    paths = list(BRAND_DIR.glob("*")) + [
        WEB_ROOT / "public" / "assets" / "icons" / "mm-ribbon_ribbon.svg"
    ]
    offenders = {
