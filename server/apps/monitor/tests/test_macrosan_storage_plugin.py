"""Contract tests for the MacroSAN storage SNMP plugin.

MacroSAN MS-series arrays (enterprise PEN 35904) expose full controller health:

  - device_cpu_usage   : cpuRate (.35904.1.2.5.3.1.0), direct percent
  - device_memory_usage: vmRate  (.35904.1.2.5.3.2.0), direct percent (no used/free)
  ...
  - device_disk_state  : diskHealthState (.35904.1.2.2.5.2.1.6), enum 1=normal/2=fault

Temperature is N/A. The storage object is shared (hardwareDevice/storage.tsx).
"""
import json
...
BRAND_DIR = PLUGINS / "snmp" / "storage_macrosan"
...
PLUGIN_NAME = "Storage MacroSAN SNMP"
OBJECT_NAME = "Storage"
...
def test_uses_private_pen_35904(toml_text):
    assert PEN_ROOT in toml_text, "MacroSAN private health lives under PEN 35904"
...
def test_cpu_and_memory_are_direct_percent(metrics, toml_text):
    ...
    assert mem["unit"] == "percent" and mem["metric_group"] == "Memory"
    # usage-only, no used/free split
    assert "device_memory_used" not in by and "device_memory_free" not in by
    assert "1.3.6.1.4.1.35904.1.2.5.3.1.0" in toml_text  # cpuRate
    assert "1.3.6.1.4.1.35904.1.2.5.3.2.0" in toml_text  # vmRate
...
@pytest.mark.unit
def test_no_temperature_modelled(metrics):
    names = {m["name"] for m in metrics["metrics"]}
    assert "device_temperature_celsius" not in names, \
        "MacroSAN profile exposes no numeric temperature OID -> N/A"
...
def test_storage_dashboard_renders_health_panels():
    ...
    text = storage_tsx.read_text(encoding="utf-8")
    for panel in ("device_cpu_usage", "device_memory_usage", "device_fan_state",
                  "device_psu_state", "device_disk_state"):
