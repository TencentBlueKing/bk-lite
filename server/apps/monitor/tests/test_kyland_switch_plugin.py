"""Contract tests for the Kyland switch SNMP plugin.

Kyland industrial managed Ethernet switches are represented by enterprise PEN
26067. Current production input confirms the public enterprise root, industrial
switch product surface and an official MIB entry page, but not stable shared
CPU, memory, temperature, fan or power leaf OIDs. The plugin therefore remains
conservative: it uses the shared SNMP floor only, keeps metrics.json as a
zero-delta child, and does not promote private switch-management objects into
shared device-health metrics.
"""
import json
...
BRAND_DIR = PLUGINS / "snmp" / "switch_kyland"
...
PLUGIN_NAME = "Switch Kyland SNMP"
OBJECT_NAME = "Switch"
PEN_ROOT = "1.3.6.1.4.1.26067"
...
@pytest.mark.unit
def test_metrics_json_is_zero_vendor_delta_child(metrics):
...
@pytest.mark.unit
def test_frontend_collecttype_wired_once_to_switch_object():
    switch_tsx = (
...
@pytest.mark.unit
def test_brand_label_present_once_in_common():
    common = WEB_ROOT / "src" / "app" / "monitor" / "utils" / "common.tsx"
    text = common.read_text(encoding="utf-8")
    assert "label: 'Kyland'" in text
    assert "mm-kyland_kyland" in text
    assert text.count("label: 'Kyland'") == 1
    assert text.count("mm-kyland_kyland") == 1
    assert "kyland|switch" not in text.lower()


@pytest.mark.unit
def test_brand_svg_exists():
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-kyland_kyland.svg").exists()
...
def test_brand_files_have_no_external_source_residue():
    ...
    paths = list(BRAND_DIR.glob("*")) + [
        WEB_ROOT / "public" / "assets" / "icons" / "mm-kyland_kyland.svg"
    ]
    offenders = {
