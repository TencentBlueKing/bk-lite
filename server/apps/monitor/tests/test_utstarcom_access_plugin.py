"""Contract tests for the UTStarcom Access SNMP plugin.

The available state confirms the UTStarcom enterprise root, but does not
...
BRAND_DIR = PLUGINS / "snmp" / "access_utstarcom"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "utstarcom"
COLLECT_TYPE = "snmp_utstarcom"
CONFIG_TYPE = "utstarcom"
INSTANCE_TYPE = "access"
PLUGIN_NAME = "Access UTStarcom SNMP"
OBJECT_NAME = "Access"
PEN_ROOT = "1.3.6.1.4.1.4666"
...
def test_brand_match_and_icon_present_in_common():
    ...
    assert "utstarcom" in text.lower()
    assert "utstar" in text.lower()
    brand_rule = text.lower().split("label: 'utstarcom'")[0].split("{ match:")[-1]
    assert "access" not in brand_rule
    assert "dsl" not in brand_rule
    assert "broadband" not in brand_rule
    assert "mm-utstarcom_utstarcom" in text
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-utstarcom_utstarcom.svg").exists()
...
def test_new_files_do_not_leak_external_source_names():
    checked_paths = [
        ...
        Path(__file__),
        WEB_ROOT / "public" / "assets" / "icons" / "mm-utstarcom_utstarcom.svg",
    ]
