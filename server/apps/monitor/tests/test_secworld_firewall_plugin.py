"""Contract tests for the Secworld firewall SNMP plugin.

Secworld (安世) reuses the Foundry/Brocade private tree (IANA PEN 1991) and
...
BRAND_DIR = PLUGINS / "snmp" / "firewall_secworld"
BASE_DIR = PLUGINS / "snmp" / "firewall_hillstone"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "secworld"
COLLECT_TYPE = "snmp_secworld"
CONFIG_TYPE = "secworld"
INSTANCE_TYPE = "firewall"
PLUGIN_NAME = "Firewall Secworld SNMP"
...
def test_no_used_free_temp_fan_psu(metrics):
    ...
    assert present == [], f"Secworld reports mem as %; must not model: {present}"
...
def test_frontend_brand_match_registered():
    ...
    assert "secworld" in common.read_text(encoding="utf-8").lower()


@pytest.mark.unit
def test_brand_icon_asset_present():
    icon = WEB_ROOT / "public" / "assets" / "icons" / "mm-secworld_secworld.svg"
    assert icon.exists()
