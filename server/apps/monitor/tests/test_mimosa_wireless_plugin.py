"""Contract tests for the Mimosa wireless SNMP plugin.

Mimosa wireless PtP / PtMP backhaul devices use enterprise PEN 43356. The confirmed
MIB facts include an internal temperature scalar reported as tenths of a degree
Celsius. CPU, memory, fan and power-supply health OIDs are not confirmed for
this pass, and wireless link or subscriber tables are not promoted into shared
device health. The plugin is therefore a single-delta Wireless child: metrics.json
only declares device_temperature_celsius while the Telegraf child keeps sysUpTime
and 64-bit IF-MIB HC collection for the shared Wireless SNMP floor.
"""
import json
...
BRAND = "mimosa"
COLLECT_TYPE = "snmp_mimosa"
CONFIG_TYPE = "mimosa"
INSTANCE_TYPE = "wireless"
PLUGIN_NAME = "Wireless Mimosa SNMP"
OBJECT_NAME = "Wireless"
PEN_ROOT = "1.3.6.1.4.1.43356"
...
def test_metrics_json_declares_only_temperature_vendor_delta_child(metrics):
    ...
    assert "/ 10" in temp["query"]


@pytest.mark.unit
def test_policy_is_subset_of_metrics(metrics, policy):
    ...
    assert bad == []
    assert {template["metric_name"] for template in policy["templates"]} == {"device_temperature_celsius"}


@pytest.mark.unit
def test_only_confirmed_temperature_private_health_oid_is_collected(toml_text):
    assert "1.3.6.1.4.1.43356.2.1.2.1.8.0" in toml_text
    assert "device_temperature_celsius" in toml_text
    for name in UNSUPPORTED_HEALTH_METRICS:
...
def test_brand_match_and_icon_present_in_common():
    ...
    assert "mm-mimosa_mimosa" in text
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-mimosa_mimosa.svg").exists()
...
def test_new_files_do_not_leak_external_source_names():
    checked_paths = [
        ...
        Path(__file__),
        WEB_ROOT / "public" / "assets" / "icons" / "mm-mimosa_mimosa.svg",
    ]
