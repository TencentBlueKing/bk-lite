"""Contract tests for the AddPac voice gateway SNMP plugin.

AddPac devices are represented conservatively in this pass: production facts
prove the VoIP/media gateway object family and SNMP MIB support, but do not
provide stable shared health leaf OIDs for CPU, memory, temperature, fan, or
power. The plugin therefore uses the shared VoiceGateway SNMP floor only and
keeps brand metrics as a zero-delta child.
"""
import json
...
BRAND_DIR = PLUGINS / "snmp" / "voice_gateway_addpac"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "addpac"
COLLECT_TYPE = "snmp_addpac"
CONFIG_TYPE = "addpac"
INSTANCE_TYPE = "voice_gateway"
PLUGIN_NAME = "VoiceGateway AddPac SNMP"
...
def test_brand_label_present_in_common():
    ...
    assert "AddPac" in text
    assert "mm-addpac_addpac" in text
    assert "{ match: /addpac|\\bap(?:26[0-9]{2}|mg[0-9]{3,4})\\b/i, label: 'AddPac'" in text
    assert "gateway|addpac" not in text.lower()
    assert "voice|addpac" not in text.lower()


@pytest.mark.unit
def test_brand_svg_exists():
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-addpac_addpac.svg").exists()
...
def test_brand_files_have_no_external_source_residue():
    ...
    paths = list(BRAND_DIR.glob("*")) + [
        WEB_ROOT / "public" / "assets" / "icons" / "mm-addpac_addpac.svg"
    ]
    offenders = {
