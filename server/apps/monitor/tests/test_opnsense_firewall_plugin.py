"""Contract tests for the OPNsense firewall SNMP plugin.

Validated against the generic **firewall** baseline (Telegraf/snmp/firewall),
NOT the Cisco switch baseline. OPNsense is the FreeBSD/pf sibling of pfSense
and reuses the same BEGEMOT-PF-MIB + HOST-RESOURCES collection, so the plugin
is a thin set:
  ...
  - firewall_pf_states (short, BEGEMOT-PF-MIB pfStateTableCount ~ active conns;
                        SHARED metric name with pfSense -> already translated)
  - interface_ifHCInOctets / ifHCOutOctets          (64-bit HC traffic)

Memory / temperature / fan / PSU are N/A (FreeBSD bsnmpd memory is hrStorage
row-level, no sensor tables) and must not be modelled. Because firewall_pf_states
is shared with pfSense, both the i18n and the shared firewall dashboard
or-chain are already in place — no new dashboard change is needed. The
collectType must wire into the firewall object, not the switch.
...
OP_DIR = PLUGINS / "snmp_opnsense" / "firewall"
...
PLUGIN_NAME = "Firewall OPNsense SNMP"
...
OP_HEALTH = ("device_cpu_usage", "firewall_pf_states")
...
def metrics():
    return _read_json(OP_DIR / "metrics.json")
...
@pytest.fixture(scope="module")
def policy():
    return _read_json(OP_DIR / "policy.json")


@pytest.fixture(scope="module")
def ui():
    return _read_json(OP_DIR / "UI.json")


@pytest.fixture(scope="module")
def toml_text():
    return (OP_DIR / "opnsense.child.toml.j2").read_text(encoding="utf-8")
...
def test_plugin_lives_under_firewall_dir():
    assert OP_DIR.parent.name == COLLECT_TYPE
    assert OP_DIR.name == INSTANCE_TYPE


@pytest.mark.unit
def test_toml_filename_follows_convention():
    assert (OP_DIR / f"{CONFIG_TYPE}.child.toml.j2").exists()
...
def test_is_superset_of_firewall_baseline(metrics, base_metrics):
    op = {m["name"] for m in metrics["metrics"]}
    base = {m["name"] for m in base_metrics["metrics"]}
    missing = base - op
    assert missing == set(), f"opnsense dropped baseline firewall metrics: {missing}"
...
# --------------------------------------------------------------------------- #
# thin OPNsense profile: CPU% aggregate + pf state table count
# --------------------------------------------------------------------------- #
@pytest.mark.unit
def test_opnsense_health_metrics_present(metrics):
    names = {m["name"] for m in metrics["metrics"]}
    missing = [m for m in OP_HEALTH if m not in names]
    assert missing == [], f"opnsense health metrics missing: {missing}"
...
# --------------------------------------------------------------------------- #
# i18n completeness (zh-Hans + en) — shared firewall_pf_states already present
# --------------------------------------------------------------------------- #
@pytest.mark.unit
...
# --------------------------------------------------------------------------- #
# frontend wiring + shared dashboard or-chain (already includes pf_states)
# --------------------------------------------------------------------------- #
@pytest.mark.unit
...
def test_frontend_collecttype_not_wired_to_switch_object():
    ...
    assert COLLECT_TYPE not in text, "opnsense firewall must not be wired into the switch object"
