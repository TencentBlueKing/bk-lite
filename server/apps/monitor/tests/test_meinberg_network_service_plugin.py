"""Contract tests for the Meinberg LANTIME NetworkService SNMP plugin.

Meinberg LANTIME appliances expose the Meinberg PEN 5597 and public LANTIME
MIB facts for NTP/PTP service objects. The current facts do not prove stable
shared health scalars for CPU, memory, temperature, fan, or power with safe
status semantics. The plugin therefore stays conservative: it uses the shared
SNMP floor only, keeps brand metrics as a zero-delta child, and does not promote
time-service indicators into device-health metrics.
"""
import json
...
BRAND_DIR = PLUGINS / "snmp" / "network_service_meinberg"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "meinberg"
COLLECT_TYPE = "snmp_meinberg"
CONFIG_TYPE = "meinberg"
INSTANCE_TYPE = "network_service"
PLUGIN_NAME = "NetworkService Meinberg LANTIME SNMP"
OBJECT_NAME = "NetworkService"
PEN_ROOT = "1.3.6.1.4.1.5597"
...
UNSUPPORTED_BUSINESS_METRICS = (
    "meinberg_ntp_sync_state",
    "meinberg_ptp_state",
    "meinberg_refclock_state",
    "meinberg_receiver_state",
)
...
@pytest.mark.unit
def test_meinberg_private_health_oids_are_not_guessed(metrics, policy, toml_text):
...
def test_brand_label_present_in_common():
    ...
    assert "Meinberg" in text
    assert "mm-meinberg_meinberg" in text


@pytest.mark.unit
def test_brand_svg_exists():
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-meinberg_meinberg.svg").exists()
...
def test_brand_files_have_no_external_source_residue():
    ...
    paths = list(BRAND_DIR.glob("*")) + [
        WEB_ROOT / "public" / "assets" / "icons" / "mm-meinberg_meinberg.svg"
    ]
    offenders = {
