"""Contract tests for the Netgear (FASTPATH) switch SNMP plugin.

Validates Telegraf/snmp_netgear/switch against the Cisco baseline and the
cross-vendor design decisions for the SNMP brand-plugin family.

Netgear (FASTPATH, enterprise 4526) reports system memory as a total/free byte
pool (same shape as Extreme), so device_memory_usage is computed
(total - free) / total and the shared brand-adaptive dashboard covers it with
zero config change. Temperature/fan/psu tables expose no descr name column, so
those metrics declare no descr dimension and their alert templates carry no
${metric_descr}. boxServicesFan/PowSupplyItemState are natively 1=operational,
but both are normalized to the unified 1=normal convention via a brand-scoped
enum processor (1 -> 1, everything else -> 2) so the policy `>1` is robust
regardless of the raw encoding.
...
CONFIG_TYPE = "netgear"
PLUGIN_NAME = "Switch Netgear SNMP"
OBJECT_NAME = "Switch"
...
def metrics():
    return _read_json(NETGEAR_DIR / "metrics.json")
...
@pytest.fixture(scope="module")
def policy():
    return _read_json(NETGEAR_DIR / "policy.json")


@pytest.fixture(scope="module")
def ui():
    return _read_json(NETGEAR_DIR / "UI.json")


@pytest.fixture(scope="module")
def toml_text():
    return (NETGEAR_DIR / "netgear.child.toml.j2").read_text(encoding="utf-8")
...
# --------------------------------------------------------------------------- #
# total/free byte-pool memory ordered before computed usage
# --------------------------------------------------------------------------- #
@pytest.mark.unit
def test_memory_total_free_ordered_before_usage(metrics):
    ...
    assert "device_memory_used" not in idx  # FASTPATH exposes total+free, not used


@pytest.mark.unit
def test_memory_usage_computed_from_total_and_free(metrics):
...
def test_fan_and_psu_normalized_via_brand_scoped_enum_processor(toml_text):
    ...
    # native 1=operational -> 1; default 2 catches every non-operational code
