"""Contract tests for the Patton SmartNode voice gateway SNMP plugin.

Patton SmartNode SBC and voice gateway devices are represented conservatively here:
...
BRAND_DIR = PLUGINS / "snmp" / "voice_gateway_patton"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "patton"
COLLECT_TYPE = "snmp_patton"
CONFIG_TYPE = "patton"
INSTANCE_TYPE = "voice_gateway"
PLUGIN_NAME = "VoiceGateway Patton SNMP"
...
def test_brand_label_present_in_common():
    ...
    assert "Patton" in text
    assert "mm-patton_patton" in text


@pytest.mark.unit
def test_brand_svg_exists():
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-patton_patton.svg").exists()
...
def test_brand_files_have_no_external_source_residue():
    ...
    paths = list(BRAND_DIR.glob("*")) + [
        WEB_ROOT / "public" / "assets" / "icons" / "mm-patton_patton.svg"
    ]
    offenders = {
