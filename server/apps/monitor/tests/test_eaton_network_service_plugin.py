"""Contract tests for the Eaton UPS/PDU/Environmental NetworkService SNMP plugin.

Eaton PowerMIB facts identify UPS, PDU, receptacle, battery, power output,
temperature, humidity and environmental tables under PEN 534. The currently available evidence does not
...
BRAND_DIR = PLUGINS / "snmp" / "network_service_eaton"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "eaton"
COLLECT_TYPE = "snmp_eaton"
CONFIG_TYPE = "eaton"
INSTANCE_TYPE = "network_service"
PLUGIN_NAME = "NetworkService Eaton UPS PDU Environmental SNMP"
OBJECT_NAME = "NetworkService"
PEN_ROOT = "1.3.6.1.4.1.534"
...
UNSUPPORTED_EATON_METRICS = (
    "eaton_input_voltage",
    "eaton_input_current",
    "eaton_output_voltage",
    "eaton_output_current",
    "eaton_power_watts",
    "eaton_power_factor",
    "eaton_battery_state",
    "eaton_bypass_state",
    "eaton_load_percent",
    "eaton_temperature_celsius",
    "eaton_humidity_percent",
    "eaton_remote_sensor_state",
    "eaton_threshold_state",
)
...
@pytest.mark.unit
def test_eaton_private_health_oids_are_not_guessed(metrics, policy, toml_text):
    names = {m["name"] for m in metrics["metrics"]}
    for absent in UNSUPPORTED_HEALTH_METRICS + UNSUPPORTED_EATON_METRICS:
...
def test_brand_label_present_in_common_without_generic_terms():
    ...
    assert "Eaton" in text
    assert "mm-eaton_eaton" in text
    assert "eaton|powerware|xups" in text
    assert "eaton|pdu|ups|network.?service" not in text


@pytest.mark.unit
def test_brand_svg_exists():
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-eaton_eaton.svg").exists()
...
def test_brand_files_have_no_external_source_residue():
    ...
    paths = list(BRAND_DIR.glob("*")) + [
        WEB_ROOT / "public" / "assets" / "icons" / "mm-eaton_eaton.svg"
    ]
    offenders = {
