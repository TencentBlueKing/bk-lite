"""Contract tests for the Yeastar voice gateway SNMP plugin.

Yeastar IP PBX and VoIP gateway devices are represented conservatively here:
the current production input proves the enterprise root and gateway product
family, but does not prove stable shared health scalars for CPU, memory,
...
BRAND_DIR = PLUGINS / "snmp" / "voice_gateway_yeastar"
...
PLUGIN_NAME = "VoiceGateway Yeastar SNMP"
...
def test_private_health_oids_are_not_guessed(metrics, policy, toml_text):
    ...
    assert "1.3.6.1.4.1.62704" not in toml_text
...
def test_brand_label_present_in_common():
    ...
    assert "Yeastar" in text
    assert "mm-yeastar_yeastar" in text
    assert "{ match: /yeastar/i, label: 'Yeastar'" in text
    assert "fxs|fxo|isdn|gsm|bri" not in text


@pytest.mark.unit
def test_brand_svg_exists():
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-yeastar_yeastar.svg").exists()
...
def test_brand_files_have_no_external_source_residue():
    ...
    paths = list(BRAND_DIR.glob("*")) + [
        WEB_ROOT / "public" / "assets" / "icons" / "mm-yeastar_yeastar.svg"
    ]
    offenders = {
