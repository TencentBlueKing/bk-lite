"""Contract tests for the Polycom voice gateway SNMP plugin.

Polycom endpoint and conference phone devices are represented conservatively
here: the current production input proves the enterprise root and endpoint MIB
subtree, but not stable shared health scalars for CPU, memory, temperature,
fan, or power. The plugin therefore uses the shared VoiceGateway SNMP floor
only and keeps brand metrics as a zero-delta child.
"""
import json
...
BRAND_DIR = PLUGINS / "snmp" / "voice_gateway_polycom"
...
PLUGIN_NAME = "VoiceGateway Polycom SNMP"
...
def test_brand_label_present_in_common():
    ...
    assert "Polycom" in text
    assert "mm-polycom_polycom" in text
    assert "endpoint" not in text.lower().split("polycom", 1)[0]


@pytest.mark.unit
def test_brand_svg_exists():
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-polycom_polycom.svg").exists()
...
def test_brand_files_have_no_external_source_residue():
    ...
    paths = list(BRAND_DIR.glob("*")) + [
        WEB_ROOT / "public" / "assets" / "icons" / "mm-polycom_polycom.svg"
    ]
    offenders = {
