"""Contract tests for the Geist PDU NetworkService SNMP plugin.

Geist and Vertiv MIB facts identify environmental, relay, RPM, outlet, device-availability and remote-power tables. The currently available evidence does not
...
metrics as a zero-delta child, and does not promote PDU and environmental business tables
...
BRAND_DIR = PLUGINS / "snmp" / "network_service_geist"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "geist"
COLLECT_TYPE = "snmp_geist"
CONFIG_TYPE = "geist"
INSTANCE_TYPE = "network_service"
PLUGIN_NAME = "NetworkService Geist PDU Environmental SNMP"
OBJECT_NAME = "NetworkService"
PEN_ROOT = "1.3.6.1.4.1.21239"
...
UNSUPPORTED_PDU_METRICS = (
    "geist_temperature_celsius",
    "geist_humidity_percent",
    "geist_airflow_state",
    "geist_relay_state",
    "geist_rpm_state",
    "geist_analog_value",
    "geist_outlet_state",
    "geist_device_availability",
    "geist_remote_power_state",
)
...
@pytest.mark.unit
def test_geist_private_health_oids_are_not_guessed(metrics, policy, toml_text):
...
def test_brand_label_present_in_common_without_generic_terms():
    ...
    assert "mm-geist_geist" in text
    assert "geist|blackbird|watchdog" in text
    assert "geist|pdu|network.?service" not in text


@pytest.mark.unit
def test_brand_svg_exists():
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-geist_geist.svg").exists()
...
def test_brand_files_have_no_external_source_residue():
    ...
    paths = list(BRAND_DIR.glob("*")) + [
        WEB_ROOT / "public" / "assets" / "icons" / "mm-geist_geist.svg"
    ]
    offenders = {
