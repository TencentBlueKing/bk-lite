"""Contract tests for the Enlogic PDU NetworkService SNMP plugin.

Enlogic PDU MIB facts expose PDU, power and external-sensor tables, but the
...
floor only, keeps brand metrics as a zero-delta child, and does not promote PDU
...
BRAND_DIR = PLUGINS / "snmp" / "network_service_enlogic"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "enlogic"
COLLECT_TYPE = "snmp_enlogic"
CONFIG_TYPE = "enlogic"
INSTANCE_TYPE = "network_service"
PLUGIN_NAME = "NetworkService Enlogic PDU SNMP"
OBJECT_NAME = "NetworkService"
PEN_ROOT = "1.3.6.1.4.1.38446"
...
UNSUPPORTED_PDU_METRICS = (
    "enlogic_input_current",
    "enlogic_input_voltage",
    "enlogic_input_power",
    "enlogic_outlet_state",
    "enlogic_breaker_state",
    "enlogic_sensor_temperature",
    "enlogic_sensor_humidity",
    "enlogic_cabinet_state",
)
...
@pytest.mark.unit
def test_enlogic_private_health_oids_are_not_guessed(metrics, policy, toml_text):
...
def test_brand_label_present_in_common_without_generic_terms():
    ...
    assert "Enlogic" in text
    assert "mm-enlogic_enlogic" in text
    assert "enlogic|en2\\.?" in text
    assert "pdu|sensor|cabinet|network.?service" not in text


@pytest.mark.unit
def test_brand_svg_exists():
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-enlogic_enlogic.svg").exists()
...
def test_brand_files_have_no_external_source_residue():
    ...
    paths = list(BRAND_DIR.glob("*")) + [
        WEB_ROOT / "public" / "assets" / "icons" / "mm-enlogic_enlogic.svg"
    ]
    offenders = {
