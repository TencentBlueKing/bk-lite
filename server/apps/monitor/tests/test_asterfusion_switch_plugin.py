"""Contract tests for the AsterFusion Switch SNMP plugin.

AsterFusion AsterNOS exposes SNMP and the AsterFusion PEN 55955, but the
current facts do not prove stable shared health scalars for CPU, memory,
temperature, fan, or power. The plugin therefore stays conservative: it uses
the shared SNMP floor only and keeps brand metrics as a zero-delta child.
"""
import json
...
BRAND_DIR = PLUGINS / "snmp" / "switch_asterfusion"
...
PLUGIN_NAME = "Switch AsterFusion SNMP"
OBJECT_NAME = "Switch"
PEN_ROOT = "1.3.6.1.4.1.55955"
...
UNSUPPORTED_PRIVATE_METRICS = (
    "asterfusion_roce_state",
    "asterfusion_bgp_state",
    "asterfusion_rib_state",
)
...
@pytest.mark.unit
def test_asterfusion_private_health_oids_are_not_guessed(metrics, policy, toml_text):
    names = {m["name"] for m in metrics["metrics"]}
    for absent in UNSUPPORTED_HEALTH_METRICS + UNSUPPORTED_PRIVATE_METRICS:
...
def test_brand_label_present_in_common_without_generic_terms():
    ...
    assert "AsterFusion" in text
    assert "mm-asterfusion_asterfusion" in text
    assert "asterfusion|asternos" in text
    assert "asterfusion|switch" not in text
    assert "asterfusion|storage" not in text
    assert "asterfusion|router" not in text


@pytest.mark.unit
def test_brand_svg_exists():
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-asterfusion_asterfusion.svg").exists()
...
def test_brand_files_have_no_external_source_residue():
    ...
    paths = list(BRAND_DIR.glob("*")) + [
        WEB_ROOT / "public" / "assets" / "icons" / "mm-asterfusion_asterfusion.svg"
    ]
    offenders = {
