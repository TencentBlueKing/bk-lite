"""Contract tests for the Acme Packet voice gateway SNMP plugin."""
import json
...
BRAND_DIR = PLUGINS / "snmp" / "voice_gateway_acmepacket"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "acmepacket"
COLLECT_TYPE = "snmp_acmepacket"
CONFIG_TYPE = "acmepacket"
INSTANCE_TYPE = "voice_gateway"
PLUGIN_NAME = "VoiceGateway Acme Packet SNMP"
...
def test_brand_label_present_in_common():
    ...
    assert "Acme Packet" in text
    assert "mm-acmepacket_acmepacket" in text
    assert "session|border|controller" not in text


@pytest.mark.unit
def test_brand_svg_exists():
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-acmepacket_acmepacket.svg").exists()
...
def test_brand_files_have_no_external_source_residue():
    ...
    paths = list(BRAND_DIR.glob("*")) + [
        WEB_ROOT / "public" / "assets" / "icons" / "mm-acmepacket_acmepacket.svg"
    ]
    offenders = {
