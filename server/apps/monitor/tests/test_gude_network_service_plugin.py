"""Contract tests for the Gude PDU NetworkService SNMP plugin.

Gude PDU and ETS MIB facts identify PDU power, channel status and primary or
secondary power-availability tables. The currently available evidence does not
...
metrics as a zero-delta child, and does not promote PDU or ETS business tables
...
BRAND_DIR = PLUGINS / "snmp" / "network_service_gude"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "gude"
COLLECT_TYPE = "snmp_gude"
CONFIG_TYPE = "gude"
INSTANCE_TYPE = "network_service"
PLUGIN_NAME = "NetworkService Gude PDU SNMP"
OBJECT_NAME = "NetworkService"
PEN_ROOT = "1.3.6.1.4.1.28507"
...
UNSUPPORTED_PDU_METRICS = (
    ...
    "gude_current_ampere",
    "gude_voltage_volt",
    "gude_frequency_hertz",
    "gude_power_factor",
    "gude_energy_total",
    "gude_primary_power_available",
    "gude_secondary_power_available",
)
...
@pytest.mark.unit
def test_gude_private_health_oids_are_not_guessed(metrics, policy, toml_text):
...
def test_brand_label_present_in_common_without_generic_terms():
    ...
    assert "Gude" in text
    assert "mm-gude_gude" in text
    assert "gude|expert\s*power\s*control" in text
    assert "gude|pdu|network.?service" not in text


@pytest.mark.unit
def test_brand_svg_exists():
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-gude_gude.svg").exists()
...
def test_brand_files_have_no_external_source_residue():
    ...
    paths = list(BRAND_DIR.glob("*")) + [
        WEB_ROOT / "public" / "assets" / "icons" / "mm-gude_gude.svg"
    ]
    offenders = {
