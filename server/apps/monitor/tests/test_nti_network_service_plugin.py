"""Contract tests for the NTI ENVIROMUX NetworkService SNMP plugin.

NTI ENVIROMUX MIB facts identify internal, auxiliary, external and IP sensor
tables, ACLM power tables, dry-contact, relay, message and alarm objects. The
...
floor only, keeps brand metrics as a zero-delta child, and does not promote
environment sensor, relay, alarm or power business tables into generic
...
BRAND_DIR = PLUGINS / "snmp" / "network_service_nti"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "nti"
COLLECT_TYPE = "snmp_nti"
CONFIG_TYPE = "nti"
INSTANCE_TYPE = "network_service"
PLUGIN_NAME = "NetworkService NTI ENVIROMUX SNMP"
OBJECT_NAME = "NetworkService"
PEN_ROOT = "1.3.6.1.4.1.3699"
...
UNSUPPORTED_ENVIROMUX_METRICS = (
    "nti_temperature_celsius",
    "nti_humidity_percent",
    "nti_voltage_volt",
    "nti_current_ampere",
    "nti_frequency_hertz",
    "nti_sensor_state",
    "nti_dry_contact_state",
    "nti_relay_state",
    "nti_alarm_state",
    "nti_power_state",
)
...
@pytest.mark.unit
def test_nti_private_health_oids_are_not_guessed(metrics, policy, toml_text):
    names = {m["name"] for m in metrics["metrics"]}
    for absent in UNSUPPORTED_HEALTH_METRICS + UNSUPPORTED_ENVIROMUX_METRICS:
...
def test_brand_svg_exists():
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-nti_nti.svg").exists()
...
def test_brand_files_have_no_external_source_residue():
    ...
    paths = list(BRAND_DIR.glob("*")) + [
        WEB_ROOT / "public" / "assets" / "icons" / "mm-nti_nti.svg"
    ]
    offenders = {
