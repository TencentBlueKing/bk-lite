"""Contract tests for the BDCOM switch SNMP plugin.

Validates Telegraf/snmp_bdcom/switch against the Cisco baseline and the
cross-vendor design decisions for the SNMP brand-plugin family.

BDCOM (Shanghai Baud Data Communication, BDCOM private MIBs, IANA PEN 3320)
mirrors the Cisco-style health structure: control-plane CPU over the last 5
minutes (cpmCPUTotal5min), a system memory pool exposing used and free bytes
(mirroring CISCO-MEMORY-POOL-MIB) and a dedicated per-card temperature column
(nmscardTemperature). Unlike Eltex/SNR there is NO fan-state and NO PSU object
in the BDCOM switch MIBs, so neither device_fan_state nor device_psu_state is
modelled (both N/A) and there must be NO processors.enum block.

  - device_cpu_usage: percent, group CPU
  - device_memory_used / device_memory_free: bytes, group Memory
  - device_memory_usage: percent = used/(used+free)*100 (branch: used+free)
  - device_temperature_celsius: max per instance, group Temperature
  - interface_ifHCIn/OutOctets: byte-identical Cisco

BDCOM reuses the shared Switch metric names + existing CPU/Memory/Temperature/
Traffic groups, so i18n and the shared switch dashboard are already in place.
New brand `bdcom` adds a common.tsx match + icon + switch.tsx collect-type wire.
...
ABSENT_METRICS = ("device_fan_state", "device_psu_state")
BDCOM_PEN_ROOT = "1.3.6.1.4.1.3320"
...
def metrics():
    return _read_json(BDCOM_DIR / "metrics.json")
...
@pytest.fixture(scope="module")
def policy():
    return _read_json(BDCOM_DIR / "policy.json")


@pytest.fixture(scope="module")
def ui():
    return _read_json(BDCOM_DIR / "UI.json")


@pytest.fixture(scope="module")
def toml_text():
    return (BDCOM_DIR / "bdcom.child.toml.j2").read_text(encoding="utf-8")
...
def test_plugin_lives_under_correct_dir(metrics):
    ...
    assert BDCOM_DIR.parent.name == "snmp"  # 扁平布局:厂商目录直接在 snmp/ 下


@pytest.mark.unit
def test_toml_filename_follows_convention():
    assert (BDCOM_DIR / f"{CONFIG_TYPE}.child.toml.j2").exists()
...
# --------------------------------------------------------------------------- #
# CPU + interface parity with Cisco
# --------------------------------------------------------------------------- #
@pytest.mark.unit
...
def test_memory_usage_is_used_over_used_plus_free(metrics):
    ...
    assert ext["metric_group"] == "Memory"
    assert ext["dimensions"] == []
    q = ext["query"].replace(" ", "")
    # used / (used + free) * 100
    assert "device_memory_used" in q and "device_memory_free" in q
    assert "/(" in q and "+sum" in q and "*100" in q, \
        "memory_usage must be computed as used/(used+free)*100"


# --------------------------------------------------------------------------- #
# Temperature present; NO fan / NO PSU (BDCOM switch MIB has neither)
# --------------------------------------------------------------------------- #
@pytest.mark.unit
...
@pytest.mark.unit
def test_fan_and_psu_not_modelled(metrics):
    names = {m["name"] for m in metrics["metrics"]}
    present = [a for a in ABSENT_METRICS if a in names]
    assert present == [], f"BDCOM switch MIB has no fan/psu object; must not model: {present}"


@pytest.mark.unit
def test_no_enum_processor_block(toml_text):
    assert "[[processors.enum]]" not in toml_text, \
        "BDCOM has no fan/psu enum to normalize; no processors.enum expected"


# --------------------------------------------------------------------------- #
# OID hygiene: private OIDs rooted under BDCOM PEN 3320; 64-bit IF-MIB counters
# --------------------------------------------------------------------------- #
@pytest.mark.unit
def test_private_oids_under_bdcom_pen(toml_text):
    # CPU / memory / temperature private columns must live under PEN 3320
    for marker in ("device_cpu", "device_memory", "device_temperature"):
        assert marker in toml_text
    assert BDCOM_PEN_ROOT in toml_text, "BDCOM private OIDs must be rooted under PEN 3320"
...
def test_policy_covers_cpu_mem_temp_only(policy):
    ...
    }, "BDCOM policy must cover cpu/mem/temp and NOT fan/psu"
...
def test_frontend_brand_match_registered():
    ...
    assert "bdcom" in text.lower(), "common.tsx must register a BDCOM brand match"


@pytest.mark.unit
def test_brand_icon_asset_present():
    icon = WEB_ROOT / "public" / "assets" / "icons" / "mm-bdcom_bdcom.svg"
    assert icon.exists(), "BDCOM brand icon mm-bdcom_bdcom.svg must exist"
