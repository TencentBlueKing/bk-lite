"""Contract tests for the Raritan SX console-server SNMP plugin."""
import json
...
BRAND_DIR = PLUGINS / "snmp" / "console_server_raritan"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "raritan"
COLLECT_TYPE = "snmp_raritan"
CONFIG_TYPE = "raritan"
INSTANCE_TYPE = "console_server"
PLUGIN_NAME = "ConsoleServer Raritan SX SNMP"
OBJECT_NAME = "ConsoleServer"
RARITAN_ROOT = "1.3.6.1.4.1.13742"
BASE_METRICS = {
    "snmp_uptime",
...
@pytest.mark.unit
def test_no_unverified_private_health_metrics_are_promoted(metrics, policy, toml_text):
    names = {m["name"] for m in metrics["metrics"]}
    assert UNSUPPORTED_METRICS.isdisjoint(names)
    assert RARITAN_ROOT not in toml_text
...
def test_brand_label_present_in_common_without_generic_terms():
    ...
    assert "Raritan SX" in text
    assert "mm-raritan_raritan" in text
    assert "dominion" in text.lower()
    raritan_line = next(line for line in text.splitlines() if "mm-raritan_raritan" in line)
    assert "console" not in raritan_line.lower()
    assert "serial" not in raritan_line.lower()
    assert "server" not in raritan_line.lower()


@pytest.mark.unit
def test_brand_svg_exists():
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-raritan_raritan.svg").exists()
...
def test_brand_files_have_no_external_source_residue():
    ...
    paths = list(BRAND_DIR.glob("*")) + [
        WEB_ROOT / "public" / "assets" / "icons" / "mm-raritan_raritan.svg"
    ]
    offenders = {
