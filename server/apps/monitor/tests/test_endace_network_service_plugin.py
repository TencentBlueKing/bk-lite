"""Contract tests for the Endace NetworkService SNMP plugin.

Endace network visibility appliances expose the Endace PEN 18418, but the current
MIB facts do not prove stable shared health scalars for CPU, memory,
...
not promote capture or visibility indicators into device-health metrics.
"""
import json
...
BRAND_DIR = PLUGINS / "snmp" / "network_service_endace"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "endace"
COLLECT_TYPE = "snmp_endace"
CONFIG_TYPE = "endace"
INSTANCE_TYPE = "network_service"
PLUGIN_NAME = "NetworkService Endace SNMP"
OBJECT_NAME = "NetworkService"
PEN_ROOT = "1.3.6.1.4.1.18418"
...
UNSUPPORTED_BUSINESS_METRICS = (
    "endace_capture_drop_rate",
    "endace_packet_capture_rate",
    "endace_visibility_session_count",
)
...
@pytest.mark.unit
def test_endace_private_health_oids_are_not_guessed(metrics, policy, toml_text):
...
def test_brand_label_present_in_common():
    ...
    assert "Endace" in text
    assert "mm-endace_endace" in text
    assert "endace|dag" not in text


@pytest.mark.unit
def test_brand_svg_exists():
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-endace_endace.svg").exists()
...
def test_brand_files_have_no_external_source_residue():
    ...
    paths = list(BRAND_DIR.glob("*")) + [
        WEB_ROOT / "public" / "assets" / "icons" / "mm-endace_endace.svg"
    ]
    offenders = {
