"""Contract tests for the EfficientIP NetworkService SNMP plugin.

EfficientIP SOLIDserver DDI materials and public OID identity confirm SNMP
capability under PEN 2440, including hardware and service subtree names. The
available evidence does not prove stable shared-health scalar leaf OIDs or
enum meanings for CPU, memory, temperature, fan, PSU, DNS, or DHCP health. The
plugin therefore stays conservative: it uses the shared SNMP floor only, keeps
brand metrics as a zero-delta child, and does not promote private subtrees into
...
BRAND_DIR = PLUGINS / "snmp" / "network_service_efficientip"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "efficientip"
COLLECT_TYPE = "snmp_efficientip"
CONFIG_TYPE = "efficientip"
INSTANCE_TYPE = "network_service"
PLUGIN_NAME = "NetworkService EfficientIP SNMP"
OBJECT_NAME = "NetworkService"
PEN_ROOT = "1.3.6.1.4.1.2440"
...
UNSUPPORTED_EFFICIENTIP_METRICS = (
    "efficientip_hw_temperature",
    "efficientip_hw_fan_state",
    "efficientip_hw_psu_state",
    "efficientip_hw_cpu_usage",
    "efficientip_hw_memory_usage",
    "efficientip_dns_state",
    "efficientip_dhcp_state",
    "efficientip_service_state",
    "efficientip_snmp_service_state",
)
...
@pytest.mark.unit
def test_efficientip_private_health_oids_are_not_guessed(metrics, policy, toml_text):
    names = {m["name"] for m in metrics["metrics"]}
    for absent in UNSUPPORTED_HEALTH_METRICS + UNSUPPORTED_EFFICIENTIP_METRICS:
...
def test_brand_label_present_in_common_without_generic_terms():
    ...
    assert "EfficientIP" in text
    assert "mm-efficientip_efficientip" in text
    assert r"efficient\s*ip|solidserver" in text
    assert "efficient|ddi|dns|dhcp|network.?service" not in text


@pytest.mark.unit
def test_brand_svg_exists():
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-efficientip_efficientip.svg").exists()
...
def test_brand_files_have_no_external_source_residue():
    ...
    paths = list(BRAND_DIR.glob("*")) + [
        WEB_ROOT / "public" / "assets" / "icons" / "mm-efficientip_efficientip.svg"
    ]
    offenders = {
