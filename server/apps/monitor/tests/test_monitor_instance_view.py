...
def test_effective_plugins_service_does_not_primary_match_when_secondary_labels_exist(db, monkeypatch):
    # 回归：主键退化只适用于 status_query 缺少 pod/node 等副维度标签的旧 K8S 查询。
    # 若 status_query 已按完整维度上报,同集群下另一个 Pod 不应被误判为命中该插件。
    from apps.monitor.services import effective_plugins

    monitor_object = MonitorObject.objects.create(
        name="PodFullKey",
        display_name="PodFullKey",
        instance_id_keys=["instance_id", "pod"],
    )
    reported_plugin = MonitorPlugin.objects.create(
        name="K8SFullKey",
        display_name="K8SFullKey",
        template_id="k8s-full-key",
        template_type="api",
        collector="push_api",
        collect_type="push_api",
        status_query="any({instance_type='k8s'}) by (instance_id,pod)",
        is_pre=False,
    )
    reported_plugin.monitor_object.add(monitor_object)

    target_instance_id = "('mac', 'target-pod')"
    ...
    class StubVictoriaMetricsAPI:
        def query(self, query, step="5m", time=None):
            return {
                "data": {
                    "result": [
                        {"metric": {"instance_id": "mac", "pod": "other-pod"}, "value": [100, "1"]}
                    ]
                }
            }
    ...
    assert result == [], "完整副维度已上报时,同集群其他 Pod 不应按主键退化误命中"
