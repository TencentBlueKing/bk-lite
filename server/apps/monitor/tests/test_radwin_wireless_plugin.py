"""Contract tests for the Radwin wireless SNMP plugin.

Radwin wireless broadband PtP/PtMP devices use enterprise PEN 4458. The current
state facts confirm RADWIN-MIB-WINLINK1000 and Radwin enterprise identity, but
no stable row-filter-free CPU, memory, temperature, fan or power-supply
device-health OID is confirmed for this pass. The plugin is therefore a
zero-delta Wireless child: metrics.json declares no vendor-specific metrics,
while the Telegraf child keeps sysUpTime and 64-bit IF-MIB HC collection for the
shared Wireless SNMP floor.
"""
import json
...
BRAND_DIR = PLUGINS / "snmp" / "wireless_radwin"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "radwin"
COLLECT_TYPE = "snmp_radwin"
CONFIG_TYPE = "radwin"
INSTANCE_TYPE = "wireless"
PLUGIN_NAME = "Wireless Radwin SNMP"
OBJECT_NAME = "Wireless"
PEN_ROOT = "1.3.6.1.4.1.4458"
...
def test_brand_match_and_icon_present_in_common():
    ...
    assert "mm-radwin_radwin" in text
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-radwin_radwin.svg").exists()
...
def test_new_files_do_not_leak_external_source_names():
    checked_paths = [
        ...
        Path(__file__),
        WEB_ROOT / "public" / "assets" / "icons" / "mm-radwin_radwin.svg",
    ]
