"""Contract tests for the EnGenius wireless SNMP plugin.

EnGenius enterprise wireless APs (Senao PEN 14125) expose config-oriented
private MIBs with no reliably pollable CPU/memory/temperature/client-count/
channel-utilization objects across the cloud-managed (ECW/EWS) and standalone
(ENH/ENS) ranges. The plugin is therefore a baseline template (aligned with
...
BRAND_DIR = PLUGINS / "snmp" / "wireless_engenius"
...
PLUGIN_NAME = "Wireless EnGenius SNMP"
...
def test_no_health_metrics_faked(metrics):
    ...
    for absent in ("device_cpu_usage", "device_memory_usage", "device_memory_used",
        ...
        assert absent not in names, f"{absent} has no reliable EnGenius object -> N/A"
