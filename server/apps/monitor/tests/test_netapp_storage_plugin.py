"""Contract tests for the NetApp storage SNMP plugin.

NetApp FAS/AFF (enterprise PEN 789, NETWORK-APPLIANCE-MIB) exposes:

  - device_cpu_usage  : cpuBusyTimePerCent (.789.1.2.1.3.0), direct percent
  - device_disk_state : raidStatus (.789.1.6.1.1.3), active=1 normalized to 1=normal/2=fault

Memory/temperature/fan/power are N/A (env scalars are count/boolean, not enum
semantics). The storage object is shared (hardwareDevice/storage.tsx).
"""
import json
...
BRAND_DIR = PLUGINS / "snmp" / "storage_netapp"
...
PLUGIN_NAME = "Storage NetApp SNMP"
OBJECT_NAME = "Storage"
PEN_ROOT = "1.3.6.1.4.1.789"
...
@pytest.mark.unit
def test_uses_private_pen_789(toml_text):
    assert PEN_ROOT in toml_text, "NetApp private health lives under PEN 789"
...
def test_cpu_is_direct_percent(metrics, toml_text):
    ...
    assert cpu["unit"] == "percent" and cpu["metric_group"] == "CPU"
    assert "1.3.6.1.4.1.789.1.2.1.3.0" in toml_text  # cpuBusyTimePerCent


@pytest.mark.unit
def test_disk_state_is_enum_normalized(metrics):
    by = {m["name"]: m for m in metrics["metrics"]}
    assert "device_disk_state" in by
    m = by["device_disk_state"]
    ...
    assert {1, 2}.issubset(ids), "device_disk_state must expose 正常(1)/异常(2)"


@pytest.mark.unit
def test_enum_processor_normalizes_disk_active_to_normal(toml_text):
    assert 'namepass = ["device_disk"]' in toml_text
    assert "default = 2" in toml_text
    assert '"1" = 1' in toml_text  # active -> normal


@pytest.mark.unit
def test_no_memory_temp_fan_psu_modelled(metrics):
    names = {m["name"] for m in metrics["metrics"]}
    for absent in ("device_memory_usage", "device_temperature_celsius",
                   "device_fan_state", "device_psu_state"):
        assert absent not in names, \
            f"{absent}: NetApp env scalars are count/boolean -> N/A"
...
def test_storage_dashboard_renders_health_panels():
    ...
    text = storage_tsx.read_text(encoding="utf-8")
    for panel in ("device_cpu_usage", "device_disk_state"):
