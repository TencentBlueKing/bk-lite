"""Contract tests for the Nomadix NetworkService SNMP plugin.

Nomadix public SNMP facts confirm PEN 3309 and the AG-2000w gateway MIB
identity, but no stable public private CPU, memory, temperature, fan or power
leaf OID is available in this workspace. The plugin is intentionally a baseline
NetworkService SNMP child.
"""
import json
...
BRAND_DIR = PLUGINS / "snmp" / "network_service_nomadix"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "nomadix"
COLLECT_TYPE = "snmp_nomadix"
CONFIG_TYPE = "nomadix"
INSTANCE_TYPE = "network_service"
PLUGIN_NAME = "NetworkService Nomadix SNMP"
OBJECT_NAME = "NetworkService"
PEN_ROOT = "1.3.6.1.4.1.3309"
...
UNSUPPORTED_METRICS = {
    ...
    "device_psu_state",
    "device_alarm_state",
    "network_service_subscriber_count",
    "network_service_portal_session_count",
    "network_service_bandwidth_usage",
}
...
def test_private_health_oids_are_not_collected_without_leaf_evidence(toml_text):
    ...
    forbidden_terms = ("fan", "power", "temperature", "cpu", "memory", "subscriber", "portal", "bandwidth")
...
def test_frontend_collecttype_wired_to_network_service_object():
    ...
    text = network_service_tsx.read_text(encoding="utf-8")
...
def test_brand_match_and_icon_present_in_common():
    ...
    nomadix_line = next(line for line in text.splitlines() if "mm-nomadix_nomadix" in line)
    assert "nomadix" in nomadix_line
    assert "ag-?2000w" in nomadix_line
    assert "gateway|hotel|network|mdu|access" not in nomadix_line.lower()
    assert "mm-nomadix_nomadix" in text
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-nomadix_nomadix.svg").exists()
...
def test_new_files_do_not_leak_external_source_names():
    checked_paths = [
        ...
        Path(__file__),
        WEB_ROOT / "public" / "assets" / "icons" / "mm-nomadix_nomadix.svg",
    ]
