"""Contract tests for the Tripp Lite UPS/PDU/Environmental NetworkService SNMP plugin.

Official Tripp Lite/Eaton materials confirm PowerAlert-managed UPS, PDU and
environmental sensor SNMP capability and public MIB file names under PEN 3417.
The available evidence does not prove stable shared health scalars for CPU,
memory, fan, or power. The plugin therefore stays conservative: it uses the
shared SNMP floor only, keeps brand metrics as a zero-delta child, and does not
promote UPS, PDU or environmental business tables into device-health metrics.
"""
import json
...
BRAND = "tripplite"
COLLECT_TYPE = "snmp_tripplite"
CONFIG_TYPE = "tripplite"
INSTANCE_TYPE = "network_service"
PLUGIN_NAME = "NetworkService Tripp Lite UPS PDU Environmental SNMP"
OBJECT_NAME = "NetworkService"
PEN_ROOT = "1.3.6.1.4.1.3417"
...
UNSUPPORTED_TRIPPLITE_METRICS = (
    "tripplite_input_voltage",
    "tripplite_input_current",
    "tripplite_output_voltage",
    "tripplite_output_current",
    "tripplite_power_watts",
    "tripplite_power_factor",
    "tripplite_battery_state",
    "tripplite_load_percent",
    "tripplite_temperature_celsius",
    "tripplite_humidity_percent",
    "tripplite_water_sensor_state",
    "tripplite_remote_sensor_state",
    "tripplite_outlet_state",
)
...
@pytest.mark.unit
def test_tripplite_private_health_oids_are_not_guessed(metrics, policy, toml_text):
    names = {m["name"] for m in metrics["metrics"]}
    for absent in UNSUPPORTED_HEALTH_METRICS + UNSUPPORTED_TRIPPLITE_METRICS:
...
def test_brand_label_present_in_common_without_generic_terms():
    ...
    assert "Tripp Lite" in text
    assert "mm-tripplite_tripplite" in text
    assert r"tripp\s*lite|poweralert|webcardlx|snmpwebcard" in text
    assert "tripp|pdu|ups|network.?service" not in text


@pytest.mark.unit
def test_brand_svg_exists():
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-tripplite_tripplite.svg").exists()
...
def test_brand_files_have_no_external_source_residue():
    ...
    paths = list(BRAND_DIR.glob("*")) + [
        WEB_ROOT / "public" / "assets" / "icons" / "mm-tripplite_tripplite.svg"
    ]
    offenders = {
