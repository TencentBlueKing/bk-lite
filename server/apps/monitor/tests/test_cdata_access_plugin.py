"""Contract tests for the C-Data Access SNMP plugin.

The state confirms the C-Data enterprise root and GPON access family, but does
not confirm reliable row-filter-free scalar CPU, memory, temperature, fan or
power supply health OIDs. This child is therefore conservative: metrics.json
declares no vendor metric deltas, while the shared Access SNMP floor supplies
uptime, IF-MIB/ifXTable traffic and aggregate device traffic.
"""
import json
...
BRAND_DIR = PLUGINS / "snmp" / "access_cdata"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "cdata"
COLLECT_TYPE = "snmp_cdata"
CONFIG_TYPE = "cdata"
INSTANCE_TYPE = "access"
PLUGIN_NAME = "Access C-Data SNMP"
OBJECT_NAME = "Access"
PEN_ROOT = "1.3.6.1.4.1.34592"
PRIVATE_MIB_MARKERS = ("CDATA-GPON-MIB", "C-Data")
...
def test_brand_match_and_icon_present_in_common():
    ...
    assert "c-?data" in text.lower()
    assert "cdatatec" in text.lower()
    brand_rule = text.lower().split("label: 'c-data'")[0].split("{ match:")[-1]
    assert "access" not in brand_rule
    assert "gpon" not in brand_rule
    assert "olt" not in brand_rule
    assert "mm-cdata_cdata" in text
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-cdata_cdata.svg").exists()
...
def test_new_files_do_not_leak_external_source_names():
    checked_paths = [
        ...
        Path(__file__),
        WEB_ROOT / "public" / "assets" / "icons" / "mm-cdata_cdata.svg",
    ]
