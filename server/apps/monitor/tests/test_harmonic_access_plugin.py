"""Contract tests for the Harmonic Access SNMP plugin.

Harmonic public SNMP facts confirm PEN 1563 and the NSG Pro access-platform
identity, but no stable row-filter-free private CPU, memory, temperature, fan
or power OID is available in this workspace. The plugin is intentionally a
baseline Access SNMP child.
"""
import json
...
BRAND_DIR = PLUGINS / "snmp" / "access_harmonic"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "harmonic"
COLLECT_TYPE = "snmp_harmonic"
CONFIG_TYPE = "harmonic"
INSTANCE_TYPE = "access"
PLUGIN_NAME = "Access Harmonic SNMP"
OBJECT_NAME = "Access"
PEN_ROOT = "1.3.6.1.4.1.1563"
...
def test_private_health_oids_are_not_collected_without_leaf_evidence(toml_text):
    ...
    forbidden_terms = ("hObjects", "hTrapFields", "fan", "power", "temperature", "cpu", "memory")
...
def test_brand_match_and_icon_present_in_common():
    ...
    assert "nsg" in text
    assert "access|broadband|storage|router" not in text.lower()
    assert "mm-harmonic_harmonic" in text
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-harmonic_harmonic.svg").exists()
...
def test_new_files_do_not_leak_external_source_names():
    checked_paths = [
        ...
        Path(__file__),
        WEB_ROOT / "public" / "assets" / "icons" / "mm-harmonic_harmonic.svg",
    ]
