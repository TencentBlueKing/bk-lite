"""Contract tests for the Viavi/JDSU transmission SNMP plugin.

Viavi/JDSU optical transmission and network visibility devices expose enterprise
PEN 26052, but the current production input does not prove stable shared health
scalars for CPU, memory, temperature, fan, or power. The plugin therefore stays
conservative: it uses the shared SNMP floor only, keeps brand metrics as a
zero-delta child, and does not promote product or test-instrument counters into
...
BRAND_DIR = PLUGINS / "snmp" / "transmission_viavi"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "viavi"
COLLECT_TYPE = "snmp_viavi"
CONFIG_TYPE = "viavi"
INSTANCE_TYPE = "transmission"
PLUGIN_NAME = "Transmission Viavi SNMP"
OBJECT_NAME = "Transmission"
PEN_ROOT = "1.3.6.1.4.1.26052"
...
def test_brand_label_present_in_common():
    ...
    assert "Viavi" in text
    assert "mm-viavi_viavi" in text


@pytest.mark.unit
def test_brand_svg_exists():
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-viavi_viavi.svg").exists()
...
def test_brand_files_have_no_external_source_residue():
    ...
    paths = list(BRAND_DIR.glob("*")) + [
        WEB_ROOT / "public" / "assets" / "icons" / "mm-viavi_viavi.svg"
    ]
    offenders = {
