"""Contract tests for the Grandstream wireless SNMP plugin.

Grandstream GWN wireless APs (PEN 42397) expose the GRANDSTREAM-GWN-MIB which is
oriented to radio statistics and per-client info: only gwnDeviceUptime is a scalar
system object, with no CPU/memory/temperature objects, and associated clients
appear only as the per-MAC gwnClientTable with no scalar count that telegraf can
aggregate row-wise. The plugin is therefore a baseline template (aligned with
wireless_proxim): sysUpTime + 64-bit IF-MIB HC traffic + device_total_* roll-ups.
...
BRAND_DIR = PLUGINS / "snmp" / "wireless_grandstream"
...
PLUGIN_NAME = "Wireless Grandstream SNMP"
...
def test_no_health_metrics_faked(metrics):
    ...
    for absent in ("device_cpu_usage", "device_memory_usage", "device_memory_used",
        ...
        assert absent not in names, f"{absent} has no reliable Grandstream object -> N/A"
