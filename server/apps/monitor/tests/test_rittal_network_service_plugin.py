"""Contract tests for the Rittal CMC III NetworkService SNMP plugin.

CMC III facts identify unit, power-load, current-consumption, device-status,
and variable tables. The available evidence does not prove stable shared health
scalars for CPU, memory, fan, or power, so the plugin stays conservative: it
uses the shared SNMP floor only and does not promote environment or PDU
business tables into generic device-health metrics.
"""
import json
...
BRAND_DIR = PLUGINS / "snmp" / "network_service_rittal"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "rittal"
COLLECT_TYPE = "snmp_rittal"
CONFIG_TYPE = "rittal"
INSTANCE_TYPE = "network_service"
PLUGIN_NAME = "NetworkService Rittal CMC III SNMP"
OBJECT_NAME = "NetworkService"
PEN_ROOT = "1.3.6.1.4.1.2606.7"
...
UNSUPPORTED_CMC_METRICS = (
    "rittal_unit_status",
    "rittal_device_status",
    "rittal_power_load",
    "rittal_current_consumption",
    "rittal_variable_value",
    "rittal_temperature_celsius",
    "rittal_humidity_percent",
)
...
@pytest.mark.unit
def test_rittal_private_health_oids_are_not_guessed(metrics, policy, toml_text):
    names = {m["name"] for m in metrics["metrics"]}
    for absent in UNSUPPORTED_HEALTH_METRICS + UNSUPPORTED_CMC_METRICS:
...
def test_brand_label_present_in_common_without_generic_terms():
    ...
    assert "Rittal" in text
    assert "mm-rittal_rittal" in text
    assert "rittal|cmc\\s*iii" in text
    assert "pdu|environment|network.?service|cabinet|sensor" not in text


@pytest.mark.unit
def test_brand_svg_exists():
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-rittal_rittal.svg").exists()
...
def test_brand_files_have_no_external_source_residue():
    ...
    paths = list(BRAND_DIR.glob("*")) + [
        WEB_ROOT / "public" / "assets" / "icons" / "mm-rittal_rittal.svg"
    ]
    offenders = {
