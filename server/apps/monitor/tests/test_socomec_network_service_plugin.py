"""Contract tests for the Socomec iPDU/UPS NetworkService SNMP plugin.

Socomec MIB facts identify iPDU, EMD sensor, threshold alarm, UPS input,
output, bypass, battery, load and device connection tables. The currently
available evidence does not prove stable shared health scalars for CPU, memory,
fan, or power. The plugin therefore stays conservative: it uses the shared SNMP
floor only, keeps brand metrics as a zero-delta child, and does not promote UPS
or PDU business tables into device-health metrics.
"""
import json
...
BRAND_DIR = PLUGINS / "snmp" / "network_service_socomec"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "socomec"
COLLECT_TYPE = "snmp_socomec"
CONFIG_TYPE = "socomec"
INSTANCE_TYPE = "network_service"
PLUGIN_NAME = "NetworkService Socomec iPDU UPS SNMP"
OBJECT_NAME = "NetworkService"
PEN_ROOT = "1.3.6.1.4.1.4555"
...
UNSUPPORTED_SOCOMEC_METRICS = (
    "socomec_input_voltage",
    "socomec_input_current",
    "socomec_output_voltage",
    "socomec_output_current",
    "socomec_battery_state",
    "socomec_battery_voltage",
    "socomec_load_percent",
    "socomec_outlet_state",
    "socomec_temperature_celsius",
    "socomec_humidity_percent",
    "socomec_alarm_state",
)
...
@pytest.mark.unit
def test_socomec_private_health_oids_are_not_guessed(metrics, policy, toml_text):
    names = {m["name"] for m in metrics["metrics"]}
    for absent in UNSUPPORTED_HEALTH_METRICS + UNSUPPORTED_SOCOMEC_METRICS:
...
def test_brand_label_present_in_common_without_generic_terms():
    ...
    assert "Socomec" in text
    assert "mm-socomec_socomec" in text
    assert "socomec|net\\s*vision" in text
    assert "socomec|pdu|ups|network.?service" not in text


@pytest.mark.unit
def test_brand_svg_exists():
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-socomec_socomec.svg").exists()
...
def test_brand_files_have_no_external_source_residue():
    ...
    paths = list(BRAND_DIR.glob("*")) + [
        WEB_ROOT / "public" / "assets" / "icons" / "mm-socomec_socomec.svg"
    ]
    offenders = {
