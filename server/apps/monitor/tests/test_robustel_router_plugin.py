"""Contract tests for the Robustel router SNMP plugin.

Robustel industrial cellular routers currently have a verified baseline profile: standard MIB-II
uptime plus IF-MIB interface metrics, including 64-bit ifHC counters. No stable
vendor-specific scalar device-health gauges are confirmed, so metrics.json is a
zero-delta child. CPU, memory, temperature, fan and power are N/A and must not
be modelled here; cellular, WAN, VPN, SMS and trap-event tables are also not
promoted to shared device-health metrics without stable scalar OID proof.
"""
import json
...
PLUGIN_NAME = "Router Robustel SNMP"
OBJECT_NAME = "Router"
PRIVATE_PEN_ROOT = "1.3.6.1.4.1.39699"
...
def test_no_private_health_metrics_modelled(metrics):
    ...
    assert present == [], f"Robustel baseline plugin must not model private health metrics: {present}"
...
def test_brand_match_present_in_common():
    ...
    assert "mm-robustel_robustel" in text
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-robustel_robustel.svg").exists()
...
def test_new_files_do_not_leak_external_source_names():
    checked_paths = [
        ...
        Path(__file__),
        WEB_ROOT / "public" / "assets" / "icons" / "mm-robustel_robustel.svg",
    ]
