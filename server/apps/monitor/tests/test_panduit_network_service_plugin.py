"""Contract tests for the Panduit iPDU NetworkService SNMP plugin.

Panduit iPDU MIB facts identify G5 iPDU identity, input, outlet, temperature,
humidity, rack access, HID and alarm tables. The currently available evidence
does not prove stable shared health scalars for CPU, memory, fan, or power. The
plugin therefore stays conservative: it uses the shared SNMP floor only, keeps
brand metrics as a zero-delta child, and does not promote PDU business tables
into device-health metrics.
"""
import json
...
BRAND_DIR = PLUGINS / "snmp" / "network_service_panduit"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "panduit"
COLLECT_TYPE = "snmp_panduit"
CONFIG_TYPE = "panduit"
INSTANCE_TYPE = "network_service"
PLUGIN_NAME = "NetworkService Panduit iPDU SNMP"
OBJECT_NAME = "NetworkService"
PEN_ROOT = "1.3.6.1.4.1.19536"
...
UNSUPPORTED_PDU_METRICS = (
    "panduit_input_current",
    "panduit_input_voltage",
    "panduit_input_power",
    "panduit_outlet_state",
    "panduit_outlet_current",
    "panduit_outlet_power",
    "panduit_temperature_celsius",
    "panduit_humidity_percent",
    "panduit_rack_access_state",
    "panduit_alarm_state",
)
...
@pytest.mark.unit
def test_panduit_private_health_oids_are_not_guessed(metrics, policy, toml_text):
    names = {m["name"] for m in metrics["metrics"]}
    for absent in UNSUPPORTED_HEALTH_METRICS + UNSUPPORTED_PDU_METRICS:
...
def test_brand_label_present_in_common_without_generic_terms():
    ...
    assert "Panduit" in text
    assert "mm-panduit_panduit" in text
    assert "match: /panduit/i" in text
    assert "panduit|pdu|ipdu|network.?service" not in text


@pytest.mark.unit
def test_brand_svg_exists():
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-panduit_panduit.svg").exists()
...
def test_brand_files_have_no_external_source_residue():
    ...
    paths = list(BRAND_DIR.glob("*")) + [
        WEB_ROOT / "public" / "assets" / "icons" / "mm-panduit_panduit.svg"
    ]
    offenders = {
