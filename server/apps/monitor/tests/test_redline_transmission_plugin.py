"""Contract tests for the Redline transmission SNMP plugin.

Redline broadband wireless transmission equipment is represented by enterprise
PEN 10728. The current production input proves the public enterprise root and
private subtree names, but it does not prove stable shared health scalars for
CPU, memory, temperature, fan, or power. The plugin therefore stays
conservative: it uses the shared SNMP floor only, keeps brand metrics as a
zero-delta child, and does not promote private subtree counters into shared
...
BRAND_DIR = PLUGINS / "snmp" / "transmission_redline"
...
PLUGIN_NAME = "Transmission Redline SNMP"
OBJECT_NAME = "Transmission"
PEN_ROOT = "1.3.6.1.4.1.10728"
...
def test_brand_label_present_in_common():
    ...
    assert "mm-redline_redline" in text
    assert text.count("label: 'Redline'") == 1
    assert text.count("mm-redline_redline") == 1
    assert "redline|transmission" not in text.lower()


@pytest.mark.unit
def test_brand_svg_exists():
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-redline_redline.svg").exists()
...
def test_brand_files_have_no_external_source_residue():
    ...
    paths = list(BRAND_DIR.glob("*")) + [
        WEB_ROOT / "public" / "assets" / "icons" / "mm-redline_redline.svg"
    ]
    offenders = {
