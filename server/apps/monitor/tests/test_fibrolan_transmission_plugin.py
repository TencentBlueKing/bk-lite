"""Contract tests for the Fibrolan transmission SNMP plugin.

Fibrolan Falcon carrier Ethernet and IEEE 1588 devices are currently handled
as a conservative baseline child: metrics.json declares no vendor metric deltas,
...
BRAND_DIR = PLUGINS / "snmp" / "transmission_fibrolan"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "fibrolan"
COLLECT_TYPE = "snmp_fibrolan"
CONFIG_TYPE = "fibrolan"
INSTANCE_TYPE = "transmission"
PLUGIN_NAME = "Transmission Fibrolan SNMP"
...
def test_no_private_health_metrics_are_collected(toml_text):
    ...
    assert "1.3.6.1.4.1.4467" not in toml_text
...
def test_brand_match_and_icon_present_in_common():
    ...
    assert "mm-fibrolan_fibrolan" in text
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-fibrolan_fibrolan.svg").exists()
...
def test_new_files_do_not_leak_external_source_names():
    checked_paths = [
        ...
        Path(__file__),
        WEB_ROOT / "public" / "assets" / "icons" / "mm-fibrolan_fibrolan.svg",
    ]
