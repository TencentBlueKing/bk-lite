"""Contract tests for the TP-Link (JetStream/SMB) switch SNMP plugin.

Validates Telegraf/snmp_tplink/switch against the Cisco baseline and the
cross-vendor design decisions for the SNMP brand-plugin family.

TP-Link SMB switches expose no environment-sensor OIDs (no temperature, fan or
PSU in the TPLINK private MIB or the Zabbix template), so this is the most
reduced health profile: only per-core CPU (tpSysMonitorCpu5Minutes) and memory
utilization (tpSysMonitorMemoryUtilization, reported directly as a percentage,
Huawei shape). There are no temperature/fan/psu metrics, no enum processor, and
the policy carries only CPU and memory templates. Like every switch vendor it
still exposes the 64-bit ifHC interface traffic metrics.
...
BRAND = "tplink"
COLLECT_TYPE = "snmp_tplink"
CONFIG_TYPE = "tplink"
PLUGIN_NAME = "Switch TP-Link SNMP"
OBJECT_NAME = "Switch"
...
ABSENT_HEALTH = (
    ...
    "device_memory_used", "device_memory_free", "device_memory_total",
)
...
def metrics():
    return _read_json(TPLINK_DIR / "metrics.json")
...
@pytest.fixture(scope="module")
def policy():
    return _read_json(TPLINK_DIR / "policy.json")


@pytest.fixture(scope="module")
def ui():
    return _read_json(TPLINK_DIR / "UI.json")


@pytest.fixture(scope="module")
def toml_text():
    return (TPLINK_DIR / "tplink.child.toml.j2").read_text(encoding="utf-8")
...
# --------------------------------------------------------------------------- #
# reduced profile: only CPU + memory + interface traffic
# --------------------------------------------------------------------------- #
@pytest.mark.unit
def test_reduced_profile_absent_health_metrics(metrics):
    names = {m["name"] for m in metrics["metrics"]}
    present = [a for a in ABSENT_HEALTH if a in names]
    assert present == [], f"TP-Link has no env sensors; unexpected metrics: {present}"
