"""Contract tests for the APC NetworkService SNMP plugin.

APC PowerNet-MIB facts identify UPS, PDU, outlet, battery and environmental
tables under PEN 318. The currently available evidence does not prove stable
shared health scalars for CPU, memory, fan, or power across the supported APC
device families. The plugin therefore stays conservative: it uses the shared
SNMP floor only, keeps brand metrics as a zero-delta child, and does not promote
UPS or PDU business tables into generic device-health metrics.
"""
import json
...
BRAND_DIR = PLUGINS / "snmp" / "network_service_apc"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "apc"
COLLECT_TYPE = "snmp_apc"
CONFIG_TYPE = "apc"
INSTANCE_TYPE = "network_service"
PLUGIN_NAME = "NetworkService APC UPS PDU Environmental SNMP"
OBJECT_NAME = "NetworkService"
PEN_ROOT = "1.3.6.1.4.1.318"
...
UNSUPPORTED_APC_METRICS = (
    ...
    "apc_battery_voltage",
    "apc_outlet_state",
    "apc_temperature_celsius",
    "apc_humidity_percent",
    "apc_alarm_state",
)
...
@pytest.mark.unit
def test_apc_private_health_oids_are_not_guessed(metrics, policy, toml_text):
    names = {m["name"] for m in metrics["metrics"]}
    for absent in UNSUPPORTED_HEALTH_METRICS + UNSUPPORTED_APC_METRICS:
...
def test_brand_label_present_in_common_without_generic_terms():
    ...
    assert "APC" in text
    assert "mm-apc_apc" in text
    assert "apc|schneider\\s*electric|powernet" in text
    assert "apc|pdu|ups|network.?service" not in text


@pytest.mark.unit
def test_brand_svg_exists():
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-apc_apc.svg").exists()
...
def test_brand_files_have_no_external_source_residue():
    ...
    paths = list(BRAND_DIR.glob("*")) + [
        WEB_ROOT / "public" / "assets" / "icons" / "mm-apc_apc.svg"
    ]
    offenders = {
