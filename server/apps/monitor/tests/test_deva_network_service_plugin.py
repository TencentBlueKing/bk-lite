"""Contract tests for the DEVA Broadcast NetworkService SNMP plugin.

DEVA Broadcast receivers expose DEVA PEN 35833 and broadcast-service alarms
for RF, MPX, audio and pilot domains. The current MIB facts do not prove stable
shared health scalars for CPU, memory, temperature, fan or power. The plugin
therefore stays conservative: it uses the shared SNMP floor only, keeps brand
metrics as a zero-delta child, and does not promote broadcast-service alarms
into shared device-health metrics.
"""
import json
...
BRAND_DIR = PLUGINS / "snmp" / "network_service_deva"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "deva"
COLLECT_TYPE = "snmp_deva"
CONFIG_TYPE = "deva"
INSTANCE_TYPE = "network_service"
PLUGIN_NAME = "NetworkService DEVA Broadcast SNMP"
OBJECT_NAME = "NetworkService"
PEN_ROOT = "1.3.6.1.4.1.35833"
...
UNSUPPORTED_BROADCAST_METRICS = (
    "deva_rf_alarm_state",
    "deva_mpx_alarm_state",
    "deva_audio_alarm_state",
    "deva_pilot_alarm_state",
    "deva_rds_alarm_state",
)
...
@pytest.mark.unit
def test_deva_private_health_oids_are_not_guessed(metrics, policy, toml_text):
    names = {m["name"] for m in metrics["metrics"]}
    for absent in UNSUPPORTED_HEALTH_METRICS + UNSUPPORTED_BROADCAST_METRICS:
...
def test_brand_label_present_in_common_without_generic_terms():
    ...
    assert "DEVA Broadcast" in text
    assert "mm-deva_deva" in text
    deva_line = next(line for line in text.splitlines() if "mm-deva_deva" in line)
    assert "broadcast" in deva_line.lower()
    assert "rf" not in deva_line.lower()
    assert "audio" not in deva_line.lower()
    assert "network" not in deva_line.lower()


@pytest.mark.unit
def test_brand_svg_exists():
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-deva_deva.svg").exists()
...
def test_brand_files_have_no_external_source_residue():
    ...
    paths = list(BRAND_DIR.glob("*")) + [
        WEB_ROOT / "public" / "assets" / "icons" / "mm-deva_deva.svg"
    ]
    offenders = {
