"""Contract tests for the pfSense firewall SNMP plugin.

Validated against the generic **firewall** baseline (Telegraf/snmp/firewall),
NOT the Cisco switch baseline. pfSense must be a strict superset of the
firewall baseline (every baseline interface/uptime metric present with
byte-identical metric_group/unit/query) plus a thin pfSense-specific set:

  - device_cpu_usage   = avg(...) by (instance_id)  (percent; per-core
                         HOST-RESOURCES hrProcessorLoad aggregated headline)
  - firewall_pf_states (short, BEGEMOT-PF-MIB pfStateTableCount ~ active conns)
  - interface_ifHCInOctets / ifHCOutOctets          (64-bit HC traffic)

Like watchguard/paloalto, pfSense has no standard SNMP memory-utilization
scalar (bsnmpd memory is hrStorage row-level) and no sensor tables, so
memory / temperature / fan / PSU are N/A and must not be modelled. The shared
firewall dashboard's session/connection `or`-chain includes firewall_pf_states
so pfSense surfaces in the "active sessions/connections" card. The collectType
must wire into the firewall object, not the switch.
...
PF_DIR = PLUGINS / "snmp_pfsense" / "firewall"
BASE_DIR = PLUGINS / "snmp" / "firewall"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"
FIREWALL_DASHBOARD_CONFIG = (
    WEB_ROOT / "src" / "app" / "monitor" / "dashboards" / "objects" / "firewall" / "config.ts"
)

BRAND = "pfsense"
COLLECT_TYPE = "snmp_pfsense"
CONFIG_TYPE = "pfsense"
INSTANCE_TYPE = "firewall"
PLUGIN_NAME = "Firewall pfSense SNMP"
...
PF_HEALTH = ("device_cpu_usage", "firewall_pf_states")
...
def metrics():
    return _read_json(PF_DIR / "metrics.json")
...
@pytest.fixture(scope="module")
def policy():
    return _read_json(PF_DIR / "policy.json")


@pytest.fixture(scope="module")
def ui():
    return _read_json(PF_DIR / "UI.json")


@pytest.fixture(scope="module")
def toml_text():
    return (PF_DIR / "pfsense.child.toml.j2").read_text(encoding="utf-8")
...
def test_plugin_lives_under_firewall_dir():
    assert PF_DIR.parent.name == COLLECT_TYPE
    assert PF_DIR.name == INSTANCE_TYPE


@pytest.mark.unit
def test_toml_filename_follows_convention():
    assert (PF_DIR / f"{CONFIG_TYPE}.child.toml.j2").exists()
...
def test_is_superset_of_firewall_baseline(metrics, base_metrics):
    pf = {m["name"] for m in metrics["metrics"]}
    base = {m["name"] for m in base_metrics["metrics"]}
    missing = base - pf
    assert missing == set(), f"pfsense dropped baseline firewall metrics: {missing}"
...
# --------------------------------------------------------------------------- #
# thin pfSense profile: CPU% aggregate + pf state table count
# --------------------------------------------------------------------------- #
@pytest.mark.unit
def test_pfsense_health_metrics_present(metrics):
    names = {m["name"] for m in metrics["metrics"]}
    missing = [m for m in PF_HEALTH if m not in names]
    assert missing == [], f"pfsense health metrics missing: {missing}"
...
@pytest.mark.unit
def test_pf_states_is_short(metrics):
    pf = {m["name"]: m for m in metrics["metrics"]}["firewall_pf_states"]
    assert pf["unit"] == "short"
...
# --------------------------------------------------------------------------- #
# frontend wiring: collectType + shared dashboard or-chain inclusion
# --------------------------------------------------------------------------- #
@pytest.mark.unit
...
def test_frontend_collecttype_not_wired_to_switch_object():
    ...
    assert COLLECT_TYPE not in text, "pfsense firewall must not be wired into the switch object"


@pytest.mark.unit
def test_pf_states_in_shared_dashboard_session_chain():
    """The shared firewall dashboard's session/connection or-chain must include
    firewall_pf_states so pfSense surfaces in the active-sessions card."""
    text = FIREWALL_DASHBOARD_CONFIG.read_text(encoding="utf-8")
    assert "firewall_pf_states" in text, \
        "firewall dashboard session or-chain must include firewall_pf_states"
