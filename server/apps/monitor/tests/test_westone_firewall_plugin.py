"""Contract tests for the Westone firewall SNMP plugin.

Westone (卫士通, IANA PEN 31648) exposes two scalar health OIDs only:
  - CPU  snAgGblCpuUtil1SecAvg 1.3.6.1.4.1.31648.3.15.1.0  (direct %)
  - mem  snAgGblDynMemUtil     1.3.6.1.4.1.31648.3.15.5.0  (direct %)
...
The plugin dir is differentiated as firewall_westone (separate from DPtech which
shares PEN 31648 but a different subtree). Reuses the shared Firewall metric
names + groups; i18n and the shared firewall dashboard are already in place.
"""
import json
...
BRAND_DIR = PLUGINS / "snmp" / "firewall_westone"
BASE_DIR = PLUGINS / "snmp" / "firewall_hillstone"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "westone"
COLLECT_TYPE = "snmp_westone"
CONFIG_TYPE = "westone"
INSTANCE_TYPE = "firewall"
PLUGIN_NAME = "Firewall Westone SNMP"
OBJECT_NAME = "Firewall"
PEN_ROOT = "1.3.6.1.4.1.31648"
...
def test_no_used_free_temp_fan_psu(metrics):
    ...
    assert present == [], f"Westone reports mem as %; must not model: {present}"
...
# OID hygiene — scalar health OIDs must carry the .0 instance suffix
@pytest.mark.unit
def test_private_oids_under_pen_31648(toml_text):
    assert "1.3.6.1.4.1.31648.3.15.1.0" in toml_text  # CPU scalar
    assert "1.3.6.1.4.1.31648.3.15.5.0" in toml_text  # mem usage scalar
...
def test_frontend_brand_match_registered():
    ...
    assert "westone" in common.read_text(encoding="utf-8").lower()


@pytest.mark.unit
def test_brand_icon_asset_present():
    icon = WEB_ROOT / "public" / "assets" / "icons" / "mm-westone_westone.svg"
    assert icon.exists()
