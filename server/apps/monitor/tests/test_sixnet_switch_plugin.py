"""Contract tests for the Sixnet switch SNMP plugin.

Sixnet SLX (IANA PEN 20540) industrial Ethernet switches are fanless,
passively cooled units whose temperature, dual redundant power inputs and
alarm relay are exposed mainly over Modbus/TCP and LED/alarm-relay rather
than as row-filter-free private SNMP scalars. So this is a BASELINE plugin:
it collects only standard MIB-II uptime plus IF-MIB interface metrics
(including the 64-bit ifHC counters) and device-aggregated traffic totals.
CPU, memory, temperature, fan and power are all N/A and must NOT be modelled,
so there must be NO processors.enum block and NO private PEN-20540 OID.
...
PLUGIN_DIR = PLUGINS / "snmp" / "switch_sixnet"
...
PLUGIN_NAME = "Switch Sixnet SNMP"
OBJECT_NAME = "Switch"
PRIVATE_PEN_ROOT = "1.3.6.1.4.1.20540"
...
def test_no_private_health_metrics_modelled(metrics):
    ...
    assert present == [], \
        f"Sixnet baseline plugin must not model private health metrics: {present}"
...
def test_no_private_pen_oid_used(toml_text):
    assert PRIVATE_PEN_ROOT not in toml_text, \
        "Sixnet private PEN 20540 health MIB is not collected in the baseline template"
