"""Contract tests for the Server Technology Sentry3 NetworkService SNMP plugin.

Sentry3 devices expose remote power, PDU and environmental monitor tables, but
the currently available MIB facts do not prove stable shared health scalars for
CPU, memory, fan, or power. The plugin therefore stays conservative: it uses
the shared SNMP floor only, keeps brand metrics as a zero-delta child, and does
not promote PDU or environmental business tables into device-health metrics.
"""
import json
...
BRAND_DIR = PLUGINS / "snmp" / "network_service_servertech"
...
PLUGIN_NAME = "NetworkService Server Technology Sentry3 SNMP"
OBJECT_NAME = "NetworkService"
PEN_ROOT = "1.3.6.1.4.1.1718.3"
...
UNSUPPORTED_PDU_METRICS = (
    "servertech_outlet_state",
    "servertech_infeed_load",
    "servertech_branch_power",
    "servertech_temperature_celsius",
    "servertech_humidity_percent",
    "servertech_contact_closure_state",
)
...
@pytest.mark.unit
def test_servertech_private_health_oids_are_not_guessed(metrics, policy, toml_text):
...
def test_brand_label_present_in_common_without_generic_terms():
    ...
    assert "Server Technology" in text
    assert "mm-servertech_servertech" in text
    assert "server\\s*technology|servertech|sentry3" in text
    assert "pdu|environmental|network.?service" not in text


@pytest.mark.unit
def test_brand_svg_exists():
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-servertech_servertech.svg").exists()
...
def test_brand_files_have_no_external_source_residue():
    ...
    paths = list(BRAND_DIR.glob("*")) + [
        WEB_ROOT / "public" / "assets" / "icons" / "mm-servertech_servertech.svg"
    ]
    offenders = {
