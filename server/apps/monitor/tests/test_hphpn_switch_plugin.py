"""Contract tests for the HP ProCurve / HP Networking switch SNMP plugin.

Validates Telegraf/snmp_hphpn/switch against the Cisco baseline and the
cross-vendor design decisions for the SNMP brand-plugin family.

HP ProCurve / HP Networking switches (HP-ICF MIBs) expose a CPU utilization
scalar (hpSwitchCpuStat) and a system-memory pool as used + total bytes
(hpLocalMem table), so memory utilization is computed used/total*100
(branch3, identical to mikrotik/qtech). Temperature, fan and PSU state live in
HP's shared ENTITY-SENSOR / hpicfSensorTable that needs per-row type/class
filtering, which telegraf inputs.snmp cannot express — so this is a
CPU + Memory profile with NO temperature / fan / psu metric and NO enum
processor. The policy carries only CPU and memory templates. Like every switch
vendor it still exposes the 64-bit ifHC interface metrics.
...
HPHPN_DIR = PLUGINS / "snmp_hphpn" / "switch"
...
PLUGIN_NAME = "Switch HP ProCurve SNMP"
OBJECT_NAME = "Switch"
...
def metrics():
    return _read_json(HPHPN_DIR / "metrics.json")
...
@pytest.fixture(scope="module")
def policy():
    return _read_json(HPHPN_DIR / "policy.json")


@pytest.fixture(scope="module")
def ui():
    return _read_json(HPHPN_DIR / "UI.json")


@pytest.fixture(scope="module")
def toml_text():
    return (HPHPN_DIR / "hphpn.child.toml.j2").read_text(encoding="utf-8")
...
def test_plugin_lives_under_correct_dir():
    assert HPHPN_DIR.parent.name == COLLECT_TYPE
    assert HPHPN_DIR.name == INSTANCE_TYPE


@pytest.mark.unit
def test_toml_filename_follows_convention():
    assert (HPHPN_DIR / f"{CONFIG_TYPE}.child.toml.j2").exists()
...
# --------------------------------------------------------------------------- #
# reduced profile: CPU + Memory(used/total/usage) + interface, no temp/fan/psu
# --------------------------------------------------------------------------- #
@pytest.mark.unit
...
# --------------------------------------------------------------------------- #
# memory model: used/total*100 (branch3, byte-identical to mikrotik)
# --------------------------------------------------------------------------- #
@pytest.mark.unit
...
# --------------------------------------------------------------------------- #
# frontend wiring: collectType maps to the switch object, not another type
# --------------------------------------------------------------------------- #
@pytest.mark.unit
def test_frontend_collecttype_wired_to_switch_object():
    switch_tsx = (
        SERVER_ROOT.parents[0]
        / "web" / "src" / "app" / "monitor" / "hooks" / "integration"
...
def test_memory_model_needs_no_config_ts_change():
    ...
    config_ts = (
        SERVER_ROOT.parents[0]
        / "web" / "src" / "app" / "monitor" / "dashboards"
