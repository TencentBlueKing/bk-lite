"""Contract tests for the Sangoma Vega voice gateway SNMP plugin.

Sangoma Vega devices are represented conservatively in this pass: the public
production facts prove the Vega voice-gateway object family and enterprise MIB
subtree, but do not provide stable shared health leaf OIDs for CPU, memory,
temperature, fan, or power. The plugin therefore uses the shared VoiceGateway
SNMP floor only and keeps brand metrics as a zero-delta child.
"""
import json
...
BRAND_DIR = PLUGINS / "snmp" / "voice_gateway_sangoma"
...
PLUGIN_NAME = "VoiceGateway Sangoma Vega SNMP"
...
def test_private_health_oids_are_not_guessed(metrics, policy, toml_text):
    ...
    assert "1.3.6.1.4.1.4686" not in toml_text
    assert "vsplatform" not in toml_text.lower()
...
def test_brand_label_present_in_common():
    ...
    assert "Sangoma Vega" in text
    assert "mm-sangoma_sangoma" in text
    assert "{ match: /sangoma|vega/i, label: 'Sangoma Vega'" in text
    assert "voice|sangoma" not in text.lower()
    assert "gateway|sangoma" not in text.lower()


@pytest.mark.unit
def test_brand_svg_exists():
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-sangoma_sangoma.svg").exists()
...
def test_brand_files_have_no_external_source_residue():
    ...
    paths = list(BRAND_DIR.glob("*")) + [
        WEB_ROOT / "public" / "assets" / "icons" / "mm-sangoma_sangoma.svg"
    ]
    offenders = {
