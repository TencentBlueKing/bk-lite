"""Contract tests for the Infinera transmission SNMP plugin.

The current production input confirms Infinera/Coriant optical transport MIB
availability, but does not prove stable shared device-health scalars for CPU,
memory, temperature, fan, or power. This plugin therefore stays conservative:
it keeps a zero-delta child and relies on the shared Transmission SNMP floor
for uptime and 64-bit IF-MIB traffic.
"""
import json
...
BRAND_DIR = PLUGINS / "snmp" / "transmission_infinera"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "infinera"
COLLECT_TYPE = "snmp_infinera"
CONFIG_TYPE = "infinera"
INSTANCE_TYPE = "transmission"
PLUGIN_NAME = "Transmission Infinera SNMP"
...
def test_brand_label_present_in_common():
    ...
    assert "Infinera" in text
    assert "mm-infinera_infinera" in text


@pytest.mark.unit
def test_brand_svg_exists():
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-infinera_infinera.svg").exists()
...
def test_brand_files_have_no_external_source_residue():
    ...
    paths = list(BRAND_DIR.glob("*")) + [
        WEB_ROOT / "public" / "assets" / "icons" / "mm-infinera_infinera.svg"
    ]
    offenders = {
