"""Contract tests for the Mitel voice gateway SNMP plugin.

Mitel PBX and voice gateway devices are represented conservatively here: the
current production input proves the enterprise root and call server MIB family,
but not stable shared health scalars for CPU, memory, temperature, fan, or
...
BRAND_DIR = PLUGINS / "snmp" / "voice_gateway_mitel"
...
PLUGIN_NAME = "VoiceGateway Mitel SNMP"
...
def test_private_health_oids_are_not_guessed(metrics, policy, toml_text):
    ...
    assert "1.3.6.1.4.1.1027" not in toml_text
...
def test_brand_label_present_in_common():
    ...
    assert "Mitel" in text
    assert "mm-mitel_mitel" in text
    assert "mivoice" in text.lower()


@pytest.mark.unit
def test_brand_svg_exists():
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-mitel_mitel.svg").exists()
...
def test_brand_files_have_no_external_source_residue():
    ...
    paths = list(BRAND_DIR.glob("*")) + [
        WEB_ROOT / "public" / "assets" / "icons" / "mm-mitel_mitel.svg"
    ]
    offenders = {
