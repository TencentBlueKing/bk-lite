"""Contract tests for the Avici router SNMP plugin.

Avici Systems TSR/SSR core routers (IANA PEN 2474, EOL legacy hardware) have no
public row-filter-free scalar health gauges in the private MIB. So this is a
BASELINE plugin: standard MIB-II uptime plus IF-MIB interface metrics (incl.
64-bit ifHC) and device-aggregated traffic totals. CPU/memory/temperature/fan/
power are N/A and must NOT be modelled — no processors.enum, no private
PEN-2474 OID.
...
PLUGIN_DIR = PLUGINS / "snmp" / "router_avici"
...
PLUGIN_NAME = "Router Avici SNMP"
OBJECT_NAME = "Router"
PRIVATE_PEN_ROOT = "1.3.6.1.4.1.2474"
...
def test_no_private_health_metrics_modelled(metrics):
    ...
    assert present == [], \
        f"Avici baseline plugin must not model private health metrics: {present}"
...
def test_no_private_pen_oid_used(toml_text):
    assert PRIVATE_PEN_ROOT not in toml_text, \
        "Avici private PEN 2474 health MIB is not collected in the baseline template"
