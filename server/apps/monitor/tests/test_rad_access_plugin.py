"""Contract tests for the RAD Access SNMP plugin.

RAD public SNMP facts confirm PEN 164 and the access, CPE, industrial-router and broadband-wireless identity, but no stable row-filter-free private CPU, memory, temperature, fan
...
BRAND_DIR = PLUGINS / "snmp" / "access_rad"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "rad"
COLLECT_TYPE = "snmp_rad"
CONFIG_TYPE = "rad"
INSTANCE_TYPE = "access"
PLUGIN_NAME = "Access RAD SNMP"
OBJECT_NAME = "Access"
PEN_ROOT = "1.3.6.1.4.1.164"
...
def test_private_health_oids_are_not_collected_without_leaf_evidence(toml_text):
    ...
    forbidden_terms = ("fan", "power", "temperature", "cpu", "memory")
...
def test_brand_match_and_icon_present_in_common():
    ...
    assert "rad" in text
    assert "rad\\s*data" in text
    assert "radcomms" in text
    assert "access|broadband|storage|router|cpe" not in text.lower()
    assert "mm-rad_rad" in text
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-rad_rad.svg").exists()
...
def test_new_files_do_not_leak_external_source_names():
    checked_paths = [
        ...
        Path(__file__),
        WEB_ROOT / "public" / "assets" / "icons" / "mm-rad_rad.svg",
    ]
