"""Contract tests for the Ekinops transmission SNMP plugin.

Ekinops optical transport equipment is represented by enterprise PEN 20044, but
...
zero-delta child, and does not promote optical transport counters into shared
...
BRAND_DIR = PLUGINS / "snmp" / "transmission_ekinops"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "ekinops"
COLLECT_TYPE = "snmp_ekinops"
CONFIG_TYPE = "ekinops"
INSTANCE_TYPE = "transmission"
PLUGIN_NAME = "Transmission Ekinops SNMP"
OBJECT_NAME = "Transmission"
PEN_ROOT = "1.3.6.1.4.1.20044"
...
def test_brand_label_present_in_common():
    ...
    assert "Ekinops" in text
    assert "mm-ekinops_ekinops" in text


@pytest.mark.unit
def test_brand_svg_exists():
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-ekinops_ekinops.svg").exists()
...
def test_brand_files_have_no_external_source_residue():
    ...
    paths = list(BRAND_DIR.glob("*")) + [
        WEB_ROOT / "public" / "assets" / "icons" / "mm-ekinops_ekinops.svg"
    ]
    offenders = {
