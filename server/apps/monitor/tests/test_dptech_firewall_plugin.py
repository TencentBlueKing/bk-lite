"""Contract tests for the DPtech firewall SNMP plugin.

DPtech (迪普科技, IANA PEN 31648) exposes two scalar health OIDs only:
...
The plugin dir is firewall_dptech. DPtech (迪普) is the actual PEN 31648 owner;
the Westone OEM build reuses the same subtree. Reuses the shared Firewall metric
...
BRAND_DIR = PLUGINS / "snmp" / "firewall_dptech"
BASE_DIR = PLUGINS / "snmp" / "firewall_hillstone"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "dptech"
COLLECT_TYPE = "snmp_dptech"
CONFIG_TYPE = "dptech"
INSTANCE_TYPE = "firewall"
PLUGIN_NAME = "Firewall DPtech SNMP"
...
def test_no_used_free_temp_fan_psu(metrics):
    ...
    assert present == [], f"DPtech reports mem as %; must not model: {present}"
...
def test_frontend_brand_match_registered():
    ...
    assert "dptech" in common.read_text(encoding="utf-8").lower()


@pytest.mark.unit
def test_brand_icon_asset_present():
    icon = WEB_ROOT / "public" / "assets" / "icons" / "mm-dptech_dptech.svg"
    assert icon.exists()
