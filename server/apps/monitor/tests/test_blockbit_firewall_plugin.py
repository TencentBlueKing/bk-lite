"""Contract tests for the Blockbit firewall SNMP plugin.

Validates Telegraf/snmp/firewall_blockbit against the cross-vendor design
decisions for the SNMP brand-plugin family.

Blockbit UTM / NGFW security appliances run a hardened Unix-like OS whose
CPU/memory/disk are exposed only via row-indexed standard host-resource tables
that telegraf inputs.snmp cannot filter row-wise, and no vendor private health
scalar is publicly documented. So this is a BASELINE plugin: it collects only
standard MIB-II uptime plus IF-MIB interface metrics (including the 64-bit ifHC
counters) and the device-aggregated traffic totals. CPU, memory, temperature,
fan and power are all N/A and must NOT be modelled, so there must be NO
processors.enum block.
...
Blockbit reuses the shared Firewall object name and dashboard, with a brand
match + logo in common.tsx and the firewall.tsx collect-type wire.
...
KORENIX_DIR = PLUGINS / "snmp" / "firewall_blockbit"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "blockbit"
COLLECT_TYPE = "snmp_blockbit"
CONFIG_TYPE = "blockbit"
INSTANCE_TYPE = "firewall"
PLUGIN_NAME = "Firewall Blockbit SNMP"
OBJECT_NAME = "Firewall"
PRIVATE_ENTERPRISE_ROOT = "1.3.6.1.4.1."
...
@pytest.fixture(scope="module")
def toml_text():
    return (KORENIX_DIR / "blockbit.child.toml.j2").read_text(encoding="utf-8")
...
def test_no_private_health_metrics_modelled(metrics):
    ...
    assert present == [], \
        f"Blockbit baseline plugin must not model private health metrics: {present}"


@pytest.mark.unit
def test_no_enum_processor_block(toml_text):
    assert "[[processors.enum]]" not in toml_text, \
        "Blockbit has no fan/psu enum to normalize; no processors.enum expected"


@pytest.mark.unit
def test_no_private_enterprise_oid_used(toml_text):
    assert PRIVATE_ENTERPRISE_ROOT not in toml_text, \
        "Blockbit baseline template must collect only standard MIB-II/IF-MIB OIDs (no 1.3.6.1.4.1.*)"
...
def test_policy_covers_interface_error_and_discard_rates(policy):
    ...
    }, "Blockbit baseline policy must cover interface error/discard rates only"
...
# --------------------------------------------------------------------------- #
# frontend wiring: the firewall.tsx collect-type wire + common.tsx brand match
# --------------------------------------------------------------------------- #
@pytest.mark.unit
