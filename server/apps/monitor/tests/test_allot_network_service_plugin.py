"""Contract tests for the Allot NetworkService SNMP plugin.

Allot enterprise service gateway materials and public enterprise OID identity
confirm SNMP capability under PEN 2603. The available evidence does not prove
stable shared health scalars for CPU, memory, temperature, fan, or power. The
plugin therefore stays conservative: it uses the shared SNMP floor only, keeps
brand metrics as a zero-delta child, and does not promote service, traffic or
policy tables into device-health metrics.
"""
import json
...
BRAND_DIR = PLUGINS / "snmp" / "network_service_allot"
...
PLUGIN_NAME = "NetworkService Allot SNMP"
OBJECT_NAME = "NetworkService"
PEN_ROOT = "1.3.6.1.4.1.2603"
...
UNSUPPORTED_ALLOT_METRICS = (
    "allot_service_state",
    "allot_policy_state",
    "allot_throughput_bps",
    "allot_sessions",
    "allot_subscribers",
    "allot_http_transactions",
    "allot_cpu_usage",
    "allot_memory_usage",
    "allot_packet_drops",
)
...
@pytest.mark.unit
def test_allot_private_health_oids_are_not_guessed(metrics, policy, toml_text):
    names = {m["name"] for m in metrics["metrics"]}
    for absent in UNSUPPORTED_HEALTH_METRICS + UNSUPPORTED_ALLOT_METRICS:
...
def test_brand_label_present_in_common_without_generic_terms():
    ...
    assert "Allot" in text
    assert "mm-allot_allot" in text
    assert r"allot|netxplorer|netexplorer" in text
    assert "allot|gateway|network.?service" not in text


@pytest.mark.unit
def test_brand_svg_exists():
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-allot_allot.svg").exists()
...
def test_brand_files_have_no_external_source_residue():
    ...
    paths = list(BRAND_DIR.glob("*")) + [
        WEB_ROOT / "public" / "assets" / "icons" / "mm-allot_allot.svg"
    ]
    offenders = {
