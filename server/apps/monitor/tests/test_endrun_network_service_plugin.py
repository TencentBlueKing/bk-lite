"""Contract tests for the EndRun NetworkService SNMP plugin.

EndRun time servers expose NTP, PTP, GPS and alarm objects under PEN 13827, but
the current MIB facts do not prove stable shared health scalars for CPU,
memory, temperature, fan, or power. The plugin therefore stays conservative:
it uses the shared SNMP floor only, keeps brand metrics as a zero-delta child,
and does not promote time-service indicators into device-health metrics.
"""
import json
...
BRAND_DIR = PLUGINS / "snmp" / "network_service_endrun"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "endrun"
COLLECT_TYPE = "snmp_endrun"
CONFIG_TYPE = "endrun"
INSTANCE_TYPE = "network_service"
PLUGIN_NAME = "NetworkService EndRun SNMP"
OBJECT_NAME = "NetworkService"
PEN_ROOT = "1.3.6.1.4.1.13827"
...
UNSUPPORTED_SERVICE_METRICS = (
    "endrun_ntp_peer_state",
    "endrun_ptp_sync_state",
    "endrun_gps_lock_state",
    "endrun_alarm_state",
)
...
@pytest.mark.unit
def test_endrun_private_health_oids_are_not_guessed(metrics, policy, toml_text):
    names = {m["name"] for m in metrics["metrics"]}
    for absent in UNSUPPORTED_HEALTH_METRICS + UNSUPPORTED_SERVICE_METRICS:
...
def test_brand_label_present_in_common_without_generic_terms():
    ...
    assert "EndRun" in text
    assert "mm-endrun_endrun" in text
    endrun_line = next(line for line in text.splitlines() if "mm-endrun_endrun" in line)
    assert "ntp" not in endrun_line.lower()
    assert "ptp" not in endrun_line.lower()
    assert "gps" not in endrun_line.lower()
    assert "time" not in endrun_line.lower()
    assert "server" not in endrun_line.lower()


@pytest.mark.unit
def test_brand_svg_exists():
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-endrun_endrun.svg").exists()
...
def test_brand_files_have_no_external_source_residue():
    ...
    paths = list(BRAND_DIR.glob("*")) + [
        WEB_ROOT / "public" / "assets" / "icons" / "mm-endrun_endrun.svg"
    ]
    offenders = {
