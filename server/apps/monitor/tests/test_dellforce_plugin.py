"""Contract tests for the Dell Force10 (S-Series) switch SNMP plugin.

Validates Telegraf/snmp_dellforce/switch against the Cisco baseline and the
cross-vendor design decisions for the SNMP brand-plugin family.

Dell Force10 S-Series (F10-S-SERIES-CHASSIS-MIB, enterprise 6027) reports CPU
and memory as utilization percentages directly (Huawei shape), so
device_memory_usage is a raw single series and the shared brand-adaptive
dashboard hits its raw branch with zero config change. Temperature/fan/psu
expose no descr name column, so those metrics declare no descr dimension and
their alert templates carry no ${metric_descr}. chSysFanTrayOperStatus /
chSysPowerSupplyOperStatus are natively 1=up=normal, but both are still
normalized to the unified 1=normal convention via a brand-scoped enum processor
(1 -> 1, everything else -> 2) so the policy >1 is robust regardless of the raw
encoding. Like every switch vendor it also exposes the 64-bit ifHC interface
metrics.
...
PLUGIN_NAME = "Switch Dell Force10 SNMP"
OBJECT_NAME = "Switch"
...
def metrics():
    return _read_json(DELL_DIR / "metrics.json")
...
@pytest.fixture(scope="module")
def policy():
    return _read_json(DELL_DIR / "policy.json")


@pytest.fixture(scope="module")
def ui():
    return _read_json(DELL_DIR / "UI.json")


@pytest.fixture(scope="module")
def toml_text():
    return (DELL_DIR / "dellforce.child.toml.j2").read_text(encoding="utf-8")
