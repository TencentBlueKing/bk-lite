"""Contract tests for the FiberHome OLT Access SNMP plugin.

The Access plugin is intentionally conservative. It keeps only the shared
SNMP baseline in TOML and declares no vendor delta metrics until stable
FiberHome OLT device-level health OIDs are confirmed.
"""
import json
...
BRAND_DIR = PLUGINS / "snmp" / "access_fiberhome_olt"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "fiberhome"
COLLECT_TYPE = "snmp_fiberhome_olt"
CONFIG_TYPE = "fiberhome_olt"
INSTANCE_TYPE = "access"
PLUGIN_NAME = "Access FiberHome OLT SNMP"
OBJECT_NAME = "Access"
...
@pytest.mark.unit
def test_vendor_health_oids_are_not_guessed(metrics, toml_text):
    names = {metric["name"] for metric in metrics["metrics"]}
    assert names & UNSUPPORTED_METRICS == set()
    assert "FIBERHOME-OLT-COMMON-MIB" not in toml_text
    assert "access_pon" not in toml_text
    assert "access_onu" not in toml_text
    assert "optical" not in toml_text.lower()
...
def test_brand_match_and_icon_present_in_common():
    ...
    assert "fiberhome" in text
    assert "mm-fiberhome_fiberhome" in text
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-fiberhome_fiberhome.svg").exists()
...
def test_new_files_do_not_leak_external_source_names():
    checked_paths = [
        ...
        Path(__file__),
        WEB_ROOT / "public" / "assets" / "icons" / "mm-fiberhome_fiberhome.svg",
    ]
