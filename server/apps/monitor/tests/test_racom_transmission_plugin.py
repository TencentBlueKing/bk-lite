"""Contract tests for the RACOM transmission SNMP plugin.

RACOM RAy microwave backhaul devices are currently handled as a conservative
baseline child: metrics.json declares no vendor metric deltas, while the shared
Transmission SNMP floor supplies uptime and IF-MIB/ifXTable traffic.
"""
import json
...
BRAND_DIR = PLUGINS / "snmp" / "transmission_racom"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "racom"
COLLECT_TYPE = "snmp_racom"
CONFIG_TYPE = "racom"
INSTANCE_TYPE = "transmission"
PLUGIN_NAME = "Transmission RACOM SNMP"
...
def test_no_private_health_metrics_are_collected(toml_text):
    ...
    assert "1.3.6.1.4.1.33555" not in toml_text
...
def test_brand_match_and_icon_present_in_common():
    ...
    lower = text.lower()
    assert BRAND in lower
    assert "mm-racom_racom" in text
    assert (WEB_ROOT / "public" / "assets" / "icons" / "mm-racom_racom.svg").exists()
...
def test_new_files_do_not_leak_external_source_names():
    checked_paths = [
        ...
        Path(__file__),
        WEB_ROOT / "public" / "assets" / "icons" / "mm-racom_racom.svg",
    ]
