"""Contract tests for the MultiTech router SNMP plugin.

Validates Telegraf/snmp_multitech/router against the cross-vendor design
decisions for the SNMP brand-plugin family.

MultiTech Systems MultiConnect rCell (IANA PEN 995) cellular routers / IoT
gateways run an RFC1213-compatible SNMP agent (standard MIB-II + IF-MIB) and do
not expose pollable scalar device-health gauges for CPU/memory/temperature/fan/
power across the product line. The
...
MultiTech reuses the shared Router object name and dashboard but has a
brand-specific common.tsx match and icon, plus the router.tsx collect-type wire.
...
MULTITECH_DIR = PLUGINS / "snmp" / "router_multitech"
LANGUAGE_DIR = SERVER_ROOT / "apps" / "monitor" / "language"
WEB_ROOT = SERVER_ROOT.parents[0] / "web"

BRAND = "multitech"
COLLECT_TYPE = "snmp_multitech"
CONFIG_TYPE = "multitech"
INSTANCE_TYPE = "router"
PLUGIN_NAME = "Router MultiTech SNMP"
OBJECT_NAME = "Router"
MULTITECH_PEN_ROOT = "1.3.6.1.4.1.995"
...
def metrics():
    return _read_json(MULTITECH_DIR / "metrics.json")


@pytest.fixture(scope="module")
def policy():
    return _read_json(MULTITECH_DIR / "policy.json")


@pytest.fixture(scope="module")
def ui():
    return _read_json(MULTITECH_DIR / "UI.json")


@pytest.fixture(scope="module")
def toml_text():
    return (MULTITECH_DIR / "multitech.child.toml.j2").read_text(encoding="utf-8")
...
def test_plugin_lives_under_correct_dir(metrics):
    ...
    assert MULTITECH_DIR.parent.name == "snmp"  # 扁平布局


@pytest.mark.unit
def test_toml_filename_follows_convention():
    assert (MULTITECH_DIR / f"{CONFIG_TYPE}.child.toml.j2").exists()
...
def test_no_private_health_metrics_modelled(metrics):
    ...
    assert present == [], \
        f"MultiTech baseline plugin must not model private health metrics: {present}"


@pytest.mark.unit
def test_no_enum_processor_block(toml_text):
    assert "[[processors.enum]]" not in toml_text, \
        "MultiTech has no fan/psu enum to normalize; no processors.enum expected"


@pytest.mark.unit
def test_no_private_pen_oid_used(toml_text):
    assert MULTITECH_PEN_ROOT not in toml_text, \
        "MultiTech private PEN 995 tree is not collected in the baseline template"
...
def test_policy_covers_interface_error_and_discard_rates(policy):
    ...
    }, "MultiTech baseline policy must cover interface error/discard rates only"
