...
from apps.monitor.management.utils import find_files_by_pattern, extract_plugin_path_info, parse_template_filename
...
def _process_config_templates(plugin_dir, plugin_obj, db_templates):
    ...
    # 找出需要删除的模板
    to_delete = [db_template for template_key, db_template in db_templates.items()
...
def _collect_templates_to_process(path_list, plugins_dict, all_config_templates, all_ui_templates):
    ...
    return {
        'config': (config_templates_to_create, config_templates_to_update, config_templates_to_delete),
...
def _batch_save_templates(templates_data):
    ...
    stats = {
        'config_create': 0, 'config_update': 0, 'config_delete': 0,
        'ui_create': 0, 'ui_update': 0, 'ui_delete': 0
    }
    ...
    with transaction.atomic():
        ...
        if config_update:
            MonitorPluginConfigTemplate.objects.bulk_update(
                config_update, ['content'], batch_size=DatabaseConstants.MONITOR_OBJECT_BATCH_SIZE
            )
        ...
        if ui_update:
            MonitorPluginUITemplate.objects.bulk_update(
                ui_update, ['content'], batch_size=DatabaseConstants.MONITOR_OBJECT_BATCH_SIZE
            )
