...
class UnitConverter:
    ...
    def suggest_unit(cls, values: List[float], source_unit: str, strategy: str = None) -> str:
        ...
        logger.debug(f"使用 {strategy} 策略，参考值: {reference_value}, 体系: {system_name}")
    ...
    def convert_values(cls, values: List[float], source_unit: str, target_unit: str) -> List[float]:
        ...
        if source_system != target_system:
            logger.error(f"单位转换失败：'{source_unit}' 和 '{target_unit}' 不属于同一体系")
            return values
        ...
        # 计算转换比率
        source_multiplier = cls._calculate_unit_multiplier(normalized_source, source_system)
        target_multiplier = cls._calculate_unit_multiplier(normalized_target, target_system)
    ...
    def format_value(cls, value: float, unit: str, precision: int = None) -> str:
        ...
        # 获取展示单位
        normalized_unit = cls._normalize_unit(unit)
        display_unit = UnitConverterConstants.DISPLAY_UNIT_MAPPING.get(normalized_unit, normalized_unit)
    ...
    def auto_convert(cls, values: List[float], source_unit: str, precision: int = None, strategy: str = None) -> Tuple[List[float], str]:
        ...
        try:
            ...
            # 获取展示单位（用于日志）
            display_source = UnitConverterConstants.DISPLAY_UNIT_MAPPING.get(cls._normalize_unit(source_unit), source_unit)
            display_target = UnitConverterConstants.DISPLAY_UNIT_MAPPING.get(cls._normalize_unit(target_unit), target_unit)

            logger.debug(f"自动单位转换 ({strategy}): {display_source} -> {display_target}, 样例: {values[:3]} -> {converted_values[:3]}")
    ...
    def get_display_unit(cls, unit: str) -> str:
        ...
        """
        normalized_unit = cls._normalize_unit(unit)
        return UnitConverterConstants.DISPLAY_UNIT_MAPPING.get(normalized_unit, normalized_unit)
    ...
    @classmethod
    def _generate_unit_description(cls, unit_id: str, system_name: str, base: int) -> str:
        """
        生成单位描述
