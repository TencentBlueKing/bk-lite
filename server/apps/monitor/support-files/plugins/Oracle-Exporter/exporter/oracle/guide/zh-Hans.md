...
本插件对接 `oracledb_exporter`（Prometheus exporter），其默认指标需要监控账号具备对以下系统视图/表的 `SELECT` 权限：
...
最简单的做法是授予 `SELECT_CATALOG_ROLE`：

```sql
GRANT SELECT_CATALOG_ROLE TO <monitor_user>;
```

或授予更高权限的 `SELECT ANY DICTIONARY`（包含所有数据字典视图，含 `V$` 系列动态性能视图）：
...
> 注意 Oracle 的 `V$` 视图对应的底层对象是 `V_$`，授权时必须使用 `V_$` 形式。
