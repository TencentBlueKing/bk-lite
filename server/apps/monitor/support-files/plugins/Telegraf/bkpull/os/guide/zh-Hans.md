...
- Linux 采集节点（Telegraf 拉取）必须能访问 AIX 主机的 TCP 9100（node_exporter）。
- 若防火墙拦截 9100，复制/启动可能成功，但拉取/指标阶段会失败。
...
确认 Linux 采集节点到 AIX TCP 9100 可达（防火墙未拦截），且 `plugin_init` 已执行。拉取配置里不应出现 SSH 用户名或密码。复制/启动成功但拉取失败时，优先检查采集节点到 AIX:9100 的防火墙。
