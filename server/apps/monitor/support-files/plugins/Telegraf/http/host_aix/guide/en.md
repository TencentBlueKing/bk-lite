...
This is **local AIX OS monitoring** on the Host object. Collection uses system commands; missing commands are skipped. There is no version picker on the form. OS level comes from `oslevel -r` (for example `7100-00`). Officially tested on 7.2/7.3; collection follows whichever system commands exist and skips missing ones. After you save, the platform prepares collection on the target host and then pulls metrics on the interval.
