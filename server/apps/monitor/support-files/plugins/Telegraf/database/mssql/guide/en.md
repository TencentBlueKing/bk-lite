...
The monitoring account needs at least `VIEW SERVER STATE` (and `VIEW ANY DEFINITION` if you also collect availability / replica metrics) to read SQL Server DMVs. Follow the upstream `inputs.sqlserver` "Additional Setup" steps on the target instance:
...
For per-database metrics, create the matching user inside each database and grant `VIEW DATABASE STATE`:
...
For Azure SQL Database / Elastic Pool, see the upstream README's `## Additional Setup` section, which uses the `##MS_ServerStateReader##` server role or `VIEW DATABASE STATE` instead.

- Prefer a dedicated read-only monitoring account instead of `sa` or other high-privilege admin accounts.
- If using mixed-mode auth, confirm SQL Server authentication is allowed and the login is enabled.
- The template sets `encrypt=disable` by default; no extra certificate is required.
