...
监控账号至少具备以下权限，对应 Telegraf `inputs.mysql` 实际需要的 `SHOW GLOBAL STATUS`、`SHOW GLOBAL VARIABLES`、`SHOW SLAVE STATUS`/`SHOW REPLICA STATUS`、`SHOW BINARY LOGS`、`SHOW ENGINE INNODB STATUS`，以及对 `INFORMATION_SCHEMA` / `PERFORMANCE_SCHEMA` 的 `SELECT`（用于 `gather_innodb_metrics`、`gather_table_io_waits`、`gather_table_lock_waits`、`gather_global_variables` 等扩展指标）：

- `PROCESS`（`SHOW PROCESSLIST` / `SHOW ENGINE INNODB STATUS`）
- `REPLICATION CLIENT`（`SHOW SLAVE STATUS` / `SHOW REPLICA STATUS`）
- `SELECT` on `information_schema.*` 和 `performance_schema.*`
- `RELOAD` 或 `SUPER`（仅在 MySQL 8.0 上 `SHOW SLAVE STATUS` 需要，按需授予）
