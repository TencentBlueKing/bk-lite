...
The monitoring account needs at least the following privileges, matching what Telegraf `inputs.mysql` actually executes: `SHOW GLOBAL STATUS`, `SHOW GLOBAL VARIABLES`, `SHOW SLAVE STATUS` / `SHOW REPLICA STATUS`, `SHOW BINARY LOGS`, `SHOW ENGINE INNODB STATUS`, plus `SELECT` on `INFORMATION_SCHEMA` / `PERFORMANCE_SCHEMA` (for `gather_innodb_metrics`, `gather_table_io_waits`, `gather_table_lock_waits`, `gather_global_variables`, etc.):

- `PROCESS` (`SHOW PROCESSLIST` / `SHOW ENGINE INNODB STATUS`)
- `REPLICATION CLIENT` (`SHOW SLAVE STATUS` / `SHOW REPLICA STATUS`)
- `SELECT` on `information_schema.*` and `performance_schema.*`
- `RELOAD` or `SUPER` (only required for `SHOW SLAVE STATUS` on MySQL 8.0; grant by least-privilege principle)
