...
3. 填写完整的服务器地址和采集间隔。
4. 如目标端开启认证或 HTTPS，再补充用户名、密码、超时时间和 TLS 字段。
...
| 用户名 `username` | 否 | 仅在目标端启用 Basic Auth 时填写。若填写密码，需同时填写用户名。 |
| 密码 `ENV_PASSWORD` | 否 | 仅在目标端启用 Basic Auth 时填写。单独填写密码不会生效。 |
| 间隔 `interval` | 是 | 采集周期，单位秒，默认 `10`。 |
| 超时时间 `timeout` | 是 | 单次请求 `debug/vars` 接口的超时时间，单位秒，默认 `30`。 |
...
1. 实例列表中已出现对应 InfluxDB 实例，实例名称通常取服务器地址中的 `host` 或 `host:port` 部分。
...
| 返回 `404` | 地址不是 `debug/vars` 端点 | 检查是否误填为根路径、查询接口或 v2 接口，确认 URL 以 `/debug/vars` 结尾。 |
