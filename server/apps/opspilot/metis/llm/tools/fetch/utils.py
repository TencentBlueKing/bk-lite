...
def validate_url(url: str) -> str:
    ...
    Raises:
        ValueError: URL无效时抛出
        SSRFError: URL指向内网/云元数据等危险地址时抛出
    """
    ...
    # SSRF防护：验证URL不指向内网/云元数据等危险地址
    SSRFValidator.validate(url)

    return url


def prepare_headers(custom_headers: Optional[Dict[str, str]] = None, user_agent: Optional[str] = None) -> Dict[str, str]:
    """
    准备HTTP请求头
...
def format_response_info(status_code: int, headers: Dict[str, str], content_length: int) -> Dict[str, Any]:
    """
    格式化响应信息
...
def truncate_content(content: str, max_length: Optional[int] = None, start_index: int = 0) -> Dict[str, Any]:
    """
    截取内容到指定长度
