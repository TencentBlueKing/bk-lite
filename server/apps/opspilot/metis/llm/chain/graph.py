...
class BasicGraph(ABC):
    ...
    async def agui_stream(self, request: BasicLLMRequest) -> AsyncGenerator[str, None]:
        ...
        try:
            # 发送 RUN_STARTED 事件
            yield encoder.encode(RunStartedEvent(type=EventType.RUN_STARTED, thread_id=thread_id, run_id=run_id, timestamp=int(time.time() * 1000)))
            ...
            try:
                async for chunk in result:
                    ...
                    if isinstance(message, AIMessageChunk):
                        async for event in self._handle_ai_message_chunk(message, encoder, run_id, current_message_id, current_tool_calls):
                            yield event
                    ...
                    # 处理完整 AI 消息（非流式块）
                        ...
                        if hasattr(message, "tool_calls") and message.tool_calls:
                            for event in self._handle_tool_calls_sync(message.tool_calls, encoder, complete_message_id, current_tool_calls):
                                yield event
            ...
            if current_message_id is not None:
                yield encoder.encode(
                    TextMessageEndEvent(type=EventType.TEXT_MESSAGE_END, message_id=current_message_id, timestamp=int(time.time() * 1000))
                )
    ...
    async def _handle_ai_message_chunk(
        self, message: AIMessageChunk, encoder: EventEncoder, run_id: str, current_message_id: str, current_tool_calls: Dict[str, Dict]
    ):
        ...
        if content:
            ...
            if current_message_id is None:
                ...
                yield encoder.encode(
                    TextMessageStartEvent(
                        type=EventType.TEXT_MESSAGE_START, message_id=current_message_id, role="assistant", timestamp=int(time.time() * 1000)
                    )
                )
            ...
            yield encoder.encode(
                TextMessageContentEvent(
                    type=EventType.TEXT_MESSAGE_CONTENT, message_id=current_message_id, delta=content, timestamp=int(time.time() * 1000)
                )
            )
    ...
    async def _handle_tool_calls(
        self, tool_calls: List[Dict], encoder: EventEncoder, parent_message_id: str, current_tool_calls: Dict[str, Dict]
        ...
        for tool_call in tool_calls:
            ...
            if tool_call_id not in current_tool_calls:
                ...
                # 发送 TOOL_CALL_END
                yield encoder.encode(ToolCallEndEvent(type=EventType.TOOL_CALL_END, tool_call_id=tool_call_id, timestamp=int(time.time() * 1000)))

    def _handle_tool_calls_sync(self, tool_calls: List[Dict], encoder: EventEncoder, parent_message_id: str, current_tool_calls: Dict[str, Dict]):
