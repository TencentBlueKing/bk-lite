...
class GraphitiRAG:
    ...
    def _create_embed_client(self, embed_config: dict) -> MetisEmbedder:
        """创建嵌入客户端"""
        return MetisEmbedder(MetisEmbedderConfig(url=embed_config["url"], model_name=embed_config["model_name"], api_key=embed_config["api_key"]))

    def _create_rerank_client(self, rerank_config: dict) -> MetisRerankerClient:
        """创建重排序客户端"""
        return MetisRerankerClient(
            MetisRerankerConfig(url=rerank_config["url"], model_name=rerank_config["model_name"], api_key=rerank_config["api_key"])
        )
    ...
    def _create_full_graphiti(
        self, llm_config: Optional[dict] = None, embed_config: Optional[dict] = None, rerank_config: Optional[dict] = None, graph_database: str = None
    ) -> Graphiti:
        """创建完整配置的Graphiti实例
    ...
    def _extract_configs_from_request(
        self, req: Union[DocumentIngestRequest, DocumentRetrieverRequest, RebuildCommunityRequest]
        ...
        if hasattr(req, "openai_api_key") and req.openai_api_key:
            llm_config = {"api_key": req.openai_api_key, "base_url": req.openai_api_base, "model": req.openai_model}

        # 提取嵌入模型配置（如果存在）
        if hasattr(req, "embed_model_base_url") and req.embed_model_base_url:
            embed_config = {"url": req.embed_model_base_url, "model_name": req.embed_model_name, "api_key": req.embed_model_api_key}

        # 提取重排序模型配置（如果存在）
        if hasattr(req, "rerank_model_base_url") and req.rerank_model_base_url:
            rerank_config = {"url": req.rerank_model_base_url, "model_name": req.rerank_model_name, "api_key": req.rerank_model_api_key}
    ...
    async def delete_index(self, req: IndexDeleteRequest):
        ...
        try:
            ...
            logger.warning(f"清空graph database失败(可能不存在): {req.group_id}, 错误: {e}")
        finally:
    ...
    async def build_communities(self, graphiti_instance: Graphiti, graph_database: str):
        ...
        """
        logger.info("---------- 社区构建流程开始 ----------")
        logger.info(f"目标数据库: {graph_database}")

        # 检查图谱数据
        try:
            logger.info("正在检查图谱数据...")
            node_check, _, _ = await graphiti_instance.driver.execute_query("MATCH (n) RETURN count(n) as count")
            node_count = node_check[0]["count"] if node_check else 0

            edge_check, _, _ = await graphiti_instance.driver.execute_query("MATCH ()-[r]->() RETURN count(r) as count")
        ...
        try:
            ...
            # 检查构建后的社区数量
            community_check, _, _ = await graphiti_instance.driver.execute_query("MATCH (n:Community) RETURN count(n) as count")
        ...
        logger.info("---------- 社区构建流程完成 ----------")

    async def ingest(self, req: DocumentIngestRequest, callback, task_id):
        """文档摄取

        Args:
            req: 文档摄取请求
            callback: 进度回调函数，接收参数 (task_obj, current_index, total_count)
            task_id: 任务ID
        """
        ...
        # 使用group_id作为graph database名称
        # 创建完整配置的Graphiti实例
        graphiti_instance = self._create_full_graphiti(llm_config, embed_config, rerank_config, graph_database=req.group_id)
        ...
        try:
            ...
            for i, doc in enumerate(tqdm(req.docs, desc="处理文档")):
                ...
                try:
                    ...
                    # 每处理10个文档输出一次进度
                    if (i + 1) % 10 == 0:
                        logger.info(f"已处理 {i + 1}/{all_count} 个文档，成功: {success_count}, 失败: {failed_count}")
                except Exception as e:
                    failed_count += 1
                    ...
                    callback(i + 1, all_count, task_id)
            ...
            return {"mapping": mapping, "success_count": success_count, "failed_count": failed_count, "total_count": len(req.docs)}
        finally:
            # 确保关闭 driver 连接
    ...
    async def rebuild_community(self, req: RebuildCommunityRequest):
        ...
        logger.info(f"配置信息 - LLM模型: {llm_config.get('model') if llm_config else 'None'}")
        logger.info(f"配置信息 - Embed模型: {embed_config.get('model_name') if embed_config else 'None'}")
        logger.info(f"配置信息 - Rerank模型: {rerank_config.get('model_name') if rerank_config else 'None'}")
        ...
        graphiti_instance = self._create_full_graphiti(llm_config, embed_config, rerank_config, graph_database=graph_database)

        # 验证组件配置
        has_llm = hasattr(graphiti_instance, "llm_client") and graphiti_instance.llm_client is not None
        has_embedder = hasattr(graphiti_instance, "embedder") and graphiti_instance.embedder is not None
        has_reranker = hasattr(graphiti_instance, "cross_encoder") and graphiti_instance.cross_encoder is not None

        logger.info(
            f"组件配置状态 - LLM客户端: {'已配置' if has_llm else '未配置'}, "
    ...
    async def search(self, req: DocumentRetrieverRequest) -> List[Document]:
        ...
        # 创建Graphiti实例
        graphiti_instance = self._create_full_graphiti(embed_config=embed_config, rerank_config=rerank_config, graph_database=graph_database)
        ...
        try:
            ...
            logger.debug(f"调用graphiti_instance.search - query: {req.search_query}, num_results: {req.size}")
            result = await graphiti_instance.search(query=req.search_query, num_results=req.size)
            logger.debug(f"graphiti_instance.search返回结果数量: {len(result) if result else 0}")

            # 如果结果为空,检查是否有向量索引
            if not result or len(result) == 0:
                logger.warning("搜索结果为空 - 检查向量索引")
                # 检查是否有边的嵌入向量
            ...
            node_uid_set = set()
            for r in result:
                logger.debug(f"搜索结果项: fact={r.fact}, name={r.name}, group_id={r.group_id}, source={r.source_node_uuid}, target={r.target_node_uuid}")
            ...
            if node_uid_set:
                ...
                node_info_map = {
                    record["uuid"]: {"name": record["name"], "fact": record["fact"], "summary": record["summary"], "labels": record["labels"]}
                    for record in node_result
                }
                logger.info(f"查询到节点信息: 节点数量={len(node_info_map)}")

            # 构建结果文档
            docs = [self._build_search_result_doc(r, node_info_map) for r in result]
    ...
    def _build_node_info(self, node_uuid: str, node_info_map: dict) -> dict:
        ...
        return {"uuid": node_uuid, "name": node_info.get("name", ""), "summary": node_info.get("summary", ""), "labels": node_info.get("labels", [])}
