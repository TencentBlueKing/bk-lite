...
def apis():
    ...
    with patch.object(res, "prepare_context", return_value=None), patch.object(res.client, "CoreV1Api", return_value=core), patch.object(
        res.client, "AppsV1Api", return_value=apps
...
class TestListNodes:
    ...
    def test_notready_when_no_ready_condition(self, apis):
        ...
        node = SimpleNamespace(
            ...
            status=SimpleNamespace(conditions=[cond], allocatable=None, node_info=None, addresses=None),
        )
        core.list_node.return_value = SimpleNamespace(items=[node])
        out = json.loads(res.list_kubernetes_nodes.invoke({"config": {}}))
...
class TestListDeployments:
    ...
    def test_namespaced_and_limit_clamped(self, apis):
        ...
        out = json.loads(res.list_kubernetes_deployments.invoke({"namespace": "prod", "limit": 999, "config": {}}))
        ...
        assert "_next_step_hint" not in out
...
class TestListServices:
    def test_ports_and_external_ip(self, apis):
        ...
        assert out[0]["ports"][0] == {"name": "http", "port": 80, "target_port": "8080", "protocol": "TCP", "node_port": 30080}
...
class TestResourceYaml:
    def test_unsupported_type(self, apis):
        out = res.get_kubernetes_resource_yaml.invoke({"namespace": "default", "resource_type": "ingress", "resource_name": "x", "config": {}})
    ...
    def test_pod_yaml_strips_status_and_managed_fields(self, apis):
        ...
        with patch.object(res.client, "ApiClient", return_value=api_client):
            out = res.get_kubernetes_resource_yaml.invoke({"namespace": "default", "resource_type": "pod", "resource_name": "p1", "config": {}})
...
class TestPreviousPodLogs:
    ...
    def test_multi_container_requires_name(self, apis):
        core, _, _ = apis
        core.read_namespaced_pod.return_value = self._pod([SimpleNamespace(name="a"), SimpleNamespace(name="b")])
        out = res.get_kubernetes_previous_pod_logs.invoke({"namespace": "ns", "pod_name": "p1", "config": {}})
...
class TestSearchWorkload:
    ...
    def test_unique_default_cluster(self, apis):
        ...
        with self._patch_instances([]):
            out = json.loads(res.search_workload_across_namespaces.invoke({"workload_name": "scheduler", "config": {}}))
    ...
    def test_not_found(self, apis):
        ...
        with self._patch_instances([]):
            out = json.loads(res.search_workload_across_namespaces.invoke({"workload_name": "ghost", "config": {}}))
