...
def test_build_post_tool_directives_stops_auto_followup_report_for_inspection_only():
    directives = build_post_tool_directives(
        [
            ToolMessage(
                name="analyze_deployment_configurations",
                tool_call_id="call-1",
                content='{"cluster_name":"Kubernetes - 1","problematic":32,"issues_detail":[{"severity":"high","issue":"未配置存活探针","count":10,"workloads":["nginx-test (default)"]}],"_next_step_hint":"如用户明确要求修复方案，再继续。"}',
            )
        ]
    )

    assert any(
        "本轮只输出一次配置检查结果" in message.content
        and "不要主动调用 request_user_choice" in message.content
        and "除非用户明确要求修复方案" in message.content
        for message in directives
        if isinstance(message, SystemMessage)
    )


def test_build_config_analysis_next_step_hint_does_not_force_followup_tools():
    hint = build_config_analysis_next_step_hint(problematic_count=32, target_name=None)

    assert "request_user_choice" not in hint
    assert "generate_repair_report" not in hint
    assert "如用户明确要求修复方案" in hint
