...
因此，本方案可以消除代码层无界内存聚合、全表序列化和重试重复写入，使单次扫描的内存与事务规模有上限；但不能消除“业务要求永久保留每轮快照”带来的数据库长期线性增长，也不能提供独立 cgroup 级故障隔离。`monitor-alert-lifecycle-events` 之后，新写入的监控 Event 只保留状态变更，不再按每轮扫描落 Event；快照仍按扫描保留。

约 170 个 Pod、每分钟一次、连续三天仍会合法产生约 73 万条逐轮快照。若要求这些快照全部保留，任何实现最终都会消耗持续增长的数据库空间。要彻底消除长期存储风险，必须另行接受快照合并、保留期或存储扩容中的至少一项；本 change 不擅自作该决定。`monitor-alert-lifecycle-events` 之后，新写入 Event 只保留状态变更，不再按扫描窗口落 Event。
...
2. 无数据 Alert 与阈值 Alert 都按指标实例区分；
3. 一个持续无数据的指标维度每个扫描窗口仍生成现有业务要求的快照；Event 只在状态变更时写入；
...
- 无数据 Alert 按指标实例聚合，Event 只记录状态变更；
...
- 170 Pod、4,320 轮压测可以产生现有语义要求的约 73 万快照，但 Celery RSS 保持稳定；
