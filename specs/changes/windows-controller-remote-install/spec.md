...
7. Ansible `always` 与 Server 侧兜底清理共同删除临时会话文件和 bootstrap；最终成功仍以 Sidecar 回连为准。
...
- `web/src/app/node-manager`：开放 Windows 远程安装入口并收集 WinRM 参数。
...
- Windows 在控制器安装页可选择远程安装，固定显示 Administrator、5986、HTTPS、NTLM 和证书校验。
- Windows 任务不会调用现有 SSH 执行方法。
- bootstrap 下发失败、WinRM 连接失败、安装失败或回连超时均进入现有节点任务错误/超时状态。
- bootstrap 输出的 `BKINSTALL_EVENT` 能进入现有安装进度模型。
