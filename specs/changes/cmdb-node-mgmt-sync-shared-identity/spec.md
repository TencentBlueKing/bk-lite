...
拉同步默认打开，作为 host 库存管道；勾选框仍控制即时推送和监控，不再表示「取消勾选则 CMDB 里不能有这台主机」。拉同步与节点→CMDB 推送共用同一套认实体规则和同一套主机字段：业务身份是 **ip + 云区域**，硬关联补上 **`node_id`**。新建时实例名为 `{ip}[{云区域}]`；更新时 IP 与云区域未变则保留自定义名，变化则刷新。命中同一条则更新，禁止双建。
...
4. As a CMDB 用户, I want 从节点进入的新建主机实例名是 IP 加云区域, so that 默认名一致；已有自定义名在 IP 与云区域未变时不被周期同步覆盖。
...
- `inst_name`：新建写 `{ip}[{云区域名}]`（无云区域名时用云区域 ID）；节点→CMDB 推送与监控→CMDB ingest 创建时忽略对端名称。更新时若 IP 与云区域未变则不写 `inst_name`，保留用户自定义名；IP 或云区域变化则刷新为新的 `{ip}[{云区域}]`。
- `ip_addr`、`cloud`、`os_type`、`organization` 取自当前节点快照；两边公式一致。
- `node_id`：新建必写；已有为空则补；已有且相同则保持；冲突不覆盖。
- `monitor_id`：拉同步不读不写；只由推送/监控路径维护。
- 现网已有自定义实例名的 host，只要 IP 与云区域未变，下次拉同步或节点再推保留该名称。
...
- 节点→CMDB：新建实例名为 `IP[云区域]`，写入 `node_id`，回填 `Node.cmdb_id`；更新且 IP/云区域未变时不覆盖自定义 `inst_name`。
...
- 监控手建→CMDB、CMDB 手建→监控的实例名规则不在本变更修改；一旦该 host 也被节点或监控联动管道命中，仅在 IP/云区域变化时把实例名改为 `IP[云区域]`。
