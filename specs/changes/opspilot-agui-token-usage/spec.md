# OpsPilot Agent token 用量日志

Status: approved

## 目标

OpsPilot 的 `execute_agui` skill 测试接口和工作台 NATS 工作流中的 Agent 节点
必须记录一次完整 Agent 问答的 token 消耗，使调用日志能够反映包含工具调用和
多轮模型推理在内的实际模型用量。
...
- NATS 工作流每执行一个 Agent 节点，必须为该节点创建一条
  `SkillRequestLog`；一个工作流执行多个 Agent 节点时分别记账，不合并为一条。
- NATS Agent 节点日志必须关联当前 `bot_id`、`execution_id`、`node_id` 和
  `skill_id`，以便定位具体工作流调用。
...
服务运行日志同时记录入口类型、bot ID、execution ID、node ID、skill ID、
skill 名称及三个 token 数值，不记录提示词、凭据、工具参数或完整回答。
...
- 工作台 `trigger_workflow_by_nats` 走同步工作流执行路径；Agent 节点在
  `ChatService.invoke_chat` 返回后读取该次节点执行已汇总的 token 数值，并以
  OpenAI 风格 `usage` 结构写入 `SkillRequestLog.response_detail`。
- NATS 日志写入失败只记录异常，不改变 Agent 节点结果，也不阻断工作流后续节点。
...
- NATS 工作流执行单个 Agent 节点时创建一条带 token 用量的
  `SkillRequestLog`，并携带可定位工作流的元数据。
- 同一 NATS 工作流执行多个 Agent 节点时，每个节点各创建一条日志，token 不串账。
- 非 NATS 入口不得因新增逻辑产生重复的 `SkillRequestLog`。
- NATS 日志持久化失败时，工作流仍返回原有 Agent 执行结果。
