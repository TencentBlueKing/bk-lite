...
- 别名表集中在 Adapter 契约常量，查询同时认：`db.system` 与 `db.system.name`；`db.name` 与 `db.namespace`；`messaging.system`；`rpc.system`（可选 `rpc.service`）；兜底 `peer.service` 再 `server.address`。折叠键优先 `peer.service`，否则库/消息 system，再否则 `server.address`。
