...
- 模型与 migration：`LLMSkill.memory_space` / `memory_write_rounds`（默认 10，1–50），`SkillConversation.memory_written_message_id`；`MemorySpace.is_builtin` 字段保留但不参与查询筛选；`Memory.owner_user_id`。`force_wiki_grounded` 保留历史 `0078_llmskill_force_wiki_grounded`；技能记忆字段在 `0079_skill_memory`（依赖 0078，不再重加 `force_wiki_grounded`，只加字段，不创建/挂接内置记忆体）
