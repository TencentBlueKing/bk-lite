...
日期：2026-09-14；2026-09-16 调整为跳过无法转换的策略。尚未对客户数据库执行。
...
以下情况跳过并保留**整条策略**，记录 WARNING 日志及 scope、策略 ID、failed_stage、error_type：未知字段、目标字段不再支持的旧文本/正则操作（例如标题正则、对象实例的字符串包含）、不明确的数组文本条件、无效级别、来源歧义、展开后超过 20 组/100 条条件，或候选超过 50 项、文本超过 256 字符。不删除失败条件，也不只迁移某个 OR 分支；需按当前字段目录人工重新配置。
...
- 整个数据步骤在一个事务中完成；单条策略转换的 `ValueError` / `TypeError` 被捕获后跳过，后续策略及其他入口继续转换。数据库读取/写入失败仍上抛并回滚此次五入口数据修改，不记录 `0033` 成功；此前已经完成的 schema 迁移仍保留。
- 跳过日志使用 `event=legacy_rule_migration_skipped`，包含 scope、policy_id、failed_stage=convert、error_type，不输出规则值、异常正文或堆栈。跳过策略的全部条件、启用状态、更新时间和其他配置原样保留。
- 有跳过项也会正常记录 `0033` 完成，后续 `migrate` 不会自动重试这些策略。按日志中的类型和 ID 在策略编辑页按新版目录重新配置并保存；无效条件在修复前仍不命中，不把筛选空规则改为全部匹配。不要使用 `--fake` 或删除迁移记录来重跑。
- 迁移只读写数据库，不调用 API、RPC、队列、Worker，不执行分派或通知，不追补历史触发。无法转换的业务配置只影响相应策略，不应阻断其他规则升级或服务启动；数据库故障仍属于关键初始化失败。
...
2026-09-16 跳过策略回归：**1306 项通过**，其中迁移相关 **178 项**；迁移代码覆盖率 **93%**。Black、isort、定向 Flake8 与 `git diff --check` 通过。测试日志：`/tmp/alert-migration-skip-regression.log`；修改前同症状复现：`/tmp/alert-migration-skip-red.log`（五入口均因转换错误阻断）。

2026-09-14 回归结果：**1299 项通过**，其中迁移相关测试 **171 项**；数据迁移代码覆盖率 **93%**。Black、isort 与定向 Flake8 检查通过。运行证据：`/tmp/alert-data-migration-regression.log`。

迁移测试覆盖五入口转换、序列化回显、保存校验、实际来源匹配和恢复事件排除、超过两批的规则扫描（含批次末尾坏规则）、停用策略、保留无关配置、幂等、转换失败跳过并继续后续策略、日志敏感值不泄露，以及数据库失败保留原异常并回滚。

另外使用独立内存数据库，按 `0032` 的历史模型状态建表，通过真实 `MigrationExecutor` 执行 `0033`，验证历史模型、跳过坏规则后仍记录成功、重复执行不再运行、空数据升级以及反向迁移明确拒绝。
