...
为实现用户完整控制通知正文，建议新增通用可选 `append_receivers`，默认 true；自定义
告警通知显式传 false。该参数只是展示选项，底层不得依据模板 ID 判断何时关闭。
...
- 告警模板试发应在 Alerts 校验模板和授权后，通过已保存的 channel ID 调用通用发送能力；
  不把 SMTP、Webhook 等原始配置复制到 Alerts，也不假装原 test_send 能发送自定义正文。
...
4. 系统管理的必要内容行为改动收敛为向后兼容的接收人追加选项；渠道授权摘要复用已有
   查询能力。业务模板渲染测试必须经过真实底层 Adapter 到 HTTP/SMTP 替身。
5. 已发送内容不能再次整体 escape 或按固定模板重排；变量值和用户写的格式标记分开处理。
6. 实施草案第 7 节的 HTML 白名单、动态属性、图片、长度上界是拟议的 Alerts 编辑/渲染
   安全与资源策略，**不是系统管理已有的模板限制**。它们尚待用户确认，不能推广到共享
   sender 或追溯拒绝其他业务现有内容。
7. 模板内容定制的排版自由度以“保留业务用户写的格式”为目标；预览隔离与变量安全并不
   等于必须重排所有 HTML。资源/协议限制与浏览器预览限制需分别呈现，不擅自声称平台不支持。
8. 本文不改变“待明确实施授权”的状态；所有改动目前仅限方案和测试设计。
