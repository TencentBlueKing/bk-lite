...
- 首轮滚动升级将新版本设为 `LOG_GROUP_RULE_MODE_ENFORCEMENT=legacy`：新 writer 已拒绝坏规则，新 reader 与旧 reader 保持相同语义；等旧 writer 全部排空后再盘点。falsey 非对象、畸形结构及可改变 LogsQL 结构的字段/值必须修正，未知字符串必须修正或加入 `LOG_GROUP_LEGACY_OR_GROUP_IDS`。
