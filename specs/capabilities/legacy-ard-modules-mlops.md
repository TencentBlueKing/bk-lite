...
- 时序 serving 的 update/start/stop/remove/DELETE 在数据库行锁内串行化运行时副作用，`container_info` 对 API 输入只读。外部调用前先递增 `container_info._runtime_generation` 并记录 transition，调用失败或超时后对账实际状态，无法对账则保留 `unknown`；成功结果和回滚也继续推进 generation。列表和详情先用唯一 query token 批量认领 generation，再以完整认领状态 CAS 写回，因此并发轮询不能乱序覆盖，旧状态也不能覆盖一次 remove→serve 切换，即使切换前后的公开状态值相同；并发删除只保留本次响应快照，不产生查询 500，批量同步固定为三条数据库查询而非逐行写入。
