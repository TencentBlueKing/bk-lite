...
- 时序 serving 的 update/start/stop/remove/DELETE 在数据库行锁内串行化运行时副作用；每次运行时所有者变更都会递增 `container_info._runtime_generation`。列表和详情的实时状态查询按该 generation 批量 CAS 写回，因此不能以旧状态覆盖一次 remove→serve 切换，即使切换前后的公开状态值相同；并发删除只保留本次响应快照，不产生查询 500。
