...
- 控制台 iframe 屏显【本阶段已实现】：`?screen=true` 隐藏平台壳层；运营分析 view 另藏目录侧栏，settings 另藏管理分段菜单，会话内切画布透传 `screen`；不改分享链路。非其它模块全量透传。证据：`web/src/console-layout/screenMode.ts`、`web/src/app/ops-analysis/(pages)/view/page.tsx`、`web/src/app/ops-analysis/(pages)/settings/layout.tsx`、`web/src/app/ops-analysis/utils/viewHref.ts`、`docs/operations/console-iframe-screen-mode.md`。
...
- `[operation_analysis#20260910-002]` `/ops-analysis/settings` 屏显下不再包分段菜单（数据源 / 连接库 / 命名空间）。证据：`web/src/app/ops-analysis/(pages)/settings/layout.tsx`。
