...
- `.husky/pre-commit`（仓库根目录唯一逻辑入口）：`web/`、`mobile/` staged 变更自动 eslint + type-check；`mobile/src` 的脚本或语言包变更再跑 `pnpm test:i18n`。`web/`、`mobile/` 的 `prepare` 注册根目录 `.husky`（`web/.husky/pre-commit` 仅转发到根钩子）。
