...
- 子对象指标只携带部分身份维度时，只能在策略已选实例范围内唯一匹配后解析
  为监控实例；存在歧义时不得猜测归属。
- 无数据 Alert 与阈值 Alert 一样按指标实例（策略 `group_by` 元组 /
  `metric_instance_id`）聚合。同一策略、同一监控实例下多条缺失序列各自
  一条活动 Alert；打开时每张 Alert 只写一条 `triggered` MonitorEvent。
  持续无数据只追加快照，不按扫描轮次再建 Event。
- 无数据 Alert 在对应指标实例重新出现数据后自动恢复；同实例其他缺失
  序列各自保持活动，互不影响。
