...
| 接入指引 | 按语言生成离线安装说明 | Java / Python / Node.js / Go / .NET（Linux x86_64 glibc）；指向系统内下载与区域内网接入；不支持公网令牌 | Beta |
...
| 错误聚合 | 按异常语义聚类错误 | 基于可见服务；服务详情错误 Tab 解释错误率：同源入口 ERROR Span 聚成 Top Issue，用 RED 请求/失败次数对账，不按 Span 翻页 | Beta |
