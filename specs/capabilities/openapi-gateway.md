...
| 多 worker | `_me` / `_docs` / `_auth` 的外部服务数据以进程内快照为基础，读侧距上次 KV 对账超过 TTL（`OPENAPI_REGISTRY_CACHE_TTL`，默认 10 秒，0 为每次回源）时现场回源，滞后 ≤ TTL；KV 不可达时降级最近快照。仍不能作为可用性判据：实际可调还取决于 Traefik 已拉取路由 |
