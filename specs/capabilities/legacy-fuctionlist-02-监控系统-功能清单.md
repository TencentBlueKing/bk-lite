...
| 告警详情 | 查看告警详情及生命周期内的指标快照 | 快照记录于告警全生命周期，支撑复盘；详情、认领、分派响应含 `handlers_display` | GA |
| 告警关闭 | 人工关闭活跃告警 | 受权限点控制 | GA |
| 告警认领 / 分派 | 空处理人的活跃告警可认领给自己或分派给告警所属组织用户 | 有处理人后本期不可再分派；认领 / 分派写 `claimed` / `assigned` MonitorEvent；手工分派按策略已开的人工渠道通知被分派人，告警中心副本 / 仅 NATS 不发；创建与认领不发分派通知 | GA |
...
| 事件动作 | 监控事件动作 | `triggered` 触发 / `escalated` 级别升级 / `claimed` 认领 / `assigned` 分派 / `recovered` 恢复 / `closed` 人工关闭；每告警每动作一条的唯一约束只覆盖 `triggered` / `recovered` / `closed`，不扩到认领与分派 | GA |
