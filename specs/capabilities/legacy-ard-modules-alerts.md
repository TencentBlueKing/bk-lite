...
五个入口共用当前模型字段目录：相关性、屏蔽、丰富匹配 Event；分派、处理匹配 Alert。分派/处理的监控源字段为 `push_source_ids`（list[str]，any_of/all_of/none_of）；相关性/屏蔽/丰富为 `push_source_id`（str，any_of/none_of）。标题与内容为 eq/ne/contains/not_contains（Event 内容用 description，Alert 用 content）；级别为 any_of/none_of 多选；Event 告警源为 `source_name`（any_of/none_of）；Alert 告警源为派生集合 `source_names`（any_of/all_of/none_of，排除 recovery），`Alert.source_name` 只保留通知和参数绑定快照。未知字段、历史别名/操作符及错误值类型拒绝保存，执行时整条规则不命中。缺失/空值/空列表不命中，包括否定条件。完整字段与操作符以 `utils/rule_fields.json` 和 `specs/changes/alert-rule-types/spec.md` 为准。

实现入口为 `utils/rule_catalog.py`、`utils/typed_rules.py`、`utils/monitor_source_rules.py`。沿用条件组 AND/OR 和已有分派时机；来源持久化由 `service/monitor_sources.py` 维护，历史数据由独立命令 `backfill_alert_monitor_sources` 回填，详见 `specs/changes/alert-monitor-sources/spec.md`。

## 3. 接口【已实现/已存在】
所有 ViewSet 路由组均以 `router.register(r"api/<name>", ...)` 注册（urls.py:35-55），故完整路径统一带 `api/` 段，例如 `api/v1/alerts/api/alert_source/`。路由组：`api/alert_source`/`api/alerts`/`api/events`/`api/level`/`api/settings`/`api/assignment`/`api/shield`/`api/enrichment`/`api/incident`(+`/(?P<incident_pk>\d+)/updates`)/`api/alarm_strategy`/`api/log`/`api/action_rule`/`api/action_execution`；开放端点 `open_api/k8s` 与 `api/open/alerts*`（列表/详情/事件/动作/批量动作，urls.py:45-54）。告警 OpenAPI 已经 `@openapi_expose` 注册到统一网关 `/openapi/v1/alerts/*`（list/detail/events/assign/acknowledge/reassign/close/batch-action/shield-create/shield-operate，以及 PUT/DELETE shield）。查询与告警生命周期锚点为业务 `alert_id`；屏蔽策略按名称定位，仅能操作本人创建的记录。企业扩展经 `alert_extension_routes` 挂载，不是独立 app。
