...
- RPC 客户端（均为客户端封装）：`executor.py`、`ansible.py`、`cmdb.py`、`monitor.py`、`log.py`、`node_mgmt.py`、`system_mgmt.py`、`opspilot.py`、`alerts.py`、`job_mgmt.py`、`mlops.py`、`console_mgmt.py`、`stargazer.py`、`operation_analysis.py`、`patch_mgmt.py`。`system_mgmt.py` 转发凭据仓库 `list_credentials` / `create_credential` / `resolve_credential`；明文只经 `resolve_credential`。引用计数是反方向：系统管理 `RpcClient().run` 直接请求 `cmdb_count_credential_refs` / `monitor_count_credential_refs`，不经 `cmdb.py` / `monitor.py` 客户端。详见 [[legacy-ard-modules-system-mgmt.md#4.2 凭据仓库跨模块契约【已实现 / 监控引用计数待对接】]]。
