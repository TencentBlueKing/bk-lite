...
- 日志分组规则模式【已实现/已存在】：`LogGroup.rule.mode` 缺省为 `AND`，写入仅接受大小写不敏感的 `AND` / `OR`，并校验 `conditions` 容器、字段、操作和值；读取 `strict` 模式下未知值或 falsey 非对象规则进入 `invalid_rule` 并 deny-all。存量滚动升级先让全部新实例以 `LOG_GROUP_RULE_MODE_ENFORCEMENT=legacy` 封住新写入并保持旧读语义，排空旧 writer 后执行 `audit_log_group_rule_modes --format jsonl`；falsey 非对象规则须修正，未知字符串须修正或通过 `LOG_GROUP_LEGACY_OR_GROUP_IDS` 按 ID 显式保留原 `OR`。只有 `--fail-on-uncovered` 通过后才切换默认 `strict`；查询兼容状态分别为 `legacy_empty_rule` / `legacy_or`。修正规则后先删兼容 ID；回滚前重新预检并先切 `legacy`，数据库无需逆向迁移。
