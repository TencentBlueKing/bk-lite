...
- Docker 模型发布的 `startup_timeout_seconds` 是从 webhookd 请求进入到业务健康检查成功的总预算；初次启动使用 `restart=no`，业务 `/health` 必须回显本次随机 `SERVING_INSTANCE_ID` 后才切换为 `unless-stopped`。失败、超时或中断按本次启动标签/CID 有界回滚。算法容器设置 `BENTOML_CONTAINERIZED=true`，模型 worker 初始化失败不得被父进程无限拉起并伪装为可用。
...
- loader 按 `config.source` 三态分发 + 显式降级门控：`algorithms/classify_anomaly_server/classify_anomaly_server/serving/models/loader.py` 的 `_fallback_or_raise` 与 `load_model`；其余四个 Docker 发布服务同构。默认缺参/异常抛错，`ALLOW_DUMMY_FALLBACK=true` 时才返回 `DummyModel`。
