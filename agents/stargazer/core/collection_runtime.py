...
logger = logging.getLogger("stargazer.collection_runtime")
...
class InMemoryRunStateStore:
        ...
        *,
        ...
        async with self._lock:
            ...
            if record and record.request_digest != request_digest:
                return LeaseAcquisition(LeaseAcquireStatus.CONFLICT, record.lease)
            if record and record.status == RunStatus.COMPLETED:
                return LeaseAcquisition(
                    LeaseAcquireStatus.COMPLETED,
                    record.lease,
                    record.summary,
                )
    ...
    async def finish(
        ...
        async with self._lock:
            ...
            record.summary = dict(summary or {})
            return True
...
class CollectionRuntime:
    ...
    async def submit(self, request: CollectionRequest) -> Submission:
        ...
        if acquisition.status == LeaseAcquireStatus.COMPLETED:
            return Submission(
                task_id=request.task_id,
        ...
        if acquisition.status == LeaseAcquireStatus.CONFLICT:
            return Submission(
                task_id=request.task_id,
                status=SubmissionStatus.CONFLICT,
                fence=lease.fence if lease else 0,
                reason="task_id already belongs to a different request",
            )
        if lease is None:
