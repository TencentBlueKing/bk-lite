...
- **已选对象小卡：** 技能包、工具、已挂载项等共用一套缩小解剖：标题行左「名称 + 数量徽章」、右「+ 添加xxx」（AntD `link` + `small`）；下一句说明；下方网格小卡（`rounded-lg p-2.5`，左 `20×20` 图标容器 + 名称，右配置/删除）。有选中即启用，空列表即关闭，**不要再给集合加总开关**；后端仍传列表（空数组即关）。数量徽章：`primary-bg-active` 底 + `primary` 字，小胶囊（这是少数允许 `rounded-full` 的计数，不是按钮）。参考：OpsPilot 智能体设置页的技能包 / 工具。
...
- **用户问题：** 右对齐气泡，底同样 `fill-1/70`，不要饱和的 `primary-bg-active`。
- **助手回复：** 左对齐纯文本，不要气泡壳。
...
- **Don't** 给快捷问答、普通按钮使用胶囊大圆角；与保存按钮一样用 `6px`。数量徽章除外。
