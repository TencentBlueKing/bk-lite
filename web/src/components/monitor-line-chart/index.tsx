...
import {
  ...
  type TableDataItem,
  type ThresholdField,
} from '@/components/monitor-chart-runtime';
...
import { LEVEL_MAP as MONITOR_LEVEL_MAP } from '@/components/monitor-shared';
import { useMonitorLevelOptions } from '@/components/monitor-shared';
import { resolveMonitorUnitLabel } from '@/components/monitor-shared';
import {
  GAP_INTERVAL_AREA_STYLE,
  getChartDataWithGapBreaks,
  getRenderedGapIntervals
} from '@/components/monitor-chart-runtime/gap-intervals';
...
const LineChart: React.FC<LineChartProps> = memo(
  ({
    ...
    const levelList = useMonitorLevelOptions();
    ...
    const {
      chartKeys: chartAreaKeys,
      colors,
      details: rawDetails,
      hasDimension,
      toggleVisibleArea,
      visibleAreas,
    } = useChartSeriesState<ChartData, Record<string, ListItem[]>>({
      data,
      generateColor: generateUniqueRandomColor,
    });
    const details = rawDetails as Record<string, ListItem[]>;
    ...
    const formatMetricValue = useCallback(
      (value: unknown) => {
        ...
        return unitName ? `${formattedValue} ${unitName}` : formattedValue;
      },
      [unitName]
    );
    ...
    return (
      <ChartWithDimensionPanel
        ...
        emptyClassName={`${chartLineStyle.chart} ${chartLineStyle.noData}`}
        chart={
          <>
            <TimeSeriesAreaChartCanvas
              ...
              gridStroke="rgba(107, 127, 152, 0.28)"
              ...
              overlaysBeforeTooltip={renderedGapIntervals.map((gap) => (
                <ReferenceArea
              ...
              overlaysAfterSeries={
                <>
                  {validThresholds.map((item, index) => {
                    const levelColor =
                      (MONITOR_LEVEL_MAP[item.level] as string) || '#F43B2C';
              ...
              renderTooltip={(visible) => (
                <ChartSeriesTooltip
                  ...
                  renderTitle={(tooltipLabel) =>
                    `${convertToLocalizedTime(
                      new Date(Number(tooltipLabel) * 1000) + ''
                    )}`
                  }
                  getItems={(tooltipPayload) =>
                    tooltipPayload.map((item: any) => {
                      const value = getEnumValue(metric as MetricItem, item.value);
                      const currentUnit =
                        (item.dataKey && seriesUnits[item.dataKey]) || unit;
                      return {
                        key: item.dataKey,
                        color: item.color,
                        ...
                        value:
                          value === '--'
                            ? value
                            : `${value} ${resolveUnitLabelSafe(currentUnit)}`.trim(),
                        sortValue:
                          typeof value === 'number'
                            ? value
                            : Number(item.value ?? 0),
              ...
              selectionYAxisId="left"
              series={chartAreaKeys.map((key, index) => ({
                dataKey: key,
                type: 'monotoneX',
                yAxisId: 'left',
              ...
              xAxisType="number"
              yAxisAllowDataOverflow={false}
              yAxisDomain={niceYAxis.domain}
              yAxisId="left"
              yAxisInterval={0}
              yAxisTick={renderYAxisTick}
              yAxisTicks={niceYAxis.ticks}
              yAxisWidth={leftAxisWidth}
            />
        ...
        filterProps={
          showDimensionFilter && hasDimension
            ? {
              ...
              visibleAreas,
              details,
              onLegendClick: toggleVisibleArea,
        ...
        tableProps={
          showDimensionTable && hasDimension
            ? {
              ...
              detailColumnTitle: t('monitor.search.identifier'),
              metricColumns: [
                {
                  key: 'minValue',
                  title: t('monitor.search.min'),
                  renderText: (value) => formatMetricValue(value),
                },
                {
                  key: 'maxValue',
