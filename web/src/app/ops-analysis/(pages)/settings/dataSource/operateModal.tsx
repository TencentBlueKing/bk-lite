...
const OperateModal: React.FC<OperateModalProps> = ({
  ...
  useEffect(() => {
    ...
    if (
      currentRow.response_field_schema_config?.fields &&
      Array.isArray(currentRow.response_field_schema_config.fields)
    ) {
      setSchemaFields(
        currentRow.response_field_schema_config.fields.map((field) => ({
          ...field,
          id: uuidv4(),
        })),
      );
  ...
  const onFinish = async (values: any) => {
    try {
      ...
      const responseFieldSchemaConfig: Record<string, any> = {};
      if (schemaFields.length > 0) {
        responseFieldSchemaConfig.fields = schemaFields.map(
      ...
      const submitData = {
        ...
        response_field_schema_config: responseFieldSchemaConfig,
        params: params
