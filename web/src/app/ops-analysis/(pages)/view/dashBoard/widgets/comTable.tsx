...
const flattenNamespaceRows = (
  rows: NamespaceGroupedItem[] = [],
): TableDataItem[] => {
...
const ComTable: React.FC<ComTableProps> = ({
  ...
  const { tableData, pagination } = useMemo(() => {
    ...
    if (Array.isArray(rawData)) {
      const isNamespaceGrouped = rawData.every(
        (item) => item && typeof item === 'object' && 'data' in item,
      );
      const mergedRows = isNamespaceGrouped
        ? flattenNamespaceRows(rawData as NamespaceGroupedItem[])
        : rawData;
      let total = mergedRows.length;
      if (isNamespaceGrouped) {
        total = (rawData as NamespaceGroupedItem[]).reduce((sum, item) => {
          const value = Number(item?.count);
          if (!Number.isNaN(value) && value >= 0) {
            return sum + value;
          }
          const itemRows = Array.isArray(item?.data) ? item.data.length : 0;
          return sum + itemRows;
        }, 0);
      }
      const firstGroup = isNamespaceGrouped
        ? (rawData as NamespaceGroupedItem[])[0]
        : undefined;
      const current = Number(firstGroup?.page);
      const pageSize = Number(firstGroup?.page_size);

      return {
        tableData: mergedRows,
        pagination: {
          current: !Number.isNaN(current) && current > 0 ? current : 1,
          pageSize: !Number.isNaN(pageSize) && pageSize > 0 ? pageSize : 20,
          total,
        },
      };
    }

    const rawItems = rawData.items || rawData.data || rawData.list || [];
    const isNamespaceGrouped =
      Array.isArray(rawItems) &&
      rawItems.every(
        (item: any) => item && typeof item === 'object' && 'data' in item,
      );
    const items = isNamespaceGrouped
      ? flattenNamespaceRows(rawItems as NamespaceGroupedItem[])
      : rawItems;
    const paginationInfo: PaginationInfo = {
      current: rawData.page || rawData.current || 1,
      pageSize: rawData.page_size || rawData.pageSize || 20,
      total: rawData.total || rawData.count || items.length,
    };

    return { tableData: items, pagination: paginationInfo };
  }, [rawData]);
  ...
  const columnConfigs = useMemo((): TableColumnConfigItem[] => {
    ...
    const schemaFields = dataSource?.response_field_schema_config?.fields;
    ...
    dataSource?.response_field_schema_config?.fields,
    tableData,
  ]);
