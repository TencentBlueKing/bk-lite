...
// displayMaps.value = 数值字段名（如 slow_count）

interface MysqlInstanceBarProps {
  rawData: any;
  loading?: boolean;
  config?: {
    displayMaps?: {
...
const MysqlInstanceBar: React.FC<MysqlInstanceBarProps> = ({
  rawData,
  loading = false,
  config
}) => {
  ...
  const valueField = config?.displayMaps?.value || 'slow_count';
...
export default MysqlInstanceBar;
