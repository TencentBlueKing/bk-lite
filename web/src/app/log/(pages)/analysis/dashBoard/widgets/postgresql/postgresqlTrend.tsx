...
interface PostgresqlTrendProps {
  rawData: any;
  loading?: boolean;
}
...
const PostgresqlTrend: React.FC<PostgresqlTrendProps> = ({ rawData, loading = false }) => {
  ...
  const option = useMemo(() => {
    ...
    return {
      animation: false,
      color: [colors.primary, colors.danger, colors.warning, colors.series[1]],
      tooltip: {
        trigger: 'axis',
      ...
      series: [
        ...
        },
        {
          name: '客户端活跃数',
          type: 'line',
          data: sortedData.map((item) => Number(item.client_count || 0)),
          yAxisIndex: 1,
          smooth: true,
          symbol: 'none',
          lineStyle: { width: 2, color: colors.series[1] },
          areaStyle: createSoftLineArea(colors.series[1])
        }
      ]
    };
  }, [colors, rawData]);
...
export default PostgresqlTrend;
