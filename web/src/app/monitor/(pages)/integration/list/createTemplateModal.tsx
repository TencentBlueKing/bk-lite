...
interface CreateTemplateModalProps {
  objects: ObjectItem[];
  onSubmit: (values: Record<string, any>, mode: 'add' | 'edit', id?: number) => Promise<void>;
}

const CreateTemplateModal = forwardRef<ModalRef, CreateTemplateModalProps>(({ objects, onSubmit }, ref) => {
  ...
  const monitorObjectOptions = useMemo(
    () =>
      objects
        .filter((item) => !selectedObjectType || item.type === selectedObjectType)
        .map((item) => ({
          value: item.id,
  ...
  useImperativeHandle(ref, () => ({
    showModal: ({ form: initialForm = {}, type }) => {
      const initialMonitorObject =
        initialForm?.monitor_object?.[0] || initialForm?.parent_monitor_object;
      const targetObject = objects.find((item) => item.id === initialMonitorObject);
  ...
  return (
    <OperateModal
      ...
      footer={
        <div>
          <Button className="mr-[10px]" type="primary" loading={loading} onClick={handleSubmit}>
            {t('common.confirm')}
          </Button>
    ...
    >
      <Form form={form} layout="vertical">
        <Form.Item label="监控对象类型" name="monitor_object_type" rules={[{ required: true, message: t('common.required') }]}>
          <Select
            disabled={mode === 'edit'}
        ...
        <Form.Item label="监控对象" name="monitor_object" rules={[{ required: true, message: t('common.required') }]}>
          <Select
            disabled={mode === 'edit'}
        ...
        <Form.Item label="模板名称" name="display_name" rules={[{ required: true, message: t('common.required') }]}>
          <Input />
        </Form.Item>
        <Form.Item label="模板ID" name="template_id" rules={[{ required: true, message: t('common.required') }]}>
          <Input disabled={mode === 'edit'} />
        </Form.Item>
        <Form.Item label="模版类型" name="template_type">
          <Radio.Group>
        ...
        <Form.Item label="模版介绍" name="description">
