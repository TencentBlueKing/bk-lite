...
import { VOICE_GATEWAY_DASHBOARD_CONFIG } from './config';
import styles from './index.module.scss';

// 语音网关接口-only 基线：仅运行时长 + 收发流量。
// 通话/媒体/SBC 会话等私有健康指标为 per-row 索引表（需行级过滤），telegraf 无法采集 → 不展示。
const KPI_TITLES = ['运行时长', '入向总流量', '出向总流量'];
const CHART_TITLES = ['设备收发流量趋势'];

export default function VoiceGatewayDashboardPage() {
  const dashboard = useSimpleDashboardData(VOICE_GATEWAY_DASHBOARD_CONFIG);
  ...
  // 共享 VoiceGateway 盘按当前实例品牌在头部标识（如 AudioCodes）。品牌按 instance_id 识别
  // （品牌采集模板把 collect_type 如 snmp_audiocodes 写进 instance_id 模板，可靠且不受自定义实例名影响）。
