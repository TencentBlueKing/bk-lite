...
// 共享网络服务设备仪表盘：覆盖 bk-lite NetworkService 对象下的 SNMP 插件（首品牌 Infoblox NIOS）。
// Infoblox NIOS 保守采集：仅 IF-MIB 64 位 HC 接口流量 + 运行时长。
// DNS/DHCP 服务统计、节点 CPU/内存为 per-row 索引表/服务计数带行级过滤，inputs.snmp 不能行级过滤，故 N/A。
// 取不到的实例对应卡片/趋势显示「--」/空，不伪造。
export const NETWORK_SERVICE_DASHBOARD_CONFIG: SimpleDashboardConfig = {
  routeKey: 'network_service',
  pageTitle: '网络服务设备监控仪表盘',
  objectFallbackName: 'NetworkService',
  instanceType: 'network_service',
  collectionStatusQuery:
    "count({instance_type='network_service', __$labels__}) by (instance_id)",
  ...
  metrics: [
    ...
    },
    {
      ...
      display_name: '入向总流量',
      description: '设备所有接口入向流量速率之和（字节/秒），取自 IF-MIB 64 位 HC 计数器。',
      unit: 'byteps',
      query:
    ...
    },
    {
      ...
      display_name: '出向总流量',
      description: '设备所有接口出向流量速率之和（字节/秒），取自 IF-MIB 64 位 HC 计数器。',
      unit: 'byteps',
      query:
