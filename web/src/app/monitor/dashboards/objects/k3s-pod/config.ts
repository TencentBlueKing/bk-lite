...
export const POD_DASHBOARD_CONFIG: SimpleDashboardConfig = {
  routeKey: 'k3s-pod',
  pageTitle: 'K3s Pod 监控仪表盘',
  objectFallbackName: 'Pod',
  instanceType: 'k3s',
  clusterFilter: true,
  collectionStatusQuery:
    "count(prometheus_remote_write_kube_pod_info{instance_type='k3s', __$labels__}) by (instance_id)",
  metaItems: ['cAdvisor', 'k3s'],
  metrics: [
    {
    ...
    },
    {
      ...
      unit: 'counts',
      query: 'prometheus_remote_write_kube_pod_container_status_restarts_total{instance_type="k3s",__$labels__}',
      color: '#ff4d4f',
    ...
    },
    {
      ...
      unit: 'percent',
      query:
        '100 * ( sum(irate(prometheus_remote_write_container_cpu_usage_seconds_total{instance_type="k3s",__$labels__}[5m])) by (instance_id,pod) / on(instance_id,pod) sum(prometheus_remote_write_kube_pod_container_resource_limits{instance_type="k3s", resource="cpu",__$labels__}) by (instance_id,pod) )',
      color: '#2f6bff'
    },
    {
      ...
      unit: 'percent',
      query:
        '100 * ( sum(prometheus_remote_write_container_memory_working_set_bytes{instance_type="k3s",__$labels__}) by (instance_id,pod) / on(instance_id,pod) sum(prometheus_remote_write_kube_pod_container_resource_limits{instance_type="k3s", resource="memory",__$labels__}) by (instance_id,pod) )',
      color: '#27c274'
    },
    {
      ...
      unit: 'byteps',
      query:
        'sum by (instance_id,pod) ( rate(prometheus_remote_write_container_network_receive_bytes_total{instance_type="k3s",__$labels__}[5m]) )',
      color: '#13c2c2'
    },
    {
      ...
      unit: 'byteps',
      query:
        'sum by (instance_id,pod) ( rate(prometheus_remote_write_container_network_transmit_bytes_total{instance_type="k3s",__$labels__}[5m]) )',
      color: '#597ef7'
    },
    {
      ...
      unit: 'counts',
      query:
        'sum by (instance_id,pod,device) ( rate(prometheus_remote_write_container_fs_reads_total{instance_type="k3s",__$labels__}[5m]) )',
      color: '#9254de',
    ...
    },
    {
      ...
      unit: 'counts',
      query:
        'sum by (instance_id,pod,device) ( rate(prometheus_remote_write_container_fs_writes_total{instance_type="k3s",__$labels__}[5m]) )',
      color: '#f5a623',
      dimensions: [{ name: 'device' }]
    }
  ],
