...
| 请求头 | 取值 |
|--------|------|
| `Api-Authorization` | 系统管理 → 设置 → API密钥 中创建的明文 Secret |

密钥所属组织必须在该智能体的使用组织范围内，且渠道已启用，否则会返回未授权或无权使用。
...
将以下代码添加到网页 HTML 中（建议放在 `</body>` 标签之前）。把 `sseUrl` 换成发布页复制的链接，把 `YOUR_USER_API_SECRET` 换成 API 密钥。样式与脚本可使用控制台同源的 `/webchat/` 静态资源。
