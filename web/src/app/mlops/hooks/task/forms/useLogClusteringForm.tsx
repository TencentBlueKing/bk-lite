...
export const useLogClusteringForm = ({ datasetOptions, onSuccess, formRef }: UseLogClusteringFormProps) => {
  ...
  const apiToForm = (data: any) => {
    ...
    const algorithmConfig = LOG_CLUSTERING_ALGORITHM_CONFIGS[algorithm];
  ...
  const handleAsyncDataLoading = useCallback(async (dataset_version_id: number) => {
    ...
    try {
      const { dataset } = await getDatasetReleaseByID('log_clustering', dataset_version_id);
  ...
  const renderOptions = useCallback(async (dataset: number) => {
    ...
    try {
      ...
      // 加载数据集版本
      const datasetVersions = await getDatasetReleases('log_clustering', { dataset });
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (modalState.type === 'add') {
        await addLogClusteringTrainTask(params as any);
      } else {
        await updateLogClusteringTrainTask(formData?.id as string, params as any);
      }
  ...
  }, [modalState.type, formData, onSuccess, addLogClusteringTrainTask, updateLogClusteringTrainTask, formToApi, t, loadingState.confirm]);
  ...
  const renderFormContent = useCallback(() => {
    ...
    const algorithmConfig = currentAlgorithm ? LOG_CLUSTERING_ALGORITHM_CONFIGS[currentAlgorithm] : null;
    ...
    return (
      <>
        ...
        {currentAlgorithm && LOG_CLUSTERING_ALGORITHM_SCENARIOS[currentAlgorithm] && (
          <div style={{ marginTop: -16, marginBottom: 24, fontSize: 12, color: '#999' }}>
            {LOG_CLUSTERING_ALGORITHM_SCENARIOS[currentAlgorithm]}
          </div>
        )}
        ...
        <Form.Item name='max_evals' label="训练轮次" rules={[{ required: true, message: '请输入训练轮次' }]}>
          <InputNumber style={{ width: '100%' }} min={0} placeholder="超参数搜索的评估轮次（0表示跳过搜索）" />
        </Form.Item>
