...
export const useImageClassificationForm = ({ datasetOptions, onSuccess, formRef }: UseImageClassificationFormProps) => {
  ...
  const apiToForm = (data: any) => {
    ...
    const algorithmConfig = IMAGE_CLASSIFICATION_ALGORITHM_CONFIGS[algorithm];
  ...
  const handleAsyncDataLoading = useCallback(async (dataset_version_id: number) => {
    ...
    try {
      const { dataset } = await getDatasetReleaseByID('image_classification', dataset_version_id);
  ...
  const renderOptions = useCallback(async (dataset: number) => {
    ...
    try {
      ...
      // 加载数据集版本
      const datasetVersions = await getDatasetReleases('image_classification', { dataset });
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (modalState.type === 'add') {
        await addImageClassificationTrainTask(params as any);
      } else {
        await updateImageClassificationTrainTask(formData?.id as string, params as any);
      }
  ...
  }, [modalState.type, formData, onSuccess, addImageClassificationTrainTask, updateImageClassificationTrainTask, formToApi, t, loadingState.confirm]);
  ...
  const renderFormContent = useCallback(() => {
    ...
    const algorithmConfig = currentAlgorithm ? IMAGE_CLASSIFICATION_ALGORITHM_CONFIGS[currentAlgorithm] : null;
    ...
    return (
      <>
        ...
        {currentAlgorithm && IMAGE_CLASSIFICATION_ALGORITHM_SCENARIOS[currentAlgorithm] && (
          <div style={{ marginTop: -16, marginBottom: 24, fontSize: 12, color: '#999' }}>
            {IMAGE_CLASSIFICATION_ALGORITHM_SCENARIOS[currentAlgorithm]}
          </div>
        )}
