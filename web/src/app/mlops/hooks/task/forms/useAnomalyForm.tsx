...
export const useAnomalyForm = ({ datasetOptions, onSuccess, formRef }: UseAnomalyFormProps) => {
  ...
  const handleAsyncDataLoading = useCallback(async (dataset_version_id: number) => {
    ...
    try {
      const { dataset } = await getDatasetReleaseByID('anomaly_detection', dataset_version_id);
  ...
  const renderOptions = useCallback(async (dataset: number) => {
    ...
    try {
      ...
      // 加载数据集版本
      const datasetVersions = await getDatasetReleases('anomaly_detection', { dataset });
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (modalState.type === 'add') {
        await addAnomalyTrainTask(params as any);
      } else {
        await updateAnomalyTrainTask(formData?.id as string, params as any);
      }
  ...
  }, [modalState.type, formData, onSuccess, addAnomalyTrainTask, updateAnomalyTrainTask, formToApi, t, loadingState.confirm]);
  ...
  const renderFormContent = useCallback(() => {
    ...
    const algorithmConfig = currentAlgorithm ? ANOMALY_ALGORITHM_CONFIGS[currentAlgorithm] : null;
    ...
    return (
      <>
        ...
        {currentAlgorithm && ANOMALY_ALGORITHM_SCENARIOS[currentAlgorithm] && (
          <div style={{ marginTop: -16, marginBottom: 24, fontSize: 12, color: '#999' }}>
            {ANOMALY_ALGORITHM_SCENARIOS[currentAlgorithm]}
          </div>
        )}
