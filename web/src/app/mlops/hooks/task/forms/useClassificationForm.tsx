...
export const useClassificationForm = ({ datasetOptions, onSuccess, formRef }: UseClassificationFormProps) => {
  ...
  const apiToForm = (data: any) => {
    ...
    const algorithmConfig = CLASSIFICATION_ALGORITHM_CONFIGS[algorithm];
  ...
  const handleAsyncDataLoading = useCallback(async (dataset_version_id: number) => {
    ...
    try {
      const { dataset } = await getDatasetReleaseByID('classification', dataset_version_id);
  ...
  const renderOptions = useCallback(async (dataset: number) => {
    ...
    try {
      ...
      // 加载数据集版本
      const datasetVersions = await getDatasetReleases('classification', { dataset });
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (modalState.type === 'add') {
        await addClassificationTrainTask(params as any);
      } else {
        await updateClassificationTrainTask(formData?.id as string, params as any);
      }
  ...
  }, [modalState.type, formData, onSuccess, addClassificationTrainTask, updateClassificationTrainTask, formToApi, t, loadingState.confirm]);
  ...
  const renderFormContent = useCallback(() => {
    ...
    const algorithmConfig = currentAlgorithm ? CLASSIFICATION_ALGORITHM_CONFIGS[currentAlgorithm] : null;
    ...
    return (
      <>
        ...
        {currentAlgorithm && CLASSIFICATION_ALGORITHM_SCENARIOS[currentAlgorithm] && (
          <div style={{ marginTop: -16, marginBottom: 24, fontSize: 12, color: '#999' }}>
            {CLASSIFICATION_ALGORITHM_SCENARIOS[currentAlgorithm]}
          </div>
        )}
