...
const ChartContent = ({
  ...
  const colmuns: ColumnItem[] = useMemo(() => {
    ...
    if (algorithmType === DatasetType.ANOMALY_DETECTION) {
      _columns.push(
        {
    ...
    return _columns;
  }, [t, convertToLocalizedTime, algorithmType]);
  ...
  }, [fileId, algorithmType, handleLabelData, getTrainDataInfo, t]);
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      // 调用后端API更新文件
      await updateAnomalyTrainDataFile(fileId, formData);
  ...
  }, [currentFileData, fileId, updateAnomalyTrainDataFile, t, getCurrentFileData]);
      ...
      <Spin className="w-full" spinning={loadingState.chartLoading}>
        ...
        >
          <div className="flex justify-between flex-1">
              ...
              <div className="flex-1 relative">
                ...
                >
                  <LineChart
                    ...
                    showDimensionFilter
                    showBrush
                    allowSelect={algorithmType === DatasetType.ANOMALY_DETECTION}
                    onXRangeChange={onXRangeChange}
                    onTimeLineChange={onTimeLineChange}
                    onAnnotationClick={algorithmType === DatasetType.ANOMALY_DETECTION ? onAnnotationClick : undefined}
                  />
                </div>
              </div>
            ...
            <div
              className={`${algorithmType === DatasetType.ANOMALY_DETECTION ? 'w-88.75' : 'w-72.5'} anomaly-container relative`}
            ...
            >
              <div style={{
                ...
                <CustomTable
                  ...
                  size="small"
                  rowKey="timestamp"
                  scroll={{ x: '100%', y: algorithmType === DatasetType.ANOMALY_DETECTION ? tableScrollHeight : 'calc(100vh - 160px)' }}
                  columns={colmuns}
