...
import type { CredentialPoolItem } from './types';
...
export type { CredentialPoolItem } from './types';
export { PASSWORD_PLACEHOLDER, MAX_CREDENTIAL_POOL_SIZE } from './constants';

const makeClientId = () =>
  `cred-local-${Date.now()}-${Math.random().toString(16).slice(2, 8)}`;
...
const createEmptyCredential = (
  ...
  return {
    _client_id: makeClientId(),
    port:
      shape === 'sql' ? (showDatabase ? '1433' : '3306') : shape === 'vm' ? '443' : '22',
    ...(shape === 'vm' ? { ssl: false } : {}),
    ...(shape === 'cloud'
...
function getPreviewFields(
  ...
  if (shape === 'snmp') {
    ...
    return [
      {
        label: t('Collection.SNMPTask.version', '版本'),
        value: item.version || 'v2',
      },
      {
        label:
          item.version === 'v3'
  ...
  if (shape === 'cloud') {
    return [
      {
        label: t('Collection.cloudTask.accessKey', '秘匙'),
      ...
      },
      {
        ...
        value:
          passwordVisible &&
          item.accessSecret &&
          item.accessSecret !== PASSWORD_PLACEHOLDER
            ? item.accessSecret
            : getMaskedSecret(item.accessSecret),
        isSecret: true,
      },
      {
        label: t('Collection.cloudTask.region', '区域'),
        value: item.regionName || item.regionId || '--',
      },
    ];
  }
  ...
  return [
    {
      label:
        shape === 'sql' || shape === 'vm'
          ? t('Collection.VMTask.username', '用户')
          : t('user', '用户'),
      value: username || '--',
    },
    {
      label:
        shape === 'sql' || shape === 'vm'
          ? t('Collection.VMTask.password', '密码')
          : t('password', '密码'),
      value:
        passwordVisible && item.password && item.password !== PASSWORD_PLACEHOLDER
          ? item.password
...
function CredentialMetaField({
  ...
  return (
    <div
      className={`${styles.credentialMetaItem} ${field.value === '--' ? styles.credentialMetaEmpty : ''}`}
    >
    ...
    return (
      <div className={styles.credentialFieldGrid}>
        <InputRow label={t('Collection.SNMPTask.version', '版本')}>
          <Select
            value={version}
            onChange={(nextValue) => updateItem(index, { version: nextValue })}
          >
        ...
        {version !== 'v3' ? (
          ...
          <>
            <InputRow label={t('Collection.SNMPTask.securityLevel', '安全级别')}>
              <Select
                value={level}
                onChange={(nextValue) => updateItem(index, { level: nextValue })}
              >
                <Select.Option value="authNoPriv">
                  {t('Collection.SNMPTask.authNoPriv', '认证不加密')}
                </Select.Option>
                <Select.Option value="authPriv">
                  {t('Collection.SNMPTask.authPriv', '认证加密')}
                </Select.Option>
              </Select>
            </InputRow>
            <InputRow label={t('Collection.SNMPTask.userName', '用户')}>
              <Input
                ...
                placeholder={t('common.inputTip', '请输入')}
                onChange={(event) =>
                  updateItem(index, { username: event.target.value })
                }
              />
            </InputRow>
          ...
          <Input
            ...
            onChange={(event) => {
              updateItem(index, {
                ...
                ...(editMode && item.accessSecret === PASSWORD_PLACEHOLDER
                  ? { accessSecret: '' }
                  : {}),
                regionId: undefined,
          ...
          <SecretInput
            ...
            onChange={(nextValue) => {
              updateItem(index, {
                ...
                ...(editMode && item.accessKey === PASSWORD_PLACEHOLDER
                  ? { accessKey: '' }
                  : {}),
                regionId: undefined,
                regionName: undefined,
          ...
          <div className={styles.credentialInlineControl}>
            <Select
              ...
              onChange={(nextValue, option) => {
                ...
                updateItem(index, {
                  regionId: nextValue,
                  regionName: typeof label === 'string' ? label : undefined,
                });
              }}
...
export default function CmdbCredentialPoolEditor({
  value = [],
  ...
  const onDragEnd = (event: any) => {
    ...
    const oldIndex = normalizedValue.findIndex(
      (item, index) => getItemKey(item, index) === active.id,
    );
    const newIndex = normalizedValue.findIndex(
  ...
  const renderCredentialCard = (item: CredentialPoolItem, index: number) => {
    ...
    return (
      <>
        ...
        <div
          className={`${styles.credentialCard} ${expanded ? styles.credentialCardExpanded : ''}`}
        >
          <div className={styles.credentialCardHeader}>
            ...
            >
              <div className={styles.credentialTitleBlock}>
                <div className={styles.credentialTitle}>
                  {`${t('Collection.credential', '凭据')} ${index + 1}`}
                </div>
              </div>
              {!expanded && (
            ...
            <div className={styles.credentialActions}>
              {allowRemove && (
                <Tooltip
                  title={normalizedValue.length <= 1 ? '至少保留 1 个凭据' : '删除凭据'}
                >
                  <Button
                    type="text"
                    danger
  ...
  return (
    <div className={`credentialPoolEditor ${styles.credentialPoolEditor}`}>
      ...
      {sortableEnabled ? (
        ...
        <ul className={styles.credentialPoolList}>
          {normalizedValue.map((item, index) => (
            <li
              key={getItemKey(item, index)}
              className={styles.credentialStaticItem}
            >
              {renderCredentialCard(item, index)}
            </li>
          ))}
