            ...
            <Space size={4}>
              <Select
                ...
                onChange={(value: TimeRange) => {
                  setTimeRange(value);
                  router.replace(`/apm/traces${entityMode === 'spans' ? '?entity=spans' : '?entity=traces'}`);
                }}
              />
              <Select
                size="small"
                  ...
                  <ApmSurface padding="none" className="overflow-hidden">
                    {entityMode === 'spans' ? (
                      <Table
                        ...
                        pagination={false}
                        onRow={(item) => ({
                          onClick: () => router.push(`/apm/traces/${item.trace_id}?span_id=${item.span_id}`),
                          onKeyDown: (event) => {
                            if (event.key === 'Enter' || event.key === ' ') {
                              event.preventDefault();
                              router.push(`/apm/traces/${item.trace_id}?span_id=${item.span_id}`);
                            }
                          },
                          role: 'link',
