...
from utils import load_yaml_packs_to_dataframe, write_lang_yaml_file


class ServerLangSyncer:
    """Server 后端语言包同步器"""

    def __init__(self, notion_token: str, server_lang_config: str):
        """初始化

        Args:
            notion_token: Notion API Token
            server_lang_config: Server 语言包配置，格式: app_name:database_id,app_name:database_id
        """
        self.client = Client(auth=notion_token)
        self.server_lang_config = server_lang_config
    ...
    def push(self):
        ...
        config_items = self.server_lang_config.split(',')
    ...
    def _push_single_app(self, app_name: str, database_id: str):
        ...
        # 加载语言包到 DataFrame
        df = load_yaml_packs_to_dataframe(lang_pack_path)
        if df.empty:
    ...
    def _add_new_keys(
        ...
        for _, row in tqdm(new_rows.iterrows(), total=len(new_rows), desc=f"构建 {app_name} 数据"):
            properties = {
                ...
                "zh-Hans": {
                    "rich_text": [{"text": {"content": str(row.get('zh-Hans', ''))}}]
                },
                "en": {
    ...
    def _get_lang_pack_path(self, app_name: str) -> Path:
        ...
        lang_pack_path = Path(__file__).parent / \
            f"../../server/apps/{app_name}/language/"
