...
- 按数据源类型分别维护连接配置与取数配置；定义返回数据结构（用于表格列、图形标签等）。
...
- 数据源配置页支持测试连接 / 数据预览：数据库预览仅允许单条 `SELECT` 查询或按表限量取数；Excel 仅支持 `.xlsx` 文件并受文件大小限制。

相关架构：[[spec/ARD/modules/operation_analysis.md#2. 数据模型与存储【已实现/已存在 / PostgreSQL】]]；对应功能清单：[[spec/fuctionlist/09-运营分析-功能清单.md#5. 数据源管理]]
> 证据来源：server/apps/operation_analysis/models/datasource_models.py:111-145，server/apps/operation_analysis/views/datasource_view.py:51-79,481-529，server/apps/operation_analysis/services/datasource_preview/database.py:15-110，server/apps/operation_analysis/services/datasource_preview/excel.py:32-85　|　同步基线：83091efe　|　【已实现】
...
- 数据源连接配置中的密码、令牌、密钥等敏感字段加密 / 脱敏处理，编辑时允许保留既有掩码值。

相关架构：[[spec/ARD/modules/operation_analysis.md#3. 接口【已实现/已存在】]]；对应功能清单：[[spec/fuctionlist/09-运营分析-功能清单.md#5. 数据源管理]]
> 证据来源：server/apps/operation_analysis/serializers/datasource_serializers.py:11-43,66-103，server/apps/operation_analysis/services/datasource_preview/database.py:15-45　|　同步基线：83091efe　|　【已实现】

## 5. 关键技术架构选择

- NATS 数据源取数经命名空间对应的消息连接路由到下游服务，下游统一返回结构化结果后归一校验。
- 数据库、REST API、Excel 数据源支持在管理页内联预览，预览结果自动推断字段结构。
...
> 证据来源：server/apps/operation_analysis/views/datasource_view.py:72-79,411-529，server/apps/operation_analysis/services/datasource_preview/schema.py:1-33　|　同步基线：83091efe　|　【已实现】

## 6. 验收标准

- 用户可配置 NATS、数据库、REST API、Excel 四类以上数据源，并定义查询参数与返回结构。
- 命名空间可配置连接与鉴权，密码加密存储。
