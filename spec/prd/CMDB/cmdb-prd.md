...
- 自动发现：采集对象树、采集任务管理、采集插件说明、任务审批、SOID 库。
- 变更记录：变更日志与时间筛选。
- 数据订阅：围绕模型与实例配置订阅规则、接收对象与通知渠道。
- 个人配置：展示字段与用户保存的筛选条件等个性化配置。
- 公共选项库：为枚举类属性提供可复用的选项集合。
...
- 公共选项库被引用时不允许删除。
- 公共选项库选项变更后，关联枚举属性同步刷新。

### 5.11 权限规则

- 模型、实例、采集任务按组织与对象权限控制可见范围和可操作范围。
- 订阅规则按所属组织控制可见范围和管理范围。
...
- 自动发现：选择采集对象 → 配置任务 → 执行采集/审批通过 → 查看采集摘要。
...
- `input_method`、`examine`：录入方式与审批状态
...
- Should：资产视图、批量导入导出、采集文档、SOID 库、数据订阅。
