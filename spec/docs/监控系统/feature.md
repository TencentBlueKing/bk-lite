...
## 4. 采集与对象接入能力 (Integration & Ingestion)
平台提供了开箱即用的丰富外围资源监控接入方案，同时保留了极高自由度的自定义数据上报通道。

### 4.1 覆盖云云原生与基础架构
- **云原生体系监控**：新增并全面优化 K8s 与 Docker 监控接入，并支持在告警列表直观展示云原生的告警类型谱系。
- **公有云资源监控**：新增对腾讯云等公有云资源的直连监控能力，跨越混合云架构限制。
- **中间件与虚拟化组件**：全面调优各类监控对象的接入能力，深度修复诸如 Kafka SASL 参数构建、MySQL特殊凭据解析以及 VMware 采集异常等边缘故障。

### 4.2 高效的批量与自定义接入
- **文件化快速部署**：支持直接通过 Excel 批量导入庞大的采集配置项，极其适合大型企业在新环境实施期间的监控项光速铺配。
- **OpenAPI 数据上报**：开放了灵活的监控对象管理与指标上报 API，原生支持 PULL（拉取）与 PUSH（推送）双重模式，彻底打通非标业务系统数据的自定义监控闭环。
