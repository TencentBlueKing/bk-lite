...
*   **卡片式全能集成与跨平台兼容**：采用如同商超般的卡片布局。内置的 Vector 探针已全面更新，完美兼容所有主流 Linux 发行版环境，提供对 `Syslog`、`File`、`Docker` 的优化直采体验（包含 Docker 日志挂载提取层与 Syslog 复杂协议头对接强化）；不仅如此，甚至涵盖 `Packetbeat` 对网络流量的侦视。
...
*   **智能 Top 值分布与极速带入**：查询面板左侧新增独立字段统计树，支持直接查看高频字段的 Top 值分布情况（如快速定位出现错误数最多的独立 IP 或接口径）。明细模式下，也支持将特定日志键值一键追加至顶部搜索框补全查寻。
