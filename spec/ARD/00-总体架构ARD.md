...
7. **基础设施层**：PostgreSQL/MySQL/达梦/GaussDB/GoldenDB/OceanBase（多引擎）、Redis（可选缓存）、RabbitMQ（Celery broker）、NATS（消息/RPC）、MinIO（对象存储）、MLflow（模型注册）、VictoriaMetrics/VictoriaLogs（时序/日志）。
...
- 多数 app 提供 `open_api/*` 端点（如 `cmdb/open_api/k8s_setup`、`monitor/open_api/infra`、`alerts/open_api/k8s`、`log/open_api/k8s`），用于免登录/集成场景。
...
- 典型 RPC 域：`executor`（local/ssh execute、download/upload、unzip、health）、`ansible`、`cmdb`、`node_mgmt`、`system_mgmt`、`opspilot`、`operation_analysis`（独立 server/namespace）。
...
| Redis | 缓存（可选，默认 LocMem） | `config/components/cache.py` | 已实现 |
...
### 9.1 认证【已实现/已存在】
- 多认证后端：`APISecretAuthBackend`（API 密钥）、`AuthBackend`（Token 经 system_mgmt 校验）、Session。（`apps/core/backends.py`）
...
| **MySQL 引擎疑似缺陷** | `DB_ENGINE=mysql` 时 ENGINE 指向达梦 backend（`database.py:31`，`cw_cornerstone.db.dameng.backend`），MySQL 可能不可用 | 已实现（缺陷，待确认是否有意） |
