...
| 命名查询保存 | 将命名查询保存到一个或多个组织范围 | 保存须填写名称并选择组织 | GA |
...
| 功能项 | 功能说明 | 规格 / 约束 | 状态 |
|---|---|---|---|
| 策略列表操作 | 策略的新增、编辑、删除、启停 | 停用策略时当前活跃告警自动关闭 | GA |
...
| 采集探测任务 | 发起接入前采集探测并轮询任务结果 | 创建接口返回 `task_id/status`；结果查询返回 `status/phase/result/error_message/started_at/finished_at`；仅创建人可查看结果 | GA |
...
> 证据来源：server/apps/monitor/views/collect_detect.py:33-86，server/apps/monitor/services/collect_detect.py:29-69　|　同步基线：83091efe　|　【已实现】
...
| 内置对象覆盖 | 平台内置监控对象覆盖多类资源 | 已随包**预置标准指标集**（开箱即用）的对象见"五、支持的监控对象与指标范围"，约 35 个采集插件 / 约 42 个对象 / 约 700 指标。注：对象类型列表中另有 **ClickHouse、SNMP Trap** 等条目，其对象类型已内置但**未随包预置标准指标集，需结合自建模板（API/PULL/SNMP）配置后使用**，不计入开箱预置范围 | GA（ClickHouse / SNMP Trap 需模板扩展） |
