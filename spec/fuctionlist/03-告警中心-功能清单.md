...
**编制依据：** 告警中心 PRD v1.2（2026-05-28）与 `server/apps/alerts`、`web/src/app/alarm` 源代码核对
...
| 功能项 | 功能说明 | 规格 / 约束 | 状态 |
|---|---|---|---|
| 入站屏蔽 | 事件入站后优先匹配屏蔽规则，命中即标记 `SHIELD`，不进入有效告警链路 | — | GA |
...
| 功能项 | 功能说明 | 规格 / 约束 | 状态 |
|---|---|---|---|
| 指纹聚合 | 基于相关性策略计算分组键（fingerprint），对活跃告警按 fingerprint 更新而非重复创建 | 策略参数含 `match_rules`、`group_by`、窗口配置 | GA |
...
| 合成告警 | 缺失触发时按模板（名称/级别/摘要）生成告警 | 未恢复期间不重复生成缺失告警 | GA |
