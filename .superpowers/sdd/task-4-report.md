...
- Task 4 聚焦集合：`86 passed in 29.58s`（格式整理后的最终复跑）。
- 覆盖率集合：`86 passed`；`diff-cover` 统计 119 个差异可执行行、22 行未覆盖，差异覆盖率 **81%**，高于 75% 门禁。
- 并入 Task 3 监控直接对象权限和业务流的扩大集合：`325 passed in 104.15s`。
...
- Black 23.1：两个新增/重写测试文件通过；两个生产文件及两个既有测试文件仍有仓库基线格式差异，本任务未做整文件机械格式化。
...
- `monitor/nats/monitor.py` 是历史大模块，整模块覆盖率为 68%；本任务以差异覆盖率 81% 和 325 项权限业务回归约束新增路径。
